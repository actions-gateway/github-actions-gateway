...
**If you use linked worktrees, leave `core.hooksPath` relative.** `make hooks` sets it to `.githooks`, which each worktree resolves against its own checkout — that is the point. Pinning it to an absolute path (`/path/to/main/.githooks`, e.g. via a `config.worktree` override) makes every worktree run the *main* checkout's hook against *its own* tree. The two disagree whenever a branch moves or renames anything the hook invokes, so a PR doing that breaks commits in every worktree until it merges — with an error naming a path that looks correct. Q571 hit exactly this when `lint-backlog.sh` moved into `scripts/docs/`. Check yours with `git config --show-origin --get-all core.hooksPath`.
