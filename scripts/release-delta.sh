...
#
# Usage:
#   scripts/release-delta.sh [FROM] [TO]
#
...
section "API surface (semver signal; review with scripts/api-surface-since.sh $from)" "$api_stat"
section "Operator-facing docs (curate notes with scripts/operator-caveats-since.sh $from)" "$ops_stat"
