#!/usr/bin/env bash
#
# Unit tests for scripts/git-merge-status.sh — the docs/STATUS.md merge driver.
#
...
DRIVER="$REPO_ROOT/scripts/git-merge-status.sh"
...
merge_repo() {
	...
	cp "$DRIVER" "$repo/scripts/git-merge-status.sh"
	cp "$REPO_ROOT/scripts/lib/merge-status-rows.awk" "$repo/scripts/lib/"
	chmod +x "$repo/scripts/git-merge-status.sh"
	(cd "$repo" && ./scripts/git-merge-status.sh --install >/dev/null)
	printf '%s\n' "$repo"
}
