#!/usr/bin/env bash
#
# Unit tests for scripts/check-path-filters.sh (Q429). The gate exists to fail on
...
# shellcheck source=scripts/check-path-filters.sh
source "$REPO_ROOT/scripts/check-path-filters.sh"
...
# An extglob prefix (e2e-test.yml's scripts/ enumeration) has an unknowable
# covered set, so it counts as no coverage rather than being assumed to match.
...
if (scripts/check-path-filters.sh >/dev/null 2>&1); then
	ok tracked-workflows-pass 'make path-filters-check is green'
else
	bad tracked-workflows-pass 'run scripts/check-path-filters.sh for the report'
fi
