...
| [check-no-plan-refs-in-code-test.sh](docs/check-no-plan-refs-in-code-test.sh) | Unit tests for `check-no-plan-refs-in-code.sh` — plants the defect in throwaway repos (stale citation in a shell comment, a workflow comment and Go source) and pins the exceptions that must stay green (a `paths:` filter, a plan file opened as data, the opt-out marker, a bare directory, the index). Runs under `make scripts-test`. |
