...
PRIORITYCLASSALLOWLIST_BLOCK="$(cat <<'EOF'
    ...
    #
    # The cost lands on UPGRADES, which skip crds/ entirely: a release created
    # before this CRD existed needs a one-time `kubectl apply -f crds/` (the chart
    # preflights that and fails with the command), and adding a field here later
    # needs the same step in the release notes, or the apiserver silently prunes
    # the new field from the rendered CR. See docs/operations/upgrade.md.
EOF
)"
V2_BLOCK="$(cat <<'EOF'
