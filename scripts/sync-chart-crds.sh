...
PRIORITYCLASSALLOWLIST_BLOCK="$(cat <<'EOF'
    ...
    #
    # The cost is that `helm upgrade` does NOT carry schema changes to this one
    # CRD (Helm never upgrades crds/). Adding a field here therefore needs a
    # documented manual `kubectl apply` step in the release notes, or the
    # apiserver silently prunes the new field from the rendered CR. See
