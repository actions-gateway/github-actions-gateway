...
assert_registry_complete() {
  ...
  scripts/check-codegen-drift.sh — its $DEEPCOPY_FILE files are unguarded. Add it to
...
assert_registry_fidelity() {
  ...
  $registry in scripts/check-codegen-drift.sh. Drop the registration, or restore the target."
		return
	fi
  ...
  Add it to the registration."
	...
	for pair in $outputs; do
		...
		if grep -qF -- "$rule" <<<"$recipe" && ! grep -qF -- "$rule$dir " <<<"$recipe "; then
			fail "$module: $registry in scripts/check-codegen-drift.sh sends '$kind' output to
