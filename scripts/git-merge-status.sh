...
#
#   make merge-driver     # or: scripts/git-merge-status.sh --install
#
...
install_driver() {
	...
	git config "merge.$DRIVER_NAME.driver" \
		'scripts/git-merge-status.sh %O %A %B %L %P %S %X %Y'
	printf 'merge driver installed: merge.%s -> scripts/git-merge-status.sh\n' "$DRIVER_NAME"
