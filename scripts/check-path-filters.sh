...
# are scripts/check-path-filters-test.sh.
set -euo pipefail
...
assert_registry_complete() {
  ...
  scoped to one gate's inputs — say why) in scripts/check-path-filters.sh."
	done
	for key in "${registered[@]}"; do
		contains "$key" "${found[@]}" && continue
		fail "scripts/check-path-filters.sh registers '$key', but $WORKFLOW_DIR/${key%%:*} declares no such filter.
