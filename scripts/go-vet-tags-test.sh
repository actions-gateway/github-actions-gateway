#!/usr/bin/env bash
#
# Unit tests for scripts/go-vet-tags.sh (Q404). Two properties, both asserted
...
# workspace) on source.
# shellcheck source=scripts/go-vet-tags.sh
source "$REPO_ROOT/scripts/go-vet-tags.sh"
...
if assert_tags_cover_tree "$BUILD_TAGS" "${tree_patterns[@]}" >/dev/null 2>&1; then
	...
	printf 'FAIL %-34s a tracked first-party .go file is excluded even with -tags %s; run %s\n' \
		tree-fully-covered "$BUILD_TAGS" "$REPO_ROOT/scripts/go-vet-tags.sh" >&2
	fails=$((fails + 1))
fi
