...
# GUI dev shell; a no-op on CI/headless, see scripts/local-throttle.sh.
set -euo pipefail
...
assert_tags_cover_tree() {
	...
	echo "new tag to BUILD_TAGS in scripts/go-vet-tags.sh, or, if the constraint is" >&2
...
main() {
	...
	# same trick scripts/go-test.sh uses).
	local patterns=() dir
