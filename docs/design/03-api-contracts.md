...
| **POST** | `{run_service_url}/completejob` | Worker pod runner binary; AGC Goroutine (duplicate-delivery only) | Reports a job's terminal result (`planId`, `jobId`, `result`). Normally called by the worker pod's runner binary for the job it ran. The AGC calls it directly **only** for a deduplicated duplicate delivery it acquired but will not run (Q260 follow-up): the loser releases its per-delivery assignment (result `skipped`, on its own `jobId`) so GitHub does not cancel the job at its ~15-minute unstarted-job timeout. **Auth:** job-scoped token, same as `renewjob`. **Guarded / not live-confirmed:** the AGC's call is off by default (`AGC_COMPLETE_ABANDONED_DELIVERIES`) pending a dogfood turn-up that confirms the run service's per-delivery completion semantics and the exact `result` serialization — see [`docs/plan/q260-planid-dedup-refix.md`](../plan/q260-planid-dedup-refix.md). |
