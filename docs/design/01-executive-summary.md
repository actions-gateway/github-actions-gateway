...
**What the displaced job costs, stated plainly.** The scheduling guarantee above is measured and real; the recovery of the work it displaces is not automatic today. A preempted worker's job concludes on GitHub but is **not** re-run by the gateway — see the paragraph after next — so oversubscription trades idle headroom for manual re-runs of the opportunistic tier until [Q497](../STATUS.md#Q497) lands. Size the floor tiers accordingly, and prefer this lever where the low-priority work is cheap to repeat.

When the **kubelet** evicts a worker under node pressure — memory or disk exhaustion — the AGC automatically re-queues the affected job without user intervention. It detects the `Evicted` pod status, stops renewing the job lock to prompt a fast GitHub cancellation, and calls GitHub's rerun API to reschedule the job. Both acquisition tiers do this, by different routes: on the classic tier the Job Lock Renewer observes the pod it is already watching, while the scale-set tier — which provisions fire-and-forget — records the workflow run on the worker pod and lets the owning reconciler detect the eviction from the pod itself (Q417). A configurable retry budget, shared across both tiers per workflow run, prevents infinite loops on persistently failing workloads. Jobs that exhaust their retry budget surface via a dedicated metric rather than silently disappearing.

**Preemption by the floor tier is *not* on that path, measured 2026-07-29 (Q423).** Preemption is kube-scheduler's, and the scheduler removes its victim by *deleting* it; only the kubelet's node-pressure eviction produces the `Evicted` status the recovery above keys on. A pod displaced by a higher-priority tier therefore gets the graceful-termination path — the Q385 SIGTERM relay gives the runner its grace period to abort and report, so the job concludes on GitHub promptly rather than hanging — but **no automatic re-run**, exactly as with an operator's `kubectl drain`. Closing that is tracked as [Q497](../STATUS.md#Q497), and it is more tractable than the drain case: a preempted pod carries a `DisruptionTarget` condition with reason `PreemptionByScheduler`, which nothing but the scheduler writes. Detail: [§4.2](04-operational-flows.md#worker-pod-eviction-and-auto-retry) and [the experiment](../plan/eviction-oversubscription-validation.md#result-measured-2026-07-29-preemption-is-not-eviction).
