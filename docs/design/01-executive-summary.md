...
- **Worker eviction is no longer a paging event.** Preempted, OOM-killed, and node-lost jobs are recovered automatically. The recurring "tenant X's CI is failing intermittently, please look at the runner pod" ticket pattern largely goes away. *Today this runs on the classic acquisition tier only; a `ScaleSet`-protocol set — the default, and `v2beta1`'s only option — still needs a manual rerun ([§4](04-operational-flows.md#worker-pod-eviction-and-auto-retry)).*
...
This recovery is **classic-acquisition-tier only as shipped**. The `ScaleSet` protocol provisions its worker fire-and-forget, so the AGC never observes the pod's terminal phase and never fires a rerun; because `ScaleSet` is the default and the only protocol `v2beta1` exposes, the capability today covers the tier that `v2.0.0` removes. [Q417](../STATUS.md#Q417) ports it ([plan](../plan/scaleset-eviction-recovery.md)) and gates that removal. Read every eviction-recovery claim below as scoped to the classic tier until it lands.
...
2. **Automatic eviction retry.** When a worker pod is evicted, the AGC detects it and stops renewing the job lock, so GitHub cancels the run once the outstanding lock lapses (within the remaining lock window, ~10 minutes at worst); the AGC then re-queues the job via GitHub's rerun API, with no user action required. A configurable retry budget caps automatic retries per job; exhausted budgets are surfaced as a metric rather than silent failures. *Requirement, not yet a property of every tier — implemented on classic acquisition only, with [Q417](../STATUS.md#Q417) porting it to `ScaleSet`.*
