...
When eviction does occur — either from the floor tier preempting lower-priority pods, or from external pressure such as node memory exhaustion — the AGC automatically re-queues the affected job without user intervention. It detects the `Evicted` pod status, stops renewing the job lock to prompt a fast GitHub cancellation, and calls GitHub's rerun API to reschedule the job. Both acquisition tiers do this, by different routes: on the classic tier the Job Lock Renewer observes the pod it is already watching, while the scale-set tier — which provisions fire-and-forget — records the workflow run on the worker pod and lets the owning reconciler detect the eviction from the pod itself (Q417). A configurable retry budget, shared across both tiers per workflow run, prevents infinite loops on persistently failing workloads. Jobs that exhaust their retry budget surface via a dedicated metric rather than silently disappearing.
