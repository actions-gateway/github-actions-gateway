...
2. **Shared quota with per-group priority guarantees.** A tenant may define multiple `RunnerGroup`s (e.g. one for CPU jobs, one for GPU jobs, one for sandboxed external PRs) and have all of them draw from a single namespace-level `ResourceQuota`. There is no per-group cap to coordinate at the namespace level. Within that shared pool, each `RunnerGroup` optionally defines a `priorityTiers` list: a ranked set of Kubernetes `PriorityClass` assignments and cumulative pod-count thresholds. The first N pods of a high-priority group (e.g. GPU runners) are assigned a preempting `PriorityClass` and will displace lower-priority pods to guarantee they schedule; additional pods beyond that threshold are scheduled opportunistically at a lower priority; and a final threshold caps the group's total concurrency. This lets a platform team express "GPU runners always get at least 5 slots, up to 20 if capacity is available, and never more than 30" without reserving idle resources — the preemption guarantee only activates when the namespace is actually contended.
