...
The attestable form reads the image from the registry before the container runs, which also covers the digest-only case and needs no pod (Q988).
It is a registry-integration feature rather than a status field: `cmd/agc/internal/runnerimage` fetches the manifest and streams the layers topmost-first for the same `deps.json`, an async resolver caches the reading per reference (re-checking a tag's manifest digest hourly and re-streaming only when it moved) and wakes the reconciler through the same `wakeCh` the listener uses, and `runnercore` folds the reading into the verdict: the tag reading stands while the read is pending or failed, and the layer reading overrides it once it lands.
The trust argument is the one the tag already rests on, since both are tenant-authored and the registry copy is the stronger: immutable once addressed by digest, and out of reach of anything running in the container.
So `RunnerVersionTooOld` is verdict-bearing for a custom or digest-only image, and `observedRunnerVersion` keeps its self-report role for the case the registry is out of the AGC's reach ([05-security.md](05-security.md#the-registry-read-behind-runnerversiontooold)).
