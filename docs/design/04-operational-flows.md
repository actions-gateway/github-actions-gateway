...
Two causes reach the recovery above, and the boundary between them and everything else is the whole safety argument:
...
| `kubectl drain` / eviction API | Deleted, with `DisruptionTarget=True` / `EvictionByEvictionAPI` | ❌ *not yet* | Decided by Q459, implementation carried by [Q502](../STATUS.md#Q502) — gated on the deletion mark, not on this condition |
| `kubectl delete pod`, the reaper | Deleted, `deletionTimestamp` only | ❌ *not yet* | Same. The reaper's own deletions must be excluded when it lands, or it would re-run every job it cleans up |
| Job failed on its own | `PodFailed`, empty reason | ❌ | Re-running genuinely failing work is a retry loop, not a recovery |

The two "not yet" rows are a **sequencing** boundary, not a permanent one, and the reason they are separate from preemption is worth being precise about. Preemption could close first because `PreemptionByScheduler` has exactly one writer — never a human, never a failing job — so it needs no further measurement to be safe. The drain rows key on `deletionTimestamp` instead, which a human cancelling a run might also have set; that had to be measured before it could be trusted. It since was (2026-07-29): a cancelled run's worker publishes the same phase and empty reason with **no** deletion mark, so the mark does separate a disruption from a cancel. Q502 carries that implementation.
...
One residual, inherent to the signal rather than the implementation: on the scale-set tier a preemption recovery is **not** restart-safe. An evicted pod sits in `PodFailed` until the reaper takes it, so a late scan still finds it; a preempted pod is being deleted and is readable only for its termination grace period (30s by default). An AGC down for that whole window loses the marker and the displaced run needs a manual re-run. What keeps the window reachable in normal operation is the worker-pod watch predicate, which admits the update where a pod *newly becomes* a preemption victim — a preemption changes no phase, so the phase-change edge alone would deliver nothing until the delete event, by which point the pod is out of the cache. The classic tier has no such window: its provisioning goroutine is already watching the pod and reads the marker off the resolving event, including the informer's delete event.
