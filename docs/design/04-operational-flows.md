...
**Both halves of that are measured against live GitHub, on both acquisition tiers (Q396) — and the refusal window is why the retry loop exists.** A worker killed by a real kubelet eviction, on a job carrying no `timeout-minutes` to confound the timing, saw GitHub conclude the job `failure` **9m36s** after the kill — consistent with the lock TTL being the mechanism, since the runner is SIGKILLed with roughly two seconds of grace and never gets its own report out. Reproduced twice on 2026-08-03 against the published `v1.3.0` images: **9m37s** on the classic tier and **9m38s** on scale-set, where detection runs from the owning reconciler instead (Q417). Three measurements within two seconds of each other put the figure on the lock TTL rather than on anything tier-specific. The AGC originally fired the re-run exactly once, 5 seconds after the eviction — squarely inside the window GitHub refuses — so the budget was spent, `actions_gateway_eviction_retries_total` incremented, and the job was never re-run (the Q503 defect). Treating the still-running refusal as "not yet" rather than a spent attempt is what closed it, and the retry loop is measured absorbing that window: **20 paced calls before GitHub accepted, on each tier**. See [eviction-oversubscription-validation.md § Result](../plan/eviction-oversubscription-validation.md#result-measured-2026-07-29) and [§ Scale-set result](../plan/eviction-oversubscription-validation.md#scale-set-result-measured-2026-08-03).
