...
**Both halves of that are measured against live GitHub, on both acquisition tiers (Q396) — and the refusal window is why the retry loop exists.** A worker killed by a real kubelet eviction, on a job carrying no `timeout-minutes` to confound the timing, saw GitHub conclude the job `failure` **9m36s** after the kill — consistent with the lock TTL being the mechanism, since the runner is SIGKILLed with roughly two seconds of grace and never gets its own report out. Reproduced on the classic tier on 2026-08-03 against the published `v1.3.0` images at **9m37s**, and once on scale-set at **9m38s**, where detection runs from the owning reconciler instead (Q417). **The scale-set figure is one observation, not a steady state:** a second run of the same spec on the same build saw GitHub conclude the job 17 seconds after it started — the graceful path's signature (see below) — and accept the re-run on the first call. A kubelet ephemeral-storage eviction is therefore not reliably ungraceful, and how long GitHub takes depends on whether the runner's report escapes, not on which tier detected the loss. Quote the ~10-minute figure for the case where nothing is reported; do not quote a scale-set-specific number. The AGC originally fired the re-run exactly once, 5 seconds after the eviction — squarely inside the window GitHub refuses — so the budget was spent, `actions_gateway_eviction_retries_total` incremented, and the job was never re-run (the Q503 defect). Treating the still-running refusal as "not yet" rather than a spent attempt is what closed it, and the retry loop is measured absorbing that window: **20 paced calls before GitHub accepted**, on the classic tier and on the scale-set run whose report did not escape. See [eviction-oversubscription-validation.md § Result](../plan/eviction-oversubscription-validation.md#result-measured-2026-07-29) and [§ Scale-set result](../plan/eviction-oversubscription-validation.md#scale-set-result-measured-2026-08-03).
...
Three causes reach the recovery above, and the boundary between them and everything else is the whole safety argument:
...
| Deleted before its container ever ran | Vanishes, or publishes a transient `Failed` + mark with **no container exit record** (a drained *Pending* worker) | ❌ | The job never ran to a reportable end, so there is no failed job for `rerun-failed-jobs` to act on. Handled instead by releasing the assignment — see [Stuck-Pending Worker Pod](#stuck-pending-worker-pod) (Q628) |
...
sequenceDiagram
    ...
    A->>GH: completejob(result=abandoned) on this delivery's own job ID
    Note over GH: assignment released — no runner ever registered
```
...
**The job assignment is released explicitly (Q628).** "Deletion is treated as completion" is a statement about the *pod*, and for a worker that never ran it used to be reported to the session as a **succeeded job** — indistinguishable from a clean run, so nothing told GitHub anything and the assignment sat acquired-but-unresolved until the ~15-minute unstarted-job timeout cancelled the whole run. A pod removed before any container started now resolves with `DeletedBeforeStart`, the session reports the job as `abandoned` — neither succeeded nor failed describes a job with no step in it — and the listener issues `completejob` on its own delivery, exactly as the Q260 fan-out does for a deduped sibling's identically dangling assignment. It is gated by the same `AGC_FANOUT_COMPLETION` switch and counted by `actions_gateway_abandoned_delivery_completions_total`.
