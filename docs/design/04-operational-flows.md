...
2a. **Admit (Q59, #784, Q405, Q406):** Before claiming the job, the goroutine consults the pre-acquisition admission gate, which asks three questions in order. **Quota:** can the namespace `ResourceQuota` admit one more worker pod right now (`hard − used` against this owner's worker footprint)? **Capacity:** if the owner opted into a capacity gate, can the cluster currently *place* one more worker pod of this shape? **Ceiling:** is the in-memory, per-RunnerGroup reservation counter below the worker ceiling (`maxWorkers` / the maximum `priorityTiers` threshold)? A no from any of them **skips `acquirejob`**, increments `actions_gateway_jobs_admission_rejected_total{reason}` (`quota`, `capacity`, or `ceiling`), and resumes polling at step 1 — the job stays queued at GitHub and is redelivered to a sibling session with capacity, rather than acquired-then-dropped. When the gate admits, it reserves a slot held until the job completes (step 11), then proceeds to step 3. The two observed rungs fail open and reserve nothing, which is why they precede the ceiling; the reservation is fail-safe soft state (reset on AGC restart). The provisioner's post-acquire ceiling check and quota-retry loop (step 6) remain the authoritative backstops.
   ...
   **The capacity rung is self-clearing, and that is what keeps it safe.** It reads the `WorkerCapacityDeclined` condition, which is derived from the *existence* of a stuck worker pod. So the gate closes, the reaper deletes that pod at `pendingPodDeadline`, the condition clears, one job is claimed, and if capacity is still absent the new pod trips it again. A burst of *N* jobs that would each have been claimed and cancelled becomes roughly one wasted claim per deadline window — a bound on the rate, not an elimination. Nothing here removes the need for `pendingPodDeadline` and the reaper.
...
| Capacity (Q405, Q406) | this set's non-terminal worker pods, while its capacity gate is declining — no room for one more | no bound (gate `Off`, an unimplemented mode, nothing declining, or nothing readable) |
...
Deletion is deliberately excluded. An API-initiated eviction (`kubectl drain`), a **kube-scheduler preemption** (what a `priorityTiers` `PriorityClass` drives), or a plain delete all leave a graceful termination, where the Q385 SIGTERM relay hands the signal to the runner and it reports its own outcome; such a pod is deleted rather than left `Failed`/`Evicted`, so it never reaches this path — covering it would double-report the case the relay already owns.

**Q421 measured that exclusion on both tiers, 2026-07-27, and it holds — with a consequence worth stating plainly.** A real `kubectl drain` against a live worker pod publishes no terminal phase at all (the pod goes straight from its current phase to absent), so neither tier's detection sees anything and no automatic re-run fires; the envtest pair (`TestAGC_Drain_ClassicWorkerEviction_DoesNotRerun`, `TestAGC_Drain_ScaleSetWorkerEviction_DoesNotRecover`) and the Tier B `E2E_AGC_WorkerNodeDrain` pin it. The consequence is that a *graceful* operator drain leaves the interrupted run needing a manual re-run, while an *ungraceful* kubelet eviction of the same job recovers automatically — and the worker pod's `safe-to-evict: false` / `do-not-disrupt` annotations do not deflect a drain either, being advisory to autoscalers and deschedulers only. Whether that asymmetry should be closed depends on what GitHub does with the runner's relayed report. [Q459](../STATUS.md#Q459) measured that at Tier C on 2026-07-28: the report does get out, the job concludes `failure` well under a minute after the disruption (15–26s across five runs), and `rerun-failed-jobs` is accepted, so an automatic re-run is available on this path. What stops it from simply being switched on is that a disrupted worker lands in `PodFailed` with an *empty* reason — the same shape a genuinely failing job produces — so recovery has to key on the pod's `deletionTimestamp` rather than on its phase. That discriminator is still being measured; [q459-drained-worker-recovery.md](../plan/q459-drained-worker-recovery.md) carries it. Operator-facing guidance is in [troubleshooting.md](../operations/troubleshooting.md#draining-or-preempting-a-worker-does-not-auto-re-run-the-jobs-it-interrupts); the full result is in [the experiment](../plan/eviction-oversubscription-validation.md#result-measured-2026-07-27).

**Q423 measured the same exclusion for `PriorityClass` preemption, 2026-07-29, and it lands in the same place — which is load-bearing for the oversubscription claim.** Preemption is kube-scheduler's, not the kubelet's: the scheduler **deletes** the victim, so a preempted worker never carries `Evicted` and no re-run fires. Measured on a real cluster by the Tier B pair `E2E_AGC_PreemptedWorkerGetsNoAutomaticRerun` and `E2E_AGC_PreemptedRunningPodPhaseFollowsItsExitCode`: the victim went `Running` → `Running` + `deletionTimestamp` → a terminal phase, carrying a `DisruptionTarget` condition with reason `PreemptionByScheduler` throughout; its `safe-to-evict: false` / `do-not-disrupt` annotations deflected nothing, and no `rerun-failed-jobs` call was made in the 45s after removal. Two consequences. First, `priorityTiers` buys the *packing* guarantee (a guaranteed tier really does preempt its way in, so reserved idle headroom is unnecessary) but **not** an automatic-recovery guarantee for the work it displaces — the displaced run needs a manual re-run today. Second, the terminal phase on this path is the interrupted container's own exit status, so it can be `Succeeded` as readily as `Failed`; that is stronger than Q459's "empty reason is ambiguous" and rules the phase out as a discriminator entirely. What preemption *does* offer that a drain does not is an unambiguous marker: `PreemptionByScheduler` is written only by the scheduler, never by a human cancel or a job failing on its own, so this slice is closable without Q459's ambiguity — [Q497](../STATUS.md#Q497) carries it. Full result: [the experiment](../plan/eviction-oversubscription-validation.md#result-measured-2026-07-29-preemption-is-not-eviction).
