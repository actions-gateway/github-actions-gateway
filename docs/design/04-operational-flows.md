...
| The AGC's own reaper deletions | As above, plus the `actions-gateway.com/deletion-reason` stamp | ❌ | The reaper deletes pods the AGC gave up on; recovering them would turn cleanup into a re-run trigger. The stamp, written before every reaper delete, is the exclusion |
| Deleted without ever publishing a terminal phase | Vanishes from its current phase | ❌ | The job never ran to a reportable end, so there is no failed job for `rerun-failed-jobs` to act on (measured: a drained *Pending* worker takes this shape) |
...
Note that the `DisruptionTarget` **condition type** alone is not the discriminator for either deleted-pod row: the eviction API stamps it too, with reason `EvictionByEvictionAPI`, and a bare `kubectl delete pod` stamps nothing. Preemption detection matches the full type/status/reason triple; drain detection keys on the deletion mark at terminal publish. On the scale-set tier, where the scan reads the pod after the fact rather than off the resolving event, the mark is additionally ordered against the container's `finishedAt` — a delete issued *before* the container exited is a disruption, one issued *after* is cleanup of an already-failed pod and must not re-run it.
