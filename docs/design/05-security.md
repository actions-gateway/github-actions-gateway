...
single-container worker pod with the defaults is Guaranteed QoS. On the
`ScaleSet` tier there is no eviction-retry budget to burn — that recovery is
classic-only until [Q417](../STATUS.md#Q417) lands
([§4](04-operational-flows.md#worker-pod-eviction-and-auto-retry)) — so the QoS
default is the whole protection there, not a first line of defence.
