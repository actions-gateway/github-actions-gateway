...
> The deprecated `CiliumFQDN` / `CalicoFQDN` enum values still work (each pins its namesake backend) but the webhook warns and steers to `FQDN` + a backend; being enum members of `v2alpha1` and `v2beta1`, both removed at `v2.0.0`, and absent from the GA `v2` version, they are removed at `v2.0.0` ([why](../operations/v1alpha1-deprecation.md#the-ciliumfqdn--calicofqdn-aliases-ride-the-v200-clock)).
...
| **Cross-Tenant Runner-Pool Takeover via a Shared Agent Identity** | High | Every runner pool derives its agent Secret `agentpool-<stem>-N` and the GitHub runner name `<stem>-N` from one *stem*: a `RunnerGroup`'s CR name, or a `RunnerSet`'s under an `rs-` prefix. The Secret is namespaced; the runner name is unique per GitHub org, enterprise, or repo. Two owners on one stem therefore collide either in a namespace or across namespaces bound to one org, and each register `409`s, resolved by deleting the incumbent's live record: the loser's whole pool never fills and never recovers, so one tenant can take down another's runners by choosing a CR name. Q466's `rs-` prefix is not injective, so a `RunnerGroup` named `rs-<x>` lands on the stem of a `RunnerSet` named `<x>`; Q979 stopped the deregistration but left the pair admissible. Admission now rejects the write on whichever object arrives second, on both boundaries: a `RunnerSet` write directly, and a v1 `ActionsGateway` write for the `RunnerGroup` names its `spec.runnerGroups` would derive (a `RunnerGroup` carries no name of its own, so no v1 `RunnerGroup` validator is needed). Unverifiable reads fail closed and a cross-tenant holder is withheld from the rejection, as for the scale-set guard. There is no reconcile-side report: a pair carried in from an older release surfaces through the AGC's existing `AgentPoolError` Event once a pool is blocked. **Closed (Q1011 admission; Q979 deregistration).** Full detail: [see below](#cross-tenant-runner-pool-takeover-via-a-shared-agent-identity). |
...
### Cross-Tenant Runner-Pool Takeover via a Shared Agent Identity

The third GitHub-side namespace, and the one that costs a tenant its runners outright rather than its jobs.

Every agent pool derives two names per index from one **stem**: the agent Secret `agentpool-<stem>-N`, and the runner name `<stem>-N` it registers with GitHub.
A `RunnerGroup`'s stem is its CR name; a `RunnerSet`'s is that name under an `rs-` prefix, which is what lets a same-named group and set coexist through a migration (Q466).
The two halves have different uniqueness boundaries, and both are real:

| Derived name | Unique within |
|---|---|
| `agentpool-<stem>-N` | the Kubernetes namespace |
| the GitHub runner name `<stem>-N` | the org, enterprise, or repo the gateway's `gitHubURL` names |

Two owners on one stem is not a recoverable state.
Each pool's selector sees only its own Secrets, so each keeps finding its index missing; registering the name anyway takes a `409`, and the `409` recovery deletes whatever record holds it, which is the other tenant's live registration.
Q979 taught the AGC to refuse that deletion when another pool's Secret claims the record, which confines the damage to the second tenant's pool and leaves it permanently unable to fill: `agent identity is owned by another pool`, on every reconcile, until an operator renames a CR.

The stem is reachable two ways, and only the first is what Q466 anticipated:

- **Across the derivations.** The `rs-` discriminator is a prefix rather than an injection, so a `RunnerGroup` named `rs-<x>` derives exactly what a `RunnerSet` named `<x>` derives.
- **Across namespaces, within one GitHub scope.** Two `RunnerSet`s of one name, or two `RunnerGroup`s of one name, under gateways bound to one org collide on the runner name alone.
  Appendix E §E.6 shards one org across namespaces deliberately, so this is the expected topology rather than an adversarial one, and it is the shape most likely to be in a running cluster.

Admission rejects the write on whichever object arrives second (Q1011), on both boundaries and from both derivations.
The landing places differ from the scale-set guard because the objects do:

- A `RunnerSet` write is checked directly, like its scale-set label.
- A **`RunnerGroup` is not tenant-authored**: an `ActionsGateway.spec.runnerGroups` entry carries no name field, and the GMC derives the CR name as `<gateway>-<first runnerLabel>-<hash>`.
  So the reverse direction is checked on the **v1 `ActionsGateway` write**, which is the last moment an operator can still act, and v1 gains no new admission surface.

Both boundary properties of the scale-set guard carry over unchanged: an unverifiable cluster-wide read rejects rather than admitting on faith, and a holder in another namespace is withheld from the rejection and written to the GMC log, since naming it would let anyone able to write in their own namespace enumerate another tenant's objects.

**There is deliberately no reconcile-side report.** A webhook fires on a write, so a pair stored before the guard shipped is never re-validated, exactly as for the scale-set name.
The difference is that this collision already announces itself where it lands: the blocked pool records an `AgentPoolError` Warning Event naming the Secret and its holder, on every reconcile, which is the signal a `ScaleSetNameCollision` condition had to be invented to supply.
Adding a second one would report the same fact twice, and to the gateway rather than to the pool that is actually blocked.
Resolution steps: [Rejected: Agent Identity Already Claimed](../operations/troubleshooting.md#runnerset-or-actionsgateway-rejected-agent-identity-already-claimed), and [Agent Pool Blocked](../operations/troubleshooting.md#agent-pool-blocked-another-tenant-owns-the-agent-secret) for one already stored.

**What stays outside GAG's control.** A runner registered at GitHub under a colliding name by something other than GAG is not in the cluster and is invisible to admission, the same limit the scale-set guard has.
**Closed (Q1011 admission; Q979 deregistration).**
...
On a cluster with a policy-enforcing CNI, the tenant policies this system generates already supply that NetworkPolicy leg, though not in the shape the sentence above implies.
NetworkPolicy is allowlist-only and has no deny primitive, so nothing here names the metadata address.
The address falls inside the link-local block `169.254.0.0/16` that the DNS rule admits for NodeLocal DNSCache (§H.5), and what keeps the metadata server out is that this rule is scoped to port 53, which the metadata service does not serve.
Widening that rule's ports, or adding any egress rule with an empty `to`, hands a worker pod the node's cloud credentials.
Both halves are now tested: `TestBuildNetworkPolicy_DeniesCloudMetadataServer` pins the authored policy on every PR, and `E2E_V2_DirectEgress_MetadataServerBlocked` proves a real CNI honours the port scoping on the Calico lane (Q716).
