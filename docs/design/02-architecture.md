...
* **Pod Provisioner:** Intercepts workflow triggers, decrypts incoming payloads, maps runner labels to target pod configurations, and schedules ephemeral worker pods within the tenant namespace. The provisioner extracts and stores the `run_id` from each acquired job payload alongside the pod reference — this is required by the eviction-retry path in the Job Lock Renewer. When a `RunnerGroup` defines `priorityTiers`, the provisioner queries the active and pending worker pod count for that group (via a label-selector list against the Kubernetes API) before each pod creation and walks the tier list in ascending threshold order, assigning the `priorityClassName` of the first tier whose threshold the current count has not yet reached. If the count equals or exceeds the last tier's threshold the pod is held until the count drops — this is the effective `maxConcurrentJobs` ceiling for that group. When `priorityTiers` is empty, no `priorityClassName` is set and the namespace `ResourceQuota` is the only active ceiling. Pod-count queries are per-RunnerGroup, not namespace-wide, so groups with distinct runner labels are correctly accounted for independently. The pod-count read and pod-create are not atomic — a benign race exists where two concurrent job acquisitions each observe count N and both proceed at the tier boundary, potentially scheduling one extra pod above the threshold. This is acceptable; the namespace `ResourceQuota` remains the hard enforcement layer and the overshoot self-corrects as the next pod creation re-reads the live count.
...
* **Job Lock Renewer:** After a job is acquired, a per-job background goroutine calls `renewjob` on the run service every 60 seconds to keep the job lock alive (GitHub grants a 10-minute window per renewal). The renewer also watches the worker pod for terminal state changes and handles two distinct exit paths:
