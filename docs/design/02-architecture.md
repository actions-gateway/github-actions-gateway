...
* **Pod Provisioner:** Intercepts workflow triggers, decrypts incoming payloads, maps runner labels to target pod configurations, and schedules ephemeral worker pods within the tenant namespace.
  ...
  **The same statistics carry the tier's only demand signal (Q720).** Every other scale-set series begins at assignment, so a set that is advertising no capacity reads idle on all of them while jobs pile up at GitHub; `statistics.totalAvailableJobs` is the count of what GitHub holds queued and has assigned to nobody, and `actions_gateway_scaleset_jobs_available` publishes it.
  ...
  A set advertising `0` gets only the first of those, and that is the state the gauge exists to explain, so it would have held a stale zero exactly when an operator needed the number.
  It therefore takes a reading of its own on the empty-poll path, a `RefreshSession` every five minutes for as long as it advertises nothing (Q960).
  Two things keep that affordable, and [§3.5](03-api-contracts.md#35-github-api-rate-limit-budget) is what sets the interval: only a set advertising zero pays, since one with a free slot reads the same statistics off the work GitHub delivers it, and the pacing runs off the last reading rather than a clock of its own, so a set the assignment check is already refreshing adds nothing on top.
...
* **Job Lock Renewer:** After a job is acquired, a per-job background goroutine calls `renewjob` on the run service every 60 seconds to keep the job lock alive (GitHub grants a 10-minute window per renewal).
  ...
  * **Detection in the reconciler, off its pod watch.** The owning `RunnerSet` reconciler already watches worker pods for phase changes; its handler hands each phase-changing or newly preempted scale-set worker straight to recovery, which judges it (`Failed`/`Evicted`, the preemption condition, or a `Failed` phase carrying an unstamped deletion mark, Q502), claims it by stamping `actions-gateway.com/eviction-handled-at` under an optimistic lock, and calls the same `handleEviction` (Q1029).
    A scan at the top of every reconcile lists the same pods from the cache for whatever no event reached — a pod already terminal when the process started — through the same judge and claim.
    The claim is taken **before** the GitHub call, which makes recovery at-most-once per disrupted pod across both paths, reconciles, restarts, and replicas; the scan runs **before** the reaper, so a terminal pod is never deleted before its identity is read.
...
Enumerating the session half rather than the `WorkerImage*` half is what makes a reason added later fail safe *at the drain*: an unrecognized one counts as the reconciler's, so the clear is dropped and the condition stays stale.
...
The other consumer is the image reading's own deference, and there the same omission is a wipe rather than staleness: a listener reason nobody enumerated reads as the reconciler's, nothing defers, and a rejection GitHub made is overwritten.
So the set is gated rather than argued (Q994): both consumers ask `IsSessionSourcedRunnerVersion`, and `reason-tiers-check` reconciles it against every reason `internal/listener` publishes on this type and refuses any other package that compares a reason against an entry instead of calling it.
