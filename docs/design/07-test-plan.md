...
* **AGC reconciler end-to-end** — Deploy a `RunnerGroup`, verify the AGC starts exactly one listener goroutine at rest. Trigger a simulated job burst and verify additional goroutines spawn up to `.spec.maxListeners`. Verify idle goroutines shut down once the queue drains, leaving exactly one active listener. Update `.spec.maxListeners`, verify the new ceiling takes effect without restarting in-flight goroutines. Delete the resource, verify all goroutines exit and no Secrets or Pods are orphaned.
