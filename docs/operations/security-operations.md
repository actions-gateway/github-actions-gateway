...
- [Tenant egress posture & deliberate widening](#tenant-egress-posture--deliberate-widening)
  ...
  - [Pinning a tenant's proxy pool to its egress IP: spec.scheduling](#pinning-a-tenants-proxy-pool-to-its-egress-ip-specscheduling)
- [Worker egress destinations: the egress allowlist](#worker-egress-destinations-the-egress-allowlist)
  - [Prefer an in-cluster caching mirror first](#prefer-an-in-cluster-caching-mirror-first)
  - [How the allowlist works](#how-the-allowlist-works)
  - [Self-service additions via a watched ConfigMap](#self-service-additions-via-a-watched-configmap)
- [Sharing an egress proxy across namespaces](#sharing-an-egress-proxy-across-namespaces)
  - [Consent lives on the provider](#consent-lives-on-the-provider)
  - [What you must not assume](#what-you-must-not-assume)
  - [Verifying a grant took effect](#verifying-a-grant-took-effect)
  - [Revoking](#revoking)
- [Tightening AGC apiserver egress: the `apiserver-cidrs` allowlist](#tightening-agc-apiserver-egress-the-apiserver-cidrs-allowlist)
  - [The one verification every platform shares](#the-one-verification-every-platform-shares)
  - [Finding the apiserver CIDR, by platform](#finding-the-apiserver-cidr-by-platform)
  - [Why GAG can't discover and tighten this for you (feasibility verdict)](#why-gag-cant-discover-and-tighten-this-for-you-feasibility-verdict)
...
- [Priority classes: the `allowed-priority-classes` allowlist](#priority-classes-the-allowed-priority-classes-allowlist)
  - [Infra pods: the separate allowed-infra-priority-classes allowlist](#infra-pods-the-separate-allowed-infra-priority-classes-allowlist)
  - [Upgrading: previously ungated priorityClassName fields are now gated](#upgrading-previously-ungated-priorityclassname-fields-are-now-gated)
  - [Self-service additions via the PriorityClassAllowlist CR (Q188, Q298)](#self-service-additions-via-the-priorityclassallowlist-cr-q188-q298)
  - [Narrowing the allowlist: drain stored references first](#narrowing-the-allowlist-drain-stored-references-first)
  - [Defense-in-depth: the priorityclass-allowlist-guard policy (Q289)](#defense-in-depth-the-priorityclass-allowlist-guard-policy-q289)
