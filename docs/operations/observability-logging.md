...
| `host` / `port` | The CONNECT destination. `host` is capped at 253 characters (the longest legal DNS name) and gains a `…(truncated)` marker if cut, so a capped value never reads as a real destination |
