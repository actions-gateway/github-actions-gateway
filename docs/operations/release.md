...
What let it through is that the commit and [the plan doc](../plan/archive/release-1.5.md) were both correct: the note was written from a sibling artifact instead of from `provisioner.go`, and a paraphrase can reverse a direction while keeping every noun in place.
...
That is the mistake the scanner was written to stop making: it reads the reason argument's index off the callee's own declaration, because two recorders here are named `Event` and two `recordEvent`, with the reason at a different index in each.
...
- **A `feat`/`fix`/`perf` subject count beside the commit count.** `v1.7.0` and `v1.8.0` opened the section with one ("68 commits, 29 of them carrying a `feat`, `fix` or `perf` subject"), and nothing a reader does with the section touches it.
  ...
  Like the commit count it went stale on every merge, including the pull request carrying its own correction, so gating it would have meant deriving it at publish time for a reader who was not using it.
...
- **The cosign verify binary is checksummed.** GitHub release assets are mutable for an existing tag, so a raw download of the release verifier can't be trusted on its own.
  ...
  All three sites move together on the weekly [updatecli](../development/dependency-updates.md#the-manifests) run rather than by hand (Q927), and the digests it writes come from the release's sigstore-signed `cosign_checksums.txt`, verified against a pinned identity by the cosign the repo already pins.
  Automating the copy without automating the trust would pin whatever the releases CDN served.
