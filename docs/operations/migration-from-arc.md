...
| `containerMode: kubernetes` / `dind` | a template choice + the namespace security profile | **Read this one before migrating.** GAG always runs one worker pod per job, and `containerMode: kubernetes` has no equivalent: it is [permanently declined](../design/appendix-d-alternatives-considered.md#d15-pod-per-step-container-execution-arcs-containermode-kubernetes), because its hook needs a Kubernetes API token inside the pod your workflow's code runs in. Docker-in-Docker is the answer, and it is unprivileged only under Kata, which needs nested-virtualization nodes ([prerequisites](kata-dind-workloads.md#prerequisite--nested-virtualization-nodes)). Without Kata it needs a platform-owned `ClusterRunnerTemplate` plus a privileged-eligible namespace; see [Security profiles](#security-profiles). |
