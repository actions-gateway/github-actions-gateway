...
| `actions_gateway_scaleset_jobs_available` | Gauge | `namespace`, `runner_set` | Jobs GitHub is holding queued for the scale set and has assigned to nobody: the demand signal every other series on this tier is blind to, because they all begin at assignment. Read it against `…_advertised_capacity` to answer *why is this job still queued*: demand with a ceiling to spare means GitHub has not delivered yet, demand against a `0` ceiling means the AGC is withholding and `…_capacity_withheld` says which rung. **It is a last-reading gauge, not a per-poll one.** Statistics ride on queue messages and session responses; an empty poll (HTTP 202) carries none, so the value holds until the next reading rather than tracking continuously. The readings are every session open, every delivered message, and, while jobs are deferred, the once-a-minute assignment check. **A set advertising `0` capacity gets none of those**, so this series sits at whatever it last read (typically `0` from the session open) for as long as the withholding lasts: do not alert on it, and do not read a `0` as *no queued demand* unless `…_advertised_capacity` is above zero (Q720, Q960). Dropped when the `RunnerSet` is deleted. |
