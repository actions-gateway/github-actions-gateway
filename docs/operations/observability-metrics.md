...
| `actions_gateway_job_duration_seconds` | Histogram | `namespace`, `runner_group` | Wall time from `acquirejob` success to worker pod terminal phase. **Classic acquisition only.** The observation sits in `provision()`, which `ProvisionScaleSetWorker` never reaches, so the series is absent on a `ScaleSet` set. Unlike the classic-only counters above there is no scale-set analog to read instead (Q713), so the `actions_gateway:job_duration_seconds:p50` and `:p95` recording rules read blank on the default tier too. |
| `actions_gateway_pod_creation_latency_seconds` | Histogram | `namespace` | Time from worker pod creation to the runner container starting (scheduling + image pull). Key SLO metric — see [Appendix A](../design/appendix-a-capacity-slos.md). **Classic acquisition only**, recorded by the classic pod waiter that the scale-set path does not use, so the series is absent on a `ScaleSet` set and, unlike the counters above, has no scale-set analog (Q713). `ActionsGatewayPodCreationLatencyP99` and `ActionsGatewayPodCreationLatencyP95` therefore never fire on the default tier. A silent latency alert there means an absent series, not a healthy one. |
