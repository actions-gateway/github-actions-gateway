...
| `actions_gateway_eviction_rerun_failures_total` | Counter | `namespace`, `runner_group`, `tier`, `cause`, `reason` | Disruption recoveries whose re-run was **never accepted** by GitHub, so the budget slot is spent but the job was not re-run and needs a manual `gh run rerun` (Q503). `reason="run_never_concluded"`: GitHub was still answering `403 This workflow is already running` when the 15-minute re-run window closed — the original run outlived the job lock's ~10-minute TTL bound, which is itself worth investigating. `reason="api_error"`: a terminal API failure (a non-403 error, or a 403 that is a permissions problem rather than the still-running refusal). `reason="conclusion_unknown"`: the window closed with the deletion arm's cancel check (Q811, next row) still unable to reach the run for fifteen minutes (the request never completed, or GitHub answered 5xx), so no verdict was taken and no re-run was fired, because a run whose conclusion cannot be read is not assumed to be uncancelled. A 4xx, or a 2xx carrying something that is not a run, is `api_error` instead: neither becomes a verdict inside the window. Each occurrence also emits an `EvictionRerunFailed` Warning Event naming the run. **Expected to be zero**; see the [runbook](troubleshooting.md#evicted-worker-pods-exhausting-retry-budget). |
| `actions_gateway_eviction_rerun_withheld_total` | Counter | `namespace`, `runner_group`, `tier`, `cause`, `reason` | Disruption recoveries that deliberately made **no** re-run call, because making one would undo a decision GitHub already recorded (Q811). `reason="run_cancelled"` with `cause="deletion"` is the only shape today: the run concluded `cancelled`, and `rerun-failed-jobs` accepts a cancelled run (measured live 2026-08-05) where a `success` conclusion refuses it. Without the check, the [cancel runbook](troubleshooting.md#cancelling-a-run-does-not-stop-its-worker-pod)'s own remedy for a worker that will not stop (delete its pod, which supplies the deletion mark the recovery keys on) re-queued the job the operator had just stopped. Each occurrence also emits a **Normal** `EvictionRerunWithheld` Event naming the run. The retry slot is spent, since it is reserved at detection, minutes before GitHub concludes the run, so read this alongside `eviction_retries_total`, whose increment it explains. **Not** a failure and **not** something to alert on: no manual re-run is wanted. |
...
| `actions_gateway_eviction_recovery_evidence_lost_total` | Counter | `namespace`, `runner_group`, `cause` | Disrupted **scale-set** worker pods deleted before the recovery could be claimed, so no automatic re-run was attempted and none can be attempted later: on that tier the pod is the disruption's only record, and it is gone (Q809). Each occurrence also emits an `EvictionRecoveryEvidenceLost` Warning Event on the owning `RunnerSet`. `cause="preemption"` and `cause="deletion"` are the exposed arms, since both act on a pod that is already terminating; recovery reads the pod off the informer (the watch event, or the per-reconcile scan's cached List) and claims it through the live API, so a pod removed in between yields a claim that finds nothing. The two readers are routinely concurrent claimants, and the loser of a claim conflict re-reads the pod to confirm the winner's stamp; a pod removed between the conflict and that re-read is reported here although the winner recovered its run, so the counter can read high by one per such race. **Expected to be zero**, and distinct from `…_identity_unknown_total`, which means the pod was there and carried no run identity. See the [runbook](troubleshooting.md#a-preempted-workers-job-is-not-re-run). |
...
| `actions_gateway_worker_scaleup_throttled_total` | Counter | `namespace`, `runner_group` | Intake withheld by the opt-in per-owner scale-up rate limit (`spec.scaleUp`): the token bucket was empty, so the AGC declined to take on another job rather than create its pod (Q223, Q717). **Zero unless the owner sets `scaleUp`**, which is default-off. A sustained rate means the ramp is actively smoothing a cold-start burst on a shared egress path (NAT/firewall/VPN); that is the knob doing its job, not an error. It means different things per tier and both are worth watching: on the classic tier it counts a job the rate rung refused to claim, the ordinary shape of a ramp; on the scale-set tier the rung is expressed as withheld capacity instead, so this counter ticks only when a pod creation *still* had to wait, which means the advertisement was stale. Sustained scale-set throttling is therefore a signal about the advertisement rather than about the ramp. If the classic rate is persistently high, raise `maxPerSecond`/`burst`, or confirm a rate limit is the right tool for the burst (see [tenant-onboarding: worker scale-up rate limit](tenant-onboarding.md#step-2-create-the-actionsgateway-resource)). |
...
| `actions_gateway_runnerset_workers_not_starting` | Gauge | `namespace`, `runner_set` | `1` when `WorkersNotStarting=True` on a v2 `RunnerSet` (Q906): one or more worker pods were placed on a node and the kubelet could not start them, almost always an image that will not pull. The sibling of `actions_gateway_runnerset_workers_unschedulable` and disjoint from it: that one is the scheduler saying no node can host the pod, this one is a pod already hosted whose container never ran, so the two can never both read `1` for the same pod. **Emitted for every set**, gated or not: it reports and decides nothing, which is what makes it the signal to alert on for a `workerImage` that will not pull, on any set rather than only one that opted into a capacity gate. A gated set ALSO reads `1` on `actions_gateway_runnerset_worker_capacity_declined{reason="PodsNotStarting"}`; that is the intake decision, this is the observation. Page if sustained; the pods and the kubelet's own text are in the condition message. |
| `actions_gateway_runnerset_worker_capacity_declined` | Gauge | `namespace`, `runner_set`, `reason` | `1` when `WorkerCapacityDeclined=True` on a v2 `RunnerSet` (Q405, Q406; gauged in Q643): the opt-in capacity gate is refusing job intake because the cluster cannot place another worker pod of this set's shape. **Emitted only for a set that set `spec.capacityGate.mode`** — a set with no gate carries no condition, and an absent series is what says so; a `0` would read as "gate evaluated, capacity available". `reason` is the condition's current reason and is the label to alert and group on: `PodsUnschedulable` is the scheduler's verdict (a fixed-size cluster, `clusterCapacity.nodeAutoscaling: Absent`), `ScaleUpDeclined` is the cluster autoscaler's own declination (the default, `Present`), `PodsNotStarting` is the kubelet's on a worker pod that bound and then could not start (Q714, read on either cluster fact since it is not a placement question), and `AwaitingProbe` is the **latched** state (Q512) — the declined pods were reaped, so intake is throttled to one probe job per `pendingPodDeadline` window until a worker pod starts. `CapacityAvailable` and `GateModeUnsupported` are the two `False` reasons and read `0`. Exactly one series exists per gated set: a reason change replaces it rather than adding one, so `max by (reason) (…) == 1` is safe. **The `AwaitingProbe` row is the one that needs the label.** It outlives the stuck pod that produced it, so `actions_gateway_runnerset_workers_unschedulable` has already fallen back to `0` while this reads `1` — an operator watching only the scheduler signal sees a recovered set whose intake is still throttled. |
...
| `actions_gateway_egress_audit_attributable` | Gauge | `namespace`, `name` | `1` when a v2 `ActionsGateway`'s `EgressAuditAttributable=True` (Q1062, §G.3): both halves of the [egress-attribution pair](observability-logging.md#attributing-a-record-to-a-tenant-and-a-job) are on (the gateway's `spec.auditLogging` is `WorkerAddresses` and the `EgressProxy` its `defaultProxyRef` resolves to logs `ConnectionsWithSource`), so a proxy audit record joins to a tenant and a job. `0` when either half is off, and the condition's reason names which: `WorkerAuditDisabled`, `ProxySourceAuditDisabled`, `EgressAuditDisabled` (neither), or `DirectEgress` (no proxy, so there is no per-connection record at all). Advisory: both halves are opt-in and `Off` is the supported default, so a `0` is not a fault. It reports the **declared** pair, so it says nothing about whether a log pipeline runs the join, and it reads only `defaultProxyRef` (Q1069). v2 only, emitted only on a v2 install. |
| `actions_gateway_build_info` | Gauge | `component`, `version` | Constant `1` per running control-plane binary, following the Prometheus `*_build_info` convention (Q318). Emitted by the GMC, AGC, and proxy — `component` is `gmc`/`agc`/`proxy` and `version` is the build tag stamped into the binary (`dev` for un-stamped local builds). Not load-bearing for alerting; join it into other series to correlate the running version during an incident (worker pods carry `app.kubernetes.io/version`, but the control plane otherwise does not expose its version in metrics). The tenant and platform [dashboards](observability-dashboards.md) surface it, which is how an operator tells whether a tenant's `job_duration_seconds` is the pre- or post-v1.5.0 span. |
...
> `WorkersNotStarting` (Q906) joins them as a v2-only fourth: the kubelet's startup verdict, which the v1 `RunnerGroup` computes but does not publish.
> Each has its own gauge — the `actions_gateway_runnerset_*` set above (Q319, Q906) — so a `RunnerSet` is alertable without scraping CRD conditions through kube-state-metrics.
>
...
> It is exported as the `actions_gateway_runnersets_degraded` gauge (Q321), alongside `actions_gateway_agc_available`, `actions_gateway_egress_unattributed`, `actions_gateway_agc_autoscaling_unavailable`, `actions_gateway_scale_set_name_collision`, and `actions_gateway_egress_audit_attributable` for the gateway's `AGCAvailable`, `EgressUnattributed`, `AGCAutoscalingUnavailable`, `ScaleSetNameCollision`, and `EgressAuditAttributable` conditions — the v2 twins of the v1 `ActionsGateway` condition gauges.
...
| `actions_gateway_scaleset_capacity_withheld` | Gauge | `namespace`, `runner_set`, `reason` | Slots the named rung removed from the declared ceiling on that same poll — `advertised_capacity` plus the sum of these equals the ceiling. `reason="quota"` is namespace-`ResourceQuota` headroom; `reason="capacity"` is the opt-in capacity gate, which bounds the total at the set's own in-flight workers while the cluster cannot place another (Q405) — plus one probe slot per `pendingPodDeadline` window while the gate is latched (`AwaitingProbe`, Q512), so under a sustained decline expect this series to *hold* near the ceiling across reap cycles rather than sawtooth back to `0`. `reason="scaleup"` is the opt-in scale-up rate limit (Q717), which bounds the total at the set's in-flight workers plus its free tokens, so on a set ramping from idle it starts near the ceiling and falls to `0` as the set climbs, the opposite shape to a sustained decline. The same accounting is on the set's own `status.withheldCapacity`, alongside `status.advertisedCapacity`, for a tenant without metrics access (Q721). Every evaluated rung publishes a value each poll, **including an explicit `0`**, so a series never sits frozen at its last non-zero reading — the capacity rung publishes its zero even with the gate `Off`, because the gate is per-set spec rather than a rung the AGC skips. Nothing is published for a rung an operator has turned off AGC-wide (`AGC_QUOTA_ADMISSION=false`). |
| `actions_gateway_scaleset_jobs_available` | Gauge | `namespace`, `runner_set` | Jobs GitHub is holding queued for the scale set and has assigned to nobody: the demand signal every other series on this tier is blind to, because they all begin at assignment. Read it against `…_advertised_capacity` to answer *why is this job still queued*: demand with a ceiling to spare means GitHub has not delivered yet, demand against a `0` ceiling means the AGC is withholding and `…_capacity_withheld` says which rung. **It is a last-reading gauge, not a per-poll one.** Statistics ride on queue messages and session responses; an empty poll (HTTP 202) carries none, so the value holds until the next reading rather than tracking continuously. The readings are every session open, every delivered message, the once-a-minute assignment check while jobs are deferred, and a five-minute refresh on the empty-poll path for a set advertising `0` capacity, which is delivered nothing and would otherwise take no reading at all (Q960). **So the value can lag GitHub by up to five minutes** while a set is withholding, which is the state you are most likely to be reading it in: it is alertable, but give any rule a window several times that interval rather than firing on a single scrape (Q720). Dropped when the `RunnerSet` is deleted. |
...
| `actions_gateway_scaleset_jobs_available` | Scale-set only | GitHub reports it in the runner-scale-set session statistics, a payload the classic many-acquirers protocol has no equivalent of; a Classic set learns of a job only when one is delivered to it. |
...
| `actions_gateway_eviction_rerun_withheld_total` | Both | As above: the cancel check sits in that shared path, and both tiers detect the graceful deletion that arms it (Q811). |
...
| `actions_gateway_worker_scaleup_throttled_total` | Both | The opt-in `spec.scaleUp` limiter is a rung of the admission ladder on both tiers, but each tier states the rung in its own form: classic as a refused claim (`jobs_admission_rejected_total{reason="scaleup"}`), scale-set as withheld capacity (`scaleset_capacity_withheld{reason="scaleup"}`). This counter is the pod-creation backstop behind both, so it is near-zero on the scale-set tier by design. |
...
| `actions_gateway_runnerset_workers_not_starting` | Both | As above: the kubelet's startup verdict, read from the same shared capacity call on both tiers. |
...
| `PodsNotStarting` | Both | The kubelet's verdict on a worker pod that bound and could not start, from the same shared capacity call. |
...
| `WorkersStarting` | Both | The cleared form of `PodsNotStarting`, evaluated on both from the same shared capacity call. |
...
| `WorkersNotStarting` | Both | The kubelet's startup verdict, recorded from the same shared capacity call and on every set, gated or not (Q906). |
