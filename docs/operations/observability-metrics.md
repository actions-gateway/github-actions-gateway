...
| `actions_gateway_scaleset_advertised_capacity` | Gauge | `namespace`, `runner_set` | The `X-ScaleSetMaxCapacity` most recently advertised for the set: the total jobs GitHub may keep assigned to it at once. This is the scale-set tier's whole admission decision — the minimum of the declared worker ceiling and live namespace-`ResourceQuota` headroom (Q443). A value below the set's `maxWorkers` means a rung is binding; **`0` means GitHub will assign nothing at all** until it recovers. Dropped when the `RunnerSet` is deleted, so a stale series does not outlive the set. |
| `actions_gateway_scaleset_capacity_withheld` | Gauge | `namespace`, `runner_set`, `reason` | Slots the named rung removed from the declared ceiling on that same poll — `advertised_capacity` plus the sum of these equals the ceiling. `reason="quota"` is namespace-`ResourceQuota` headroom. Every evaluated rung publishes a value each poll, **including an explicit `0`**, so a series never sits frozen at its last non-zero reading. Nothing is published for a rung an operator has turned off (`AGC_QUOTA_ADMISSION=false`). |
