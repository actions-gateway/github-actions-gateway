...
| `actions_gateway_egress_audit_unattributed` | Gauge | `namespace`, `name` | `1` when a v2 `ActionsGateway`'s `EgressAuditUnattributed=True` (Q1062, §G.3): either half of the [egress-attribution pair](observability-logging.md#attributing-a-record-to-a-tenant-and-a-job) is off, so no egress audit record joins to a tenant and a job. The condition's reason names which half: `WorkerAuditDisabled` (the gateway's `spec.auditLogging` is not `WorkerAddresses`), `ProxySourceAuditDisabled` (the `EgressProxy` its `defaultProxyRef` resolves to does not log `ConnectionsWithSource`), `EgressAuditDisabled` (neither), or `DirectEgress` (no `defaultProxyRef`, so the AGC's own egress leaves from no pool). `0` is `EgressAuditJoined`, both halves on. Advisory, and **`1` is the expected state on a gateway that never opted in**: both halves are opt-in and `Off` is the supported default, so this flags a fleet's un-attributed tenants rather than a fault, the same way `egress_unattributed` flags its direct-egress ones. The `0` says the pair is **configured**: it does not establish that the CNI leaves the worker's source address intact or that a log pipeline runs the join, and it reads only `defaultProxyRef` (Q1069). v2 only, emitted only on a v2 install. |
