...
| `actions_gateway_egress_audit_unattributed` | Gauge | `namespace`, `name` | `1` when a v2 `ActionsGateway`'s `EgressAuditUnattributed=True` (Q1062, §G.3): either half of the [egress-attribution pair](observability-logging.md#attributing-a-record-to-a-tenant-and-a-job) is off, so no egress audit record joins to a tenant and a job. The reason names the missing half (`WorkerAuditDisabled`, `ProxySourceAuditDisabled`, `EgressAuditDisabled` for neither, or `DirectEgress` for no `defaultProxyRef`); [tenant onboarding](tenant-onboarding.md#per-pool-egress-audit-record) says what to set for each. Advisory, and **`1` is the expected state on a gateway that never opted in**: both halves default `Off`, so this flags a fleet's un-attributed tenants rather than a fault, the way `egress_unattributed` flags its direct-egress ones. The `0` says the pair is **configured**, not that the CNI leaves the worker's source address intact or that a pipeline runs the join, and it reads only `defaultProxyRef` (Q1069). v2 only, emitted only on a v2 install. |
