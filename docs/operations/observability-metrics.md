...
| `actions_gateway_scaleset_jobs_deferred` | Gauge | `namespace`, `runner_set` | Assigned jobs the listener is holding for a later re-offer because their runner name will not register — a `generate-jitconfig` 409 that neither deleting the stale record nor a fresh suffixed name could clear (Q551). Each one is a workflow run queued at GitHub with no worker running it, so **any non-zero value is worth alerting on**; it is the metric twin of the `RunnerSet`'s advisory `JobProvisionStalled` condition, whose message names the stuck job ids. It clears when the job provisions on a re-offer or GitHub reports it complete. Fix per [Scale-Set Job Stranded by a Stale Runner Record](troubleshooting.md#scale-set-job-stranded-by-a-stale-runner-record-runner-name-409). Dropped when the `RunnerSet` is deleted. |
