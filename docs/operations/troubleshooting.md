...
**Symptoms.** A `Warning` Event with reason `WorkerPodStuckPending` appears on the RunnerGroup (`kubectl describe runnergroup -n <namespace>`), `actions_gateway_worker_pods_reaped_total{reason="pending_deadline"}` increments, and the job the pod was created for does not run: its workflow run shows `in_progress` for up to ~15 minutes and then concludes **`cancelled`**. The worker pod itself is gone.
...
**The session deliberately reports nothing for the assignment (Q628 → Q676).** No runner ever registered, so nothing inside the pod can report the job, and the AGC does not either. It logs `worker pod was removed before it ran; reporting the job as abandoned so the assignment is left to GitHub's lock lapse`, and GitHub concludes the run *and* job as **`cancelled`** at its ~15-minute unstarted-job timeout. Measured live (the Q645/Q676 probe runs, 2026-08-04): completing the winner's own never-run delivery instead concludes the run as **`success`** one second later, a job that never executed reporting green, for `result=abandoned` and `canceled` alike, while `failed` is refused with a 401, and the green run cannot be retried with `rerun-failed-jobs` (403). The ~15-minute `cancelled` ending is the only honest one available, and `AGC_FANOUT_COMPLETION` does not affect this path (it gates only the Q260 sibling fan-out). Re-run the cancelled run manually if the underlying cause (image pull, scheduling, node capacity) has been fixed; a faster automatic ending is tracked as Q683.
