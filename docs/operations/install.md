...
> **Current release — `v1.8.0`.** The chart is published, cosign-signed, and installable straight from the GHCR OCI registry.
> Pin `--version 1.8.0` (the chart version is the release tag without the leading `v`).
> Newer patch releases publish as `1.8.z`; pin the version you have verified.
...
Copy the four image digests from the [release notes](https://github.com/actions-gateway/github-actions-gateway/releases/tag/v1.8.0) (the chart ships **no** baked-in digests — empty digests are the fail-closed secure default, so an unconfigured render is rejected).
