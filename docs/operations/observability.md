...
| `actions_gateway_abandoned_delivery_completions_total` | Counter | `namespace`, `runner_group`, `outcome` | A deduplicated duplicate delivery (a Q260 loser) releasing its acquired-but-unrun job assignment via `completejob`, so GitHub does not cancel the whole job at its ~15-minute unstarted-job timeout even after the winning sibling completed it (Q260 follow-up). `outcome="completed"` released the assignment (or found it already gone); `outcome="error"` failed and the job may still be cancelled. **Only emitted when the guarded behavior is enabled** (`AGC_COMPLETE_ABANDONED_DELIVERIES=true`); flat/absent by default. A rising `error` rate warrants investigating the run service's `completejob` responses. |
