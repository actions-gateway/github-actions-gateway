...
| `actions_gateway_jobs_duplicate_delivery_total` | Counter | `namespace`, `runner_group` | Duplicate job deliveries deduplicated (Q260): the broker fanned the same job out to several sibling sessions and this one skipped `acquirejob` because a sibling in the same AGC was already provisioning it. A steady low rate during bursts is normal and benign — it means the dedup gate is protecting runner slots. A sudden spike proportional to a stalled matrix indicates heavy fan-out; correlate with `jobs_acquired_total` (which should keep climbing) to confirm work is still being provisioned. |
