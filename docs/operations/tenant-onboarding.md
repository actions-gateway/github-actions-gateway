...
| `RunnerVersionTooOld=Unknown`, reason `WorkerImageVersionUnknown` | The registry read of the image is pending or failed and the reference names no runner version to fall back on, or the image is not `actions/runner`-derived | Read the message: an authentication failure (`requires a login`, `token exchange … HTTP 401`, `unauthorized after authenticating`) wants an `imagePullSecret` on the pod template, a dial error means the AGC's egress policy does not reach the registry; otherwise tag the image with the runner version it ships, or read the worker pod's `runner version detected` log line |
...
**Checking it took effect.** `kubectl describe actionsgateway <gateway>` shows an `EgressAuditAttributable` condition: `True`/`EgressAuditJoined` when both halves are on, and a `False` whose reason names the half still off: `ProxySourceAuditDisabled` (the pool), `WorkerAuditDisabled` (the gateway), `EgressAuditDisabled` (neither), or `DirectEgress` (no `defaultProxyRef`, so there is no per-connection record to attribute).
Fleet-wide the same fact is the `actions_gateway_egress_audit_attributable` gauge (Q1062).
