...
- **The `priorityclass-allowlist-guard` policy is retained across uninstall, and `admissionPolicy.enabled=false` only removes its binding.** The `ValidatingAdmissionPolicy` carries `helm.sh/resource-policy: keep`, because deleting a policy that declares `paramKind` permanently destroys the apiserver's parameter informer for that kind — a later reinstall would then deny **every** `runnergroups`/`runnersets`/`runnertemplates` write cluster-wide until kube-apiserver restarts. Turning the guard off still works as documented: the binding is what enforces, and it *is* removed, leaving the policy object inert. Re-enabling recreates the binding. Full mechanism, cleanup command, and recovery: [security-operations.md § The policy object survives `helm uninstall`](security-operations.md#the-policy-object-survives-helm-uninstall--on-purpose).
