...
#
#   1. wormhole-animation.webp  — a light (~320 KB), opaque, looping WebP that
#      IS committed and used in the README footer and the docs 404 page. Opaque
#      (not transparent) because the plume/glow is white and both surfaces
#      default to light mode — it needs to carry its own dark backdrop.
#
...
#      GIF/WebP would be static). NOT committed; rendered on demand.
#
...
# render_pngs WIDTH DESTDIR -> SVG frames + their PNG rasters (native at WIDTH).
render_pngs() {
  local w="$1" dest="$2" h
...
main() {
  ...
  # Committed asset: light opaque WebP, native 480 px, all 64 frames (smooth).
  echo "rendering light WebP -> ${WEBP}"
  render_pngs 480 "${work}/w480"
  magick -delay 4 -loop 0 "${work}/w480"/f0*.png \
    -define webp:lossless=false -quality 72 "${WEBP}"
