...
<div class="gag-vs-hero__proof">
  ...
  <div class="gag-vs-row gag-vs-row--gag"><span class="gag-vs-row__tag">GAG</span><span class="gag-vs-row__text">the job is cancelled when its lock lapses (~10 min at worst) and re-queued automatically: no manual rerun, and it runs as soon as capacity frees up <em>— eviction recovery runs on the classic acquisition tier today; the <a href="../roadmap/#in-progress--near-term">port to scale-set</a> is in progress</em></span></div>
</div>
</div>
...
<div class="gag-stats" markdown="0">
  ...
  <div class="gag-stat">
    ...
    <span class="gag-stat__label"><strong class="gag-stat__lead">Handling for quota-blocked and evicted jobs</strong> — a job the quota has no room for is never claimed, so it stays queued for a sibling with capacity; anything that slips through is lock-cancelled and re-queued, no manual rerun. Eviction recovery is <a href="../roadmap/#in-progress--near-term">classic-acquisition-tier only so far</a></span>
  </div>
  <div class="gag-stat">
...
| Safe under a per-tenant `ResourceQuota` | :material-close-circle:{ .gag-no } quota-blocked jobs stall; manual cleanup + rerun | :material-check-circle:{ .gag-yes } [won't claim a job it can't place](design/04-operational-flows.md)<br><span class="gag-cont">live quota headroom checked *before* the claim, so the job stays queued for a sibling; if headroom is lost after the claim, auto lock-cancel + re-queue</span> |
