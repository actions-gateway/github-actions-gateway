...
| Per-tenant dedicated egress IPs | :material-close-circle:{ .gag-no } per-scale-set proxy config only<br><span class="gag-cont">a `proxy` block points controller, listener, and runner pods at a proxy you already run. ARC provisions no pool, manages no lifecycle, and gives no dedicated per-tenant egress addresses</span> | :material-check-circle:{ .gag-yes } [per-tenant proxy pool, plus the reference architecture under it](design/network-architecture.md#per-tenant-egress-ip-the-source-ip-mechanism)<br><span class="gag-cont">the pool is the per-tenant choke point and `spec.scheduling` pins it; a *distinct, stable* source IP at GitHub then comes from Cilium Egress Gateway or per-tenant cloud NAT beneath it, both specified. Live-validated on GKE Dataplane V2 (2026-07-13): a GMC-provisioned pool held one distinct, stable per-tenant Cloud NAT IP. <span class="gag-v2-badge">v2</span> proxy optional</span> |
