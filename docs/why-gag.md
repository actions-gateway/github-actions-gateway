...
| Right-size runner resources from measured usage | :material-close-circle:{ .gag-no } no feedback loop — runner `resources` stay a guess | :material-check-circle:{ .gag-yes } <span class="gag-v2-badge">v2</span> built-in [per-job CPU/memory peak metrics per runner set](operations/worker-rightsizing.md)<br><span class="gag-cont">with a derivation recipe for `requests`/`limits`</span> |
