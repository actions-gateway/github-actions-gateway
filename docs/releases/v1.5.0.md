...
**Which tier emits which series is now written down.** The [acquisition-tier ledger](https://actions-gateway.com/1.5.0/operations/observability-metrics/#acquisition-tier-reach) covers all 54 `actions_gateway_*` series, and `make metric-tiers-check` holds it to the source (#1494).
...
That verdict replaces the table below.
The run recorded here is rc.1's.
