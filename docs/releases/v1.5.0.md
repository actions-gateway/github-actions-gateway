...
It also closes the last capability `v1alpha1` allowed that `v2beta1` refused, which is why the Deprecations note below says a `v2alpha1` `Classic` set has no remaining reason to stay: every other v1-to-v2 gap had already closed by `v1.4.0`.
...
**[The default acquisition tier no longer lacks anything the classic one has](https://actions-gateway.com/1.5.0/features/).** `ScaleSet` is the default tier and the only one `v2beta1` exposes, so a capability that reached only the classic tier was one most tenants could not use.
...
How far that is checked is worth saying plainly, because this release is the reason to say it rather than claim parity flat.
The metric surface below is derived from the source and gated; the capability half is still a walk someone does by hand.
Restart-safe recovery was classic-only, carried no badge, and left the parity inventory reading clean for as long as it was open (#1487).
