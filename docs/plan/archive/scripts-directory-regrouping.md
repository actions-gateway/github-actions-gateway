...
**Status:** ✅ Complete (2026-08-01, Q571). All 99 top-level files moved into ten
groups, 762 references rewritten across 190 files, every filter that named a
script converted to a prefix glob, and the interim `nondocs` filter deleted. What
the implementation changed about the plan is in [What shipped](#what-shipped).
...
- PRs #1066 and #1071 changed a MkDocs hook, its unit test, and a doc page.
  ...
  `scripts/*` catching what was then `scripts/source-links-hook-test.sh` (now
  [`scripts/docs/source-links-hook-test.sh`](../../../scripts/docs/source-links-hook-test.sh),
  where it cannot match).
...
All five criteria met. Three things the plan got wrong, and one it did not
anticipate.

### The group table needed three more directories

The plan's seven groups left four gates with nowhere to put their scripts. The
shipped set is ten:
...
`ci/` also came out narrower than the plan implied: it holds the meta-gates and
`run-parallel.sh`, not the shared CI plumbing, which is the whole point of
`fetch/` existing separately. `ci/**` is in no heavy tier's filter; `fetch/**` is
in all of them.

### `autoscaler-drift.yml` was not in the table either

`autoscaler-cluster.sh` and `karpenter-cluster.sh` are live-cluster bring-up, so
they went to `e2e/`. That makes `scripts/e2e/**` over-trigger the kindnet e2e
lane on a change to either — accepted deliberately: both files are touched only
by the weekly updatecli pin bump, and over-triggering is the safe direction.

### Filters got *wider*, not narrower, wherever a group maps to a gate

The plan's stated risk was narrowing. In practice the enumerated filters were the
ones at risk, and collapsing them to a prefix glob widened them:
`unit-test.yml:code` went from five named scripts to `scripts/go/**`,
`security-scan.yml` from one to `scripts/security/**` + `scripts/fetch/**`,
`manifest-validate.yml` from four to `scripts/manifest/**` + `scripts/fetch/**`.
Only `e2e-test.yml` narrowed, and only there does criterion 3's new assertion
apply. `autoscaler-drift.yml` gained `scripts/fetch/**` outright — it runs
`download-verified.sh` and had never listed it.

### The move breaks two things a rename normally does not

- **Nine scripts computed `REPO_ROOT` as `dirname "$BASH_SOURCE"/..`**, which is
  now one level short. Shell resolves that at run time, so nothing failed until
  the script ran — exactly the residual risk the plan named. Grep for the form
  before moving a script, not after.
- **`git-merge-status.sh` resolved its awk rules through `$SCRIPT_DIR/lib/`**,
  and `make merge-driver` writes the driver's path into per-clone git config. A
  clone that installed the driver before this change has a dead path in its
  config and must re-run `make merge-driver`; `scripts/README.md` says so.

### The new assertion, verified by breaking it

`check-path-filters.sh` assertion 4 pairs `e2e-test.yml:e2e` with
`e2e-calico.yml:calico` and compares their `scripts/` patterns as a set. Deleting
`scripts/fetch/**` from the Calico lane and re-running the gate reproduces the
exact historical defect and fails with a diff of the two sets; restoring it
returns green. Four fixture cases cover subset, superset, order-insensitivity and
the non-`scripts/` patterns the two lanes are meant to differ on.
