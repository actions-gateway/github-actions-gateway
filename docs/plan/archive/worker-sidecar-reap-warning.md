...
> and `maxWorkers` saturated — the [Q247](gke-dogfood-turnup-findings.md) stranding symptom,
...
- **Un-clean-session GC backstop** — a pod left behind by a *crash* or a
  *superseded/orphaned* job (not a misconfig) strands regardless of sidecar
  shape. That belongs to **[Q247](gke-dogfood-turnup-findings.md)**; the fix there is light
