...
- **Does GitHub redeliver a `JobCompleted` after a multi-hour gap with no session?** cluster-only / live only.
  ...
  Q438's research confirmed this is genuinely **undocumented** — the published contract covers only within-session redelivery of unacknowledged messages, not retention across a session gap — so it needs a live measurement rather than more reading. **Answered by [Q468](q468-jobcompleted-retention.md) on 2026-07-29: GitHub does redeliver**, observed at a 13 h 3 m gap with no session in existence — past the 12 h `maxWorkerLifetime` beyond which no unstamped `Running` worker survives for redelivery to help, so the permissive assumption holds over the range that matters and the replay path is a genuine recovery path.
