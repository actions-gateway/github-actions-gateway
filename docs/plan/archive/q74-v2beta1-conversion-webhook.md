...
- **RunnerSet protocol fields need an annotation round-trip** (the one non-identity case). `ConvertTo` (spoke→hub) stashes `acquisitionProtocol` (and `maxListeners`) in an annotation on the `v2beta1` object; `ConvertFrom` (hub→spoke) restores them so a coexistence-era `v2alpha1` set (Classic *or* ScaleSet) round-trips losslessly and is never silently re-protocol'd.
