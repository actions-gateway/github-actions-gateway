...
1. **No bundled runner image after all.** The spike's item "a GAG e2e runner image bundling `dockerd`, `kind`, the Go/test toolchain and the repo checkout" predated the Q231 sidecar split.
   ...
   The six Kata setup steps live in the sidecar's entrypoint (`args:` block of [`templates/kata-dind/template.yaml`](../../../deploy/templates/kata-dind/template.yaml), adapted from the spike's [`runner-pod.yaml`](../../../deploy/kata-ci/runner-pod.yaml)), with two sidecar-specific deltas: `dockerd` listens on tcp for the runner container, and the per-worker block device is a **generic ephemeral volume** (PVC created and deleted with the pod — no orphaned Persistent Disks).
