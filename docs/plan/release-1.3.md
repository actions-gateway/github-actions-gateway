...
which [Investigation G](q583-restart-replay.md) settles in a single probe run
against the live API. Neither needs a dogfood cluster, so nothing here blocks on
rc.4 — and Q583 still rides rather than gating, because it is a restart-time
burst rather than a defect an ordinary tenant meets in steady state.
