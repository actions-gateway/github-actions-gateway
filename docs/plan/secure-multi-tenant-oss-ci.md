...
| Cross-job cache | none | **Absent by design today.** Workers are storage-less, so `actions/cache` and Docker layer cache have no home. Q215 is blocked on precisely this review: a shared cache between tenants is an obvious exfiltration path |
