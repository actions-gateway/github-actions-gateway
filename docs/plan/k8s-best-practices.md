...
| # | Sev | Finding | Location | Fix |
|---|---|---|---|---|
| B1 | ✅ | ~~Manager ClusterRole grants `rbac.authorization.k8s.io: escalate` — lets a compromised GMC create Roles at any privilege level.~~ Fixed 2026-06-01 by removing per-tenant Role creation entirely. The AGC permission set ships as a singleton `agc-tenant-role` ClusterRole ([`config/rbac/agc_tenant_role.yaml`](../../cmd/gmc/config/rbac/agc_tenant_role.yaml)); per-tenant RoleBindings reference it. The GMC ClusterRole now holds `rolebindings: {create,delete,get,list,patch,update,watch}` (no `bind`/`escalate`) plus `clusterroles: bind` with `resourceNames: [agc-tenant-role]` — the canonical RBAC-native pattern for narrow controllers, and simpler than the originally-proposed `ValidatingAdmissionPolicy`. A compromised GMC cannot create arbitrary Roles, cannot escalate, and cannot bind its SA into any ClusterRole other than `agc-tenant-role`. `roles: {get,delete}` retained for legacy GC of pre-fix per-tenant Roles. | [actionsgateway_controller.go:10](../../cmd/gmc/internal/controller/actionsgateway_controller.go), [builder.go (`buildAGCRoleBinding`)](../../cmd/gmc/internal/controller/builder.go), [agc_tenant_role.yaml](../../cmd/gmc/config/rbac/agc_tenant_role.yaml) | — |
