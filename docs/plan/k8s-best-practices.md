...
| # | Sev | Finding | Location | Fix |
|---|---|---|---|---|
| B1 | 🔴 | Manager ClusterRole grants `rbac.authorization.k8s.io: escalate` — lets a compromised GMC create Roles at any privilege level. **Audit assumption invalidated 2026-06-01:** the original fix proposed dropping `escalate` on the premise that GMC's grants are already a superset of the AGC Role it creates. They are not — the AGC Role grants `pods:{get,list,watch,create,delete}`, `pods/status:get`, `secrets:delete`, `runnergroups/{status,finalizers}:{get,update,patch}`, none of which the GMC ClusterRole currently holds. Dropping `escalate` made e2e fail with "is attempting to grant RBAC permissions not currently held". Real fix requires either expanding the GMC ClusterRole to cover the AGC Role's permission set (widens GMC blast radius) **or** a `ValidatingAdmissionPolicy` constraining which Role specs the GMC SA may create (preferred — keeps GMC's direct grants narrow). Tracked separately. | [actionsgateway_controller.go:10](../../cmd/gmc/internal/controller/actionsgateway_controller.go), [builder.go (`buildAGCRole`)](../../cmd/gmc/internal/controller/builder.go) | Add ValidatingAdmissionPolicy on `rbac.authorization.k8s.io/roles` create, restricting verbs/resources to the AGC permission set and `metadata.name` to the AGC SA name. |
| B2 | 🔴 | `patch`/`update` on `namespaces` cluster-wide so the GMC can stamp PSA labels — also lets a compromised pod relabel `kube-system` PSA to `privileged`. | [role.yaml:8](../../cmd/gmc/config/rbac/role.yaml), [actionsgateway_controller.go](../../cmd/gmc/internal/controller/actionsgateway_controller.go) (`applyNamespacePSA`) | Move PSA stamping behind a per-CR admission webhook on Namespace, OR add a ValidatingAdmissionPolicy gating which namespace names the GMC SA may patch. Tracked separately in the Queue. |
...
| B4 | 🟡 | AGC `secrets:list` returns full Secret bodies (including GitHub App credential) on every list. | [builder.go:99](../../cmd/gmc/internal/controller/builder.go) | Restrict to labeled `PartialObjectMetadata` watch + per-name `Get`, or `resourceNames` matching the agent-pool naming pattern. Tracked separately in the Queue. |
