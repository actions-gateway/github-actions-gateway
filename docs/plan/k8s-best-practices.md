...
| A4 | 🔲 | Provisioner polls pod state every 5 s via `r.Get` instead of watching — ~200 gets/s at 1,000 sessions; 5 s detection latency on a 10 s job. **Tracked as [Q64](../STATUS.md).** | [provisioner.go](../../cmd/agc/internal/provisioner/provisioner.go) | Replace the poll ticker with a cache-backed `Watch` (single shared Pod informer filtered by `managedBy`). |
