...
| A3 | 🔲 | RunnerGroup controller has no `Owns()` / watch on worker Pods or agent Secrets — evicted pods leave `ActiveSessions` stale until the next Generation bump. **Tracked as [Q63](../STATUS.md).** | [runnergroup_controller.go](../../cmd/agc/internal/controller/runnergroup_controller.go) | Add `.Watches(&corev1.Pod{}, podToRunnerGroup)` filtered by the `actions-gateway/runner-group` label. |
