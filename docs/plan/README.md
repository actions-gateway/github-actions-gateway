...
| [q300-gmc-kindnet-e2e-flake.md](q300-gmc-kindnet-e2e-flake.md) | Systemic kindnet `e2e / e2e` leg flakiness: different unrelated specs fail run to run (webhook `context deadline exceeded`, job-delivery 0-worker stalls, egress→GitHub timeouts, NetworkPolicies unenforced) — root cause is kind's 100m CPU limit on kindnetd starving its in-band kube-network-policies enforcer | ❌ Open — fix shipped (CPU limit removed at bring-up; 15 %-throttled → 0 measured); keep open until the kindnet leg soaks clean on `main` ([Q300](../STATUS.md#Q300)) |
...
| [v2-api-gap-analysis.md](v2-api-gap-analysis.md) | Full v1↔v2 gap analysis (API surface + controller/webhook behavior): what v1 does that v2 doesn't, with per-gap evidence; distinguishes genuine gaps from intentional design differences | ⓘ Review — 8 gaps found: the noProxyCIDRs admission gap was fixed same-day (#641), its GHES residual closed by the Q322 follow-up, and Q323/Q325/Q326/Q327/Q328/Q329 since fixed; still open: [Q324](../STATUS.md#Q324). Parity/intentional differences recorded so future audits don't re-litigate them |
