...
| [release-friction-reduction.md](release-friction-reduction.md) | Remove repeated manual friction from the release + pre-GA dogfood gate: make `publish.yml` own the Release (tag-derived prerelease flag + auto notes/digests), a one-command dogfood validate wrapper, the `setup.sh` `workerImage`-preserve footgun fix, and Calico path-filter narrowing | ▶ Planned — filed from the v1.1.0 cut ([Q293](../STATUS.md#Q293)–[Q296](../STATUS.md#Q296)); Bucket 1 (`publish.yml`) is the highest-leverage first build |
