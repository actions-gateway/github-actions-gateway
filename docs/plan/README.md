...
| [q242-g1-proxy-destination-allowlist.md](q242-g1-proxy-destination-allowlist.md) | G.1: admin-set, FQDN-enforced destination allowlist on the per-tenant egress proxy so CI jobs reach build dependencies (e.g. `proxy.golang.org`) without forfeiting per-tenant egress attribution | ⓘ Design — awaiting sign-off; promotes Appendix G.1 from the non-committed Q19 bundle |
