...
| [release-friction-reduction.md](release-friction-reduction.md) | Remove repeated manual friction from the release + pre-GA dogfood gate: make `publish.yml` own the Release (tag-derived prerelease flag + auto notes/digests), a one-command dogfood validate wrapper, the `setup.sh` `workerImage`-preserve footgun fix, and Calico path-filter narrowing | ▶ In progress — filed from the v1.1.0 cut; Bucket 1 (`publish.yml` owns the Release, Q293) ✅ done; Q294–Q296 open |
