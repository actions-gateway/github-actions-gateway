...
| [required-status-checks.md](required-status-checks.md) | Make the CI quality gates merge-blocking: an always-running `gate` summary job per workflow (safe to require despite path-gating) + the ruleset edit that marks them required | ✅ Done — nine `gate` jobs + ruleset `default-protect` required_status_checks all in place; CI failures now block merges |
