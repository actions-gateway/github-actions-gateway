...
| **1.6** | The ARC-parity ports: [Q719](../queue/Q719.md) RWX storage validation, then [Q727](../queue/Q727.md) the non-privileged `container:` path | release-1.6.md, written when 1.5 tags |
...
What justifies it is [Q719](../queue/Q719.md) and [Q727](../queue/Q727.md), which are the last two [ARC parity](arc-parity.md) gaps and cannot compress into 1.5: Q727 is `L` and strictly depends on Q719's `ReadWriteMany` validation, and 1.5 already carries three `M` items plus a marketing body of work.
