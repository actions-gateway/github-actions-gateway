...
> **Status: ❌ Open — filed 2026-07-20, tracked as [Q359](../STATUS.md#Q359).**
> No phase has started. This doc is the design sketch and phase plan; each phase
> revises it with findings before the next begins.
...
### Phase 1 — per-RunnerSet usage observability (M)

The AGC samples CPU/memory usage for the worker pods it owns and aggregates per
`RunnerSet` × container.

- **Metrics source (decision needed at pickup):** default candidate is
  `metrics.k8s.io` (metrics-server) — present on most clusters including GKE,
  cheap to poll, needs only a small RBAC addition (`pods.metrics.k8s.io`
  get/list in the tenant namespace). Alternatives: kubelet Summary API (finer,
  heavier RBAC) or requiring a Prometheus (external dependency — avoid).
  Known limitation either way: ~15s resolution under-samples very short jobs;
  acceptable, since short jobs are not the sizing constraint.
- **Aggregation:** in-memory per RunnerSet × container: peak, p50/p95 of
  per-job peaks, sample count, window start. Bounded cardinality (one entry per
  RunnerSet × container name).
- **Export:** Prometheus gauges/summaries (e.g.
  `actions_gateway_worker_usage_cpu_peak_cores`,
  `…_memory_peak_bytes` labeled by runnerset/container).
- **Docs:** an operator recipe — read the metrics, derive `requests`/`limits`
  the same way the dogfood plan did by hand.
...
1. Metrics source: `metrics.k8s.io` vs kubelet Summary API (Phase 1).
2. Aggregation window and decay — rolling N jobs vs time-windowed.
