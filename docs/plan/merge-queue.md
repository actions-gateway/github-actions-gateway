...
- Demote e2e from per-PR to merge-group-only (halves e2e volume again; trade-off: sessions learn of an e2e failure only at queue time).
  **Adopted 2026-08-12.**
- `docs/STATUS.md` contention (77% of commits) still forces manual conflict resolution before a PR can enqueue; a per-item-file backlog format would remove it structurally, but is a large format change touching the `session-backlog` skill and lint, and only worth it if post-queue measurement shows it binding.
