...
> proxy destination-allowlist / SSRF revisit that this audit deferred has since
> shipped as Q242 (Appendix G §G.1). The remaining deferral is the worker
...
| Proxy has no app-layer destination allowlist / connection cap | Medium | **Allowlist resolved (Q242)** — Appendix G §G.1 shipped, and `actions_gateway_proxy_connect_denied_total` makes each denial an SSRF signal; this audit's metadata-service/SSRF framing was the revisit argument that carried it. The connection-cap half remains open as [Q565](../STATUS.md#Q565) |
