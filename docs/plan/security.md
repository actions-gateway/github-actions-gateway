...
- ~~SBOM + image signing of published images.~~ **Done (Q28)** — addresses OWASP A08:2025 (Software & Data Integrity / supply chain). On a `v*` tag publish, [`publish.yml`](../../.github/workflows/publish.yml) builds and pushes the five first-party images (gmc, agc, proxy, worker, wrapper) to GHCR, generates an SPDX-JSON SBOM per image with `syft`, signs each image **keyless** with `cosign` (sigstore/Fulcio via GitHub Actions OIDC — `id-token: write`, **no long-lived key and no stored signing secret**, secure-by-default: no secrets-as-config), and attaches the SBOM as a keyless `cosign attest` attestation. PR CI ([`security-scan.yml`](../../.github/workflows/security-scan.yml)) builds each image and generates its SBOM (uploaded as a build artifact) so the SBOM path can't silently break, but does **not** push/sign/attest — those need a registry push + OIDC and run only on publish. Operator verification (`cosign verify` against the OIDC identity, SBOM retrieval) is in [security-operations.md § Image provenance](../operations/security-operations.md#image-provenance-signature--sbom-verification).
- ~~SLSA build-provenance attestation on published images.~~ **Done (Q103)** — each image carries a signed SLSA build-provenance attestation (`actions/attest-build-provenance`) on its index digest, via the same keyless Fulcio/Rekor path as the signatures (**SLSA Build L2**). buildx's unsigned default provenance is disabled in favour of it; reaching Build L3 would require an isolated reusable-workflow builder. Verify with `gh attestation verify` / `cosign verify-attestation` — see [security-operations.md § Verify build provenance](../operations/security-operations.md#verify-build-provenance).
