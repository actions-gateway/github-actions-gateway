...
|---|---|
| New or changed CRD fields / API surface | [design/03-api-contracts.md](../design/03-api-contracts.md) — add the field with its comment block; [design/02-architecture.md](../design/02-architecture.md) — update prose, and the metrics table if new metrics were added. Apply the [API design rules](api-review.md#design-rules) *before* writing the field, and check the change against [Is this change breaking?](api-review.md#is-this-change-breaking) if it touches shipped surface. |
...
| New or changed **user-facing capability** (a marketable feature, a new tenant/operator capability, a competitive differentiator) | The website positioning pages: [why-gag.md](../why-gag.md), [index.md](../index.md), **and [roadmap.md](../roadmap.md)** — keep the value proposition, the competitive placement vs ARC, and the roadmap buckets current with what actually ships. On the roadmap specifically: a shipped capability graduates from "In progress / near-term" to "Available now (1.0)", and its maturity (GA vs. `v2alpha1`/alpha) must be stated honestly — use the `gag-maturity-badge` pill (`alpha`/`beta`) for a not-yet-GA item, distinct from the `gag-v2-badge` API-surface pill. **This is gated:** every bullet under "In progress / near-term" and "Exploring / longer-term" carries an invisible `<!-- q:QN -->` annotation naming the backlog rows behind it, and `make roadmap-check` fails when a named row no longer exists (it shipped) or has moved between the Queue and Deferred (see [`scripts/check-roadmap.sh`](../../scripts/check-roadmap.sh)). Pair with the operator usage docs above. Internal-only changes (refactors, types/codegen, tooling) need no website update. |
