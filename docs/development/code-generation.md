...
- **v2 condition constants live in TWO synced files.** `api/v2alpha1/conditions.go` and `api/v2beta1/conditions.go` must be byte-identical except the `package` line; a new condition type or reason must be added to **both**. This is now **lint-enforced, not just convention**: `scripts/check-conditions-sync.sh` (wired into `make check` via the `conditions-sync-check` target) diffs the two ignoring only the package line and fails — naming the divergent lines — on a one-sided add. There is still no generator, so you edit both files by hand; the lint just catches drift. The AGC reconcilers compile against the `v2alpha1` constants even though v2beta1 is the storage/hub version. A new condition needs **no** `make manifests`/`make generate` — condition types are runtime `.status.conditions[]` values, not schema.
