...
[`git-merge-status.sh`](../../scripts/docs/git-merge-status.sh) and [`merge-table-rows.awk`](../../scripts/lib/merge-table-rows.awk) parse Markdown tables and stay in `awk` for that reason: a merge driver reconstructs the file line for line, conflict-marker fallback included.
