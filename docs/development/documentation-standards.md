...
| **Name a globally-installed skill, never link it** | A skill under `~/.claude/skills/` is outside every repo and its source is a private repo (`karlkfi/claude-skills`, measured 2026-08-16), so a link from `docs/` resolves for no reader and `check-doc-links.sh` cannot see that it is dead. Name it for agents and keep the page sufficient for a contributor without it: [maintaining-backlog.md](maintaining-backlog.md) does this for the `backlog` skill and [testing.md § Diagnosing failures](testing.md#diagnosing-failures-measure-before-asserting-a-root-cause) for `verify-claims`, each keeping the rules readable off the repo's own cases. A **repo-local** skill under `.claude/skills/` is in-tree and links normally ([`dispatch-worker`](../../.claude/skills/dispatch-worker/SKILL.md)). |
