...
| **Anchor a cross-file section reference** | Cite another file's section with an anchor, never as prose beside a bare file link. Write `[testing.md § A negative assertion must be able to fail for only one reason](testing.md#a-negative-assertion-must-be-able-to-fail-for-only-one-reason)`, not `[testing.md](testing.md) § A negative assertion...`. Both render alike and only the first is checked, for the reason the row above gives, so the prose form is verbatim the day it is written and silently wrong after any rename. `make doc-links` now rejects it (Q686). Converting the 37 the tree carried found 5 whose section had moved or been renamed out of the target, one of them into a file that had been split in two: dead pointers no gate could see. A citable place that publishes no heading slug takes an explicit `<a id>`, which the anchor check resolves. |
...
| **Table of contents** | Long docs (~400+ lines) carry a `## Table of Contents` after the intro listing h2s (plus h3 for operator docs), in document order. Anchors follow GitHub slug rules (duplicate headings get `-1`/`-2`); verify against the rendered page. **Gated on every page that carries one** (Q865, Q911): `make doc-toc-check` fails a heading the page's index never gained, an entry naming no heading, or entries out of document order. `make doc-links` does not cover this: it resolves the anchors a page writes, so a heading the index never mentions has no link for it to fail. How deep each page is held is read off its own entries: an index already naming an h3 is held to h3, one naming only h2s is held to h2, so the choice above stays the page's. |
...
- **Links and anchors are checked, twice.** `make doc-links` (in `make check`) fails on broken cross-file links and heading anchors as **github.com** resolves them, and on a section cited as prose beside a bare file link, which no anchor check can reach; `make docs-build` re-checks them as the **published site** does.
...
| `make doc-links` — broken cross-file links + heading anchors, GitHub slugs (Q52); unanchored section references (Q686) | 1, 2 | Wired (`make check`). The automated guard against link rot. |
