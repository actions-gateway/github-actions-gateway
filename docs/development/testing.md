...
**Swapping the action for a raw `docker buildx build` would be the tidier fix and is the wrong trade.** It moves `cache-from: type=gha` into a `run:` step, which on its own receives none of `ACTIONS_RUNTIME_TOKEN`, `ACTIONS_CACHE_URL` or `ACTIONS_RESULTS_URL`: the runner injects those into action processes only, and `docker/setup-buildx-action` exports no environment of its own (measured 2026-08-18 on a hosted runner).
