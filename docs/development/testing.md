...
So after any dependency change, run `make deps-sync` (the one-shot remedy) and commit the result before pushing; see [go-workspaces.md § Changing dependencies](go-workspaces.md#changing-dependencies).
...
**A nested suite only reaches that distinction if it hands the status back**, which is what `die_if_killed` in [`scripts/lib/common.sh`](../../scripts/lib/common.sh) is for: a suite that compares a killed child's `143` against a wanted `1` reports a false `FAILED` from inside a fan-out that would have called it `KILLED` (Q1023).
Guard every captured-status comparison with it; [bash-style.md § Shared helpers](bash-style.md#shared-helpers-and-makefile-wiring) has the shape and the two traps.
...
- **Reading Ginkgo's progress report as the failure.** `--poll-progress-after 30s` prints, for every spec still running at that mark, a block headed `<spec name> (Spec Runtime: 30.003s)` with an `At [By Step] waiting for probe pod … (Step Runtime: 2.85s)` line and the goroutine's source, and it is emitted for passing specs too.
  It reads like a hung wait and is a snapshot, not a verdict: the failure is the later `[FAILED] in [It] - <file>:<line> @ <time>` line, and a spec's failure-path dump (`kubectl describe` of the pods it names) carries the pod's own `Started`/`Finished` times.
  Q1015 was filed as a 2-minute pod wait timing out off that block; the pod had completed in 10 s with `CURL_RC=28`, the failure was the assertion on the next line, and the row carried "needs a live cluster and a maintainer at the keyboard" for a week on a question the log answered.
...
- **A control that cannot fail is the costliest kind, and an expected red is where you stop looking for one.** Every other check has you wanting the opposite of what a broken instrument produces; a test that inverts its own mechanism has you wanting the red, so a spurious failure reads as confirmation and the method that produced it still looks sound.
  Five instances in one batch on 2026-09-24, four of them in code that gates something.
  `scripts/ci/workflow-acting-steps-test.sh` prints `FAIL`, prints its all-passed banner, and exits 0: thirteen of its cases call the helper as `dir="$(run_check …)"`, and the `fails=$((fails + 1))` that `fail()` performs inside a command substitution increments a subshell's copy the parent never sees (Q1136, confirmed by mutation rather than by reading).
  The suite [#1976](https://github.com/actions-gateway/github-actions-gateway/pull/1976) added was built on that shape and carried it in twelve of nineteen cases.
  `check-queue-rules.py`'s rule 9 matched *any* mention of a plan in `docs/plan/README.md`, so it read whichever row sorted earliest rather than the plan's own.
  That is a verdict about a different row than the one it names, and it passes whenever the shadowing row happens to read closed.
  And a test helper restoring an admission guard read *any* create error as "the guard is back in force", so it reported a restored guard whatever went wrong.
  The fix in every case is a ledger the parent can read: `fail()` appends a line to a file, and a `count_fails` reads the count back before anything decides, because an append from a subshell does reach the parent.
  Add a count of the reports beside it, so a case that vanishes fails rather than quietly shrinking the log.
  `scripts/ci/workflow-publish-steps-test.sh` is the worked shape.
- **A tool that exits 0 having printed nothing is the same class at its mildest.** `gh attestation verify` writes its summary only to a TTY; captured or redirected it is silent, so make it emit something assertable (`--format json`, then read the predicate) rather than trusting the status.
...
- **A count or a superlative is a claim, not a recollection.** The [markdown-gates plan](../plan/archive/markdown-gates-parser.md) first called `check-doc-links.sh` the only script here with no `-test.sh` companion, from memory of an earlier sweep; re-running it found six of the fourteen scripts in `scripts/docs/` untested, and the claim shipped rescoped to the four gates that plan covers, where it is exactly true.
  ...
  **A count read off a list in front of you is not exempt, and is the easier one to publish.** On 2026-09-24 two sessions reported 21 and 15 cancelled jobs for one run and reconciled the gap as two instruments reading the same thing differently.
  Both APIs return 15; the 21 was counted by eye off printed rows, by a session that had spent the preceding hour telling others to count programmatically.
  The reading that makes both parties right was available without running a command, and that availability is the tell.
  Reconciling two numbers is one call.
...
- **`EventReader` fails open.** Nil short-circuits the autoscaler-verdict gate, which returns `CapacityAvailable` with "no direct API reader is wired" and never closes.
  That is deliberate and documented at the declaration, so a suite not exercising the gate is right to omit it.
- **`APIReader` falls back.** Nil silently substitutes the cached client, so the read still succeeds and the only difference is whether an informer was started for the kind.
  Nothing reports that, which is what makes it the shape to go looking for.

The reads where the fallback matters are the ones production takes uncached *on purpose*, each for a reason the cached read defeats:
...
Q1078 found `APIReader` nil in three of the five harnesses that construct a `RunnerSetReconciler`, while two beside them wired it, and two of the three carried a comment claiming parity with `main.go` for the field next to the missing one.
A comment claiming parity is scoped to the field it annotates and says nothing about a field that is absent, which is what makes the gap read as deliberate.
So diff the harness's literal against `main.go` field by field rather than reading the comments beside the fields that are there, and for each reader ask which of the two shapes its nil takes before deciding the omission is safe.
...
**The cert-manager manifest is cached too, not just its images.** `prepull-manifest-images.sh` persists the manifest it fetched beside the image tar, and the cluster step applies from that copy (`make apply-cert-manager CERTMANAGER_MANIFEST=…`), so the apply is no longer a second fetch of the same URL with no retry, which is the shape that killed run 34856925669 on a 504 from the release CDN (Q1125).
Its cache key carries an `-m2` generation because an `actions/cache` entry is immutable once written, so a key change is the only way a contents change takes effect before the next version bump.
Calico applies from its own retried temp-file fetch (`install_calico`) and metrics-server still applies live from `setupMetricsServer`, so neither reads the persisted manifest yet and both keep their original keys.
