...
| Investigation H | `PROBE_ABANDONED_TEST=true` | What does the run service do with a `completejob result=abandoned` for an acquired-but-never-run assignment (the Q628 release): re-dispatch the job, or conclude it? Decides whether the listener also needs a re-run arm; the same call live-confirms the completejob wire serialization that keeps `AGC_FANOUT_COMPLETION` gated off. | [q645-abandoned-completion.md](../plan/q645-abandoned-completion.md) |
