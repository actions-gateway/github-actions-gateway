...
**A proxy replica costs a whole worker node, so an autoscalable pool can starve a coexisting one.** Every proxy pod — v1's inline `actions-gateway-proxy` pool and v2's `<proxy>-proxy` pool alike — stamps `app: actions-gateway-proxy` and carries a `requiredDuringScheduling` `podAntiAffinity` on `kubernetes.io/hostname`. The scheduler enforces a *running* pod's term against an incoming one, so pods of either pool repel pods of the other: N proxy replicas in a namespace need N worker nodes, and the default cluster has two. A spec that stands up two coexisting pools there has **zero** headroom. Leave either pool autoscalable and it will eventually take both nodes — the pods request `10m` CPU, so 60 % utilization is 6 millicores and any startup burst trips the HPA. That is Q570: the v1 pool's HPA (min 1, max 3) scaled to 2 about 15 s in, and when `gag-migrate` created the v2 pool a minute later its pod was unschedulable for good. Nothing recovers it either — once both pools exist their HPA selectors overlap, so both wedge on `AmbiguousSelector`/`FailedComputeMetricsReplicas` and the v1 pool can never scale back down. Pin such a pool with `utils.TenantFixture.WithProxyReplicas(1, 1)`, and label the spec `multi-node`. Raising the wait does nothing: the pod is deadlocked, not slow, and `kubectl describe pod` says so in one line (`FailedScheduling … didn't satisfy existing pods anti-affinity rules`).
