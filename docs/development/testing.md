...
`make gate-lists-check` ([`scripts/ci/gate-list.sh`](../../scripts/ci/gate-list.sh)) keeps the set of gates `make check` runs from acquiring a second copy. `CHECK_FAST_GATES` and `CHECK_HEAVY_GATES` in the root [`Makefile`](../../Makefile) are the source of truth; the `check` recipe, `make list-gates`, and this page all derive from them. Adding a gate is one edit: append the target name to `CHECK_FAST_GATES` and give the target its own `.PHONY` line and a `##` help line beside its rule.
...
- this page stops citing `make list-gates`. That last rule keeps the pointer alive; it cannot detect a transcribed list added *beside* the pointer, which stays a review concern.
