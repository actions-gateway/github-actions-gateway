...
**Swapping the action for a raw `docker buildx build` would be the tidier fix and is the wrong trade.** It moves `cache-from: type=gha` into a `run:` step, where the Actions cache credentials are not exposed at all: such a step receives none of `ACTIONS_RUNTIME_TOKEN`, `ACTIONS_CACHE_URL` or `ACTIONS_RESULTS_URL`, and `docker/setup-buildx-action` v4.2.0 exports no environment of its own (measured 2026-08-18 on a hosted runner).
...
The first-party images (`gmc`/`agc`/`proxy`/`fakegithub`/`worker`) are built and pushed to the local registry by the bake step, whose buildx `GHA_CACHE` is configured but inert, because the bake runs from a `run:` step that has no Actions cache credentials (Q931).
