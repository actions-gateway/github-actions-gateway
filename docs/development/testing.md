...
`make doc-links` runs `scripts/check-doc-links.sh` over every tracked, non-vendored Markdown file and is wired into `make check`, so the local pre-review gate matches CI. The dedicated `doc-links` job in `.github/workflows/unit-test.yml` runs the same `make doc-links` target, gated on a `docs` paths-filter (`**.md`, the checker, and the workflow itself); it is the one job that runs on a docs-only change.
