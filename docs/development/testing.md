...
**Why the split.** `make check`'s heavy phases hold the [machine-wide heavy-build lock](#resource-auto-throttle-on-gui-dev-machines), so repeat runs — yours plus every sibling worktree session's — queue behind each other instead of overlapping. Over a 21-day sample of session transcripts, a session averaged 3–4 full runs at a median of 2 minutes but a **p90 of 9 minutes**, the tail being lock wait (Q375). Each extra run also mostly re-verifies code the branch never touched: the unit/coverage step ([`make cover-check`](#coverage-measurement-and-the-ratchet)) measures **every** workspace module regardless of change scope, so only the lint phase gets any benefit from scoping. Running it three times mid-iteration buys almost nothing the module-scoped loop above did not already tell you.
