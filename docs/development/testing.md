...
1. **Registry completeness.** Every first-party module whose `Makefile` defines a `manifests:` target is registered in the script's `MODULES` table. A new module generating manifests fails until someone registers it, so the hole cannot reopen.
2. **Registry fidelity.** Each row's generator list and explicit `output:` rules match that module's own `manifests:` recipe, so the gate regenerates exactly what `make manifests` would rather than a stale approximation.
3. **Drift.** Every regenerated file matches its committed counterpart, and every committed file under a generated output dir is either produced by that module's controller-gen run or named in the script's `EXEMPT` list with the reason. The one exemption today is the GMC-bundled `RunnerGroup` CRD, which is not GMC controller-gen output at all — [`make chart-crds-check`](#install-artifact-validation) holds it byte-identical to the AGC copy (Q73).
...
### The chart uninstall/reinstall check (Q444)

Every tier above starts from a cluster that has never had the chart installed, so nothing exercised the day-two operation an operator actually performs: `helm uninstall` followed by a reinstall. That gap hid a total outage — deleting the `paramKind`-bearing `ValidatingAdmissionPolicy` permanently destroys the apiserver's parameter informer for that GroupVersionResource, and the reinstalled binding then denies **every** `runnergroups`/`runnersets`/`runnertemplates` write cluster-wide until kube-apiserver restarts.

[`scripts/chart-reinstall-check.sh`](../../scripts/chart-reinstall-check.sh) closes it. It runs against a cluster that already has the release installed, captures the release's own values, cycles `helm uninstall` → `helm install`, and asserts admission still works. CI runs it as a step in `e2e-reusable.yml` after the Ginkgo run — `E2E_SKIP_TEARDOWN` leaves the release up, and mutating it is only safe once the suite is done. Locally, against a cluster the e2e flow left running:

```bash
make chart-reinstall-check KIND_CLUSTER=actions-gateway-e2e
```

Its **deterministic** assertion is retention: every rendered policy declaring `paramKind` must still exist after the uninstall, and no binding may. The functional probe that follows (a class-naming write must be denied by the allowlist, not by `no params found`) is confirmation, not the detector — whether a deleted policy actually breaks parameter resolution depends on the apiserver observing the empty policy set before the reinstall recreates it, and its refresh backs off to ~60 s, so a fast cycle can pass by luck. The static half of the same invariant is enforced by [`make manifest-validate`](#install-artifact-validation), which needs no cluster.
...
It also asserts the **admission-policy retention invariant** (Q444): every rendered `ValidatingAdmissionPolicy` that declares `paramKind` must carry `helm.sh/resource-policy: keep`, and no `ValidatingAdmissionPolicyBinding` may. Deleting a param-declaring policy permanently destroys the apiserver's parameter informer for that GroupVersionResource, so a `helm uninstall` + reinstall would deny every matched write cluster-wide; retaining a *binding* would instead leave the guard enforcing after uninstall and make `admissionPolicy.enabled=false` a silent no-op. This is the cluster-free half of [the chart uninstall/reinstall check](#the-chart-uninstallreinstall-check-q444) and the one that actually catches a regression.
