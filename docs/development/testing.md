...
- A Claude Code `PreToolUse` hook ([`scripts/agent/claude-go-throttle-hook.sh`](../../scripts/agent/claude-go-throttle-hook.sh), wired in `.claude/settings.json`) automates that prefix for agent-run commands: a bare `go build`/`go test` is rewritten transparently and auto-*allowed*.
  ...
  Only when the command holds more than one `go build`/`go test` invocation to throttle (one prefix, two places to put it) does it **`deny`**, with the specific reason, rather than throttle one and leave the other running unthrottled; there, add the prefix manually or split the invocations onto separate commands.
