...
| Investigation H | `PROBE_ABANDONED_TEST=true` | What does the run service do with a `completejob result=abandoned` for an acquired-but-never-run assignment (the Q628 release): re-dispatch the job, or conclude it? **Answered 2026-08-04: conclude, as `success`, in one second** — the wire format is confirmed (204) and the false-green remedy is Q676. Re-run it to measure a different result value. | [q645-abandoned-completion.md](../plan/q645-abandoned-completion.md) |
