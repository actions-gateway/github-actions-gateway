...
**Limitation: a synced branch stops auto-rebasing.** Dependabot only rebases a branch it still owns, and the `chore(deps): sync …` commit marks the branch as modified by someone else. From that point Dependabot leaves it alone, so a synced PR that is not merged before `main` moves under it goes **permanently conflicting**; it will never self-heal. Recover with `@dependabot recreate`, which rebases off current `main` and re-runs the sync from scratch.

Do **not** hand-resolve that conflict. The branch's `go.mod`/`go.sum` were resolved against the older `main`, so a manual merge can silently *downgrade* a module `main` has since moved forward: PRs #733, #734, and #735 each conflicted only in `api/go.mod` and `api/go.sum`, yet merging any of them as-is would have reverted `golang.org/x/text` from v0.39.0 back to v0.38.0 across `api/`, `cmd/agc`, and `cmd/gmc`. The practical rule: merge Go bump PRs promptly, and reach for `recreate` rather than a merge commit once one has gone stale.
