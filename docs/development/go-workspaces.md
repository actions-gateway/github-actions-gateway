...
Every gate here already passes `-o` into the gitignored `.build/`. Building one **by hand** while iterating is the case that does not: `go build ./<group>/<tool>` with no `-o` drops the binary in the module directory under the package's own name, where nothing ignores it and only reading `git status` before staging catches it. Point `GOBIN` at the same ignored directory instead, and an ad-hoc build lands where the gates' binaries already do:
