...
| cosign version + 4 per-platform digests (3 files) | `COSIGN_VERSION` in the root `Makefile`, both `cosign-release` inputs in [`publish.yml`](../../.github/workflows/publish.yml), and the digest table in [`scripts/release/download-cosign.sh`](../../scripts/release/download-cosign.sh) | **updatecli** ([`updatecli.d/cosign.yaml`](../../updatecli.d/cosign.yaml), weekly, rewriting all three, which is what keeps `make cosign-pin-check` green through a bump. The digests come from the release's **signed** checksums file, verified before they are read; see the strategy note below) |
...
| csi-driver-nfs + the NFS server image (shared-storage harness) | `CSI_DRIVER_NFS_VERSION` in [`scripts/e2e/rwx-storage-cluster.sh`](../../scripts/e2e/rwx-storage-cluster.sh), and the digest-pinned image in [`test/rwx-storage/nfs-server.yaml`](../../test/rwx-storage/nfs-server.yaml) | **manual**, per [Deliberately manual pins](#deliberately-manual-pins). Nothing runs this harness on a schedule, so a bump is only meaningful when someone runs `make test-rwx-storage` in the same change |
...
2. **(version+checksum pins only) a checksum source** — a `shell` source that `dependson` the version source and, with the resolved tag templated in, produces the matching SHA-256.
   ...
   - **Verify a signed checksum file.** [`scripts/updatecli/cosign-release-sha256.sh`](../../scripts/updatecli/cosign-release-sha256.sh) checks the sigstore signature on cosign's `cosign_checksums.txt` before reading a digest out of it, and prints nothing if it does not verify.
     Reach for this where the pin is a trust anchor rather than a convenience.
     Both cheaper strategies trust bytes fetched from the release, and a GitHub release asset stays mutable for an existing tag (Q126/Q127): tolerable for a linter, not for the binary that verifies every release this repo publishes.
     The verifier is the *currently pinned* cosign, downloaded by a `make cosign` step in the workflow, so the chain is anchored in the pin being replaced rather than in the release being fetched.
...
| [`cosign.yaml`](../../updatecli.d/cosign.yaml) | `COSIGN_VERSION`, both `cosign-release` inputs, and the four per-platform digests in `download-cosign.sh` | signature-verified `cosign_checksums.txt` lines (one shell source per platform) |
...
- **`CSI_DRIVER_NFS_VERSION`** in [`scripts/e2e/rwx-storage-cluster.sh`](../../scripts/e2e/rwx-storage-cluster.sh), and the digest-pinned NFS server image in [`test/rwx-storage/nfs-server.yaml`](../../test/rwx-storage/nfs-server.yaml): the storage backend behind the [shared worker storage validation](testing.md#the-shared-worker-storage-validation).
  Deliberately off the cadence rather than pending automation: unlike the autoscaler harnesses, this one detects nothing about upstream.
  It answers whether GAG's worker pod shape mounts an RWX volume, and the driver is scaffolding for that question rather than its subject.
  A bump is worth taking when the worker pod's volume or security handling changes, and only alongside a `make test-rwx-storage` run in the same PR.
