...
| <a id="Q242"></a>Q242 | [Implement G.1 proxy destination allowlist](plan/q242-g1-proxy-destination-allowlist.md) | `security` `infra` | 🔲 | L | Approved (was Appendix G.1/Q19). EgressProxy destinationFQDNs/CIDRs gated by GMC --allowed-egress-fqdns/cidrs allowlist + webhook + proxy CONNECT check. v2beta1 blocker; docs lead with in-cluster mirror. Unblocks [Q224](#Q224) egress half. |
| <a id="Q243"></a>Q243 | [Per-tenant egress-IP reference architecture (cloud)](plan/gke-dogfood.md) | `security` `infra` `docs` | 🔲 | L | Per-tenant egress-IP on a cloud: spike Cilium Egress Gateway vs per-tenant NAT; doc single-tenant-direct vs production topology + cost. Dogfood stays direct (single-tenant). Spike pre-beta; escalate to blocker only if it needs a CRD change. |
