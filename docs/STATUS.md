...
| <a id="Q139"></a>Q139 | Flake: e2e E2E_GMC_TenantProvisioning_ProxyConnectWorks — curl through proxy gets HTTP 504 | `tests` `bug` `infra` | 🔲 | M | Root cause: live api.github.com returned transient 504 (GMC /meta 504 same run corroborates); proxy emits 502 not 504, so not a tunnel timeout. Fix in PR: targeted curl --retry on upstream 5xx. Open pending multi-run e2e validation. |
