...
| <a id="Q247"></a>Q247 | [AGC runner session recovery after job recycling](plan/gke-dogfood.md) | `infra` | ▶ | M | Two bugs, both fixed + regression-tested: (1) RenewJob sent MessageID not RunnerRequestID (#481; dup pods); (2) residual: unbounded renewal call wedges loop under load, single-pod orphan at exactly 10min. Live GAG e2e pending. Blocks [Q224](#Q224). |
