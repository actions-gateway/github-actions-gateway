...
| <a id="Q245"></a>Q245 | [FQDN egress: split intent from CNI backend + GKE backend](plan/q245-fqdn-intent-backend-split.md) | `security` `infra` | ▶ | L | Impl+envtest done: intent CIDR\|FQDN + GMC --fqdn-policy-backend (none\|cilium\|calico\|gke); gke FQDNNetworkPolicy additive-allow so base default-deny NP stays. Non-breaking: CiliumFQDN\|CalicoFQDN deprecated, conversion fuzzed. Live-GKE deferred. |
