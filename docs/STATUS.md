...
|---|---|---|---|---|---|
| 5h | AGC envtest manager must match production Secret cache-isolation | `tests` `security` | ▶ | S | Production AGC sets `Client.Cache.DisableFor: [&corev1.Secret{}]` in `cmd/agc/main.go` (W4 / H-2, decision D-3), so `r.List` / `r.Get` on Secrets bypass the controller-runtime cache and go straight to the API server — `Secret.Data` is never resident in the in-process informer cache. The envtest harness in `cmd/agc/internal/controller/integration/suite_integration_test.go` does not replicate that option, so its manager *does* cache Secrets, including `.data`. Two consequences: (1) envtest is less safe than production for the duration of the test run, and (2) it produces false-positive flakes from cache-lag races that production code cannot hit (e.g. the `non-retriable: pool exhausted: no agent available` symptom observed on PR #63 — fix: add the same `DisableFor` to the envtest manager, which also removes the need for any defensive code in `agentpool.EnsureAgents`). |
