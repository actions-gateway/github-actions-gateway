...
| <a id="Q142"></a>Q142 | Helm the sole cluster-install path; kill chart/config CRD duplication | `infra` `bug` `tests` | ▶ | L | A) single-source CRDs DONE: chart CRDs generated from controller-gen sources + drift gate (make chart-crds-check; GMC-bundled==AGC), resolved Q73. Remaining: B) e2e via helm not make deploy; C) delete kustomize overlays (plan/drop-kustomize.md). |
