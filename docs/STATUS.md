...
| <a id="Q389"></a>Q389 | Probe's scale-set fake duplicates `scalesettest` | `tests` | 🔲 | S | `cmd/probe/scaleset_test.go` hand-rolls a ~250-line scale-set stub while [`scaleset/scalesettest`](../scaleset/scalesettest/) models the same protocol semantically and records calls via `Calls()`. Same class as Q368 (shipped), other protocol. |
...
| <a id="Q400"></a>Q400 | [File upstream k8s issue: delete-path probe results never reach `Ready`](operations/node-shutdown-budgets.md) | `docs` | 🔲 | S | Q388 measured: on ordinary delete the probe fails throughout termination but `Ready` never flips; removal comes from `deletionTimestamp`. k8s#124648 covers only eviction, so this looks unreported upstream. Evidence in the linked doc. |
| <a id="Q401"></a>Q401 | [GKE DPv2 NodeLocal DNSCache NP repro — file against Google's docs](operations/troubleshooting.md) | `docs` | 🔲 | S | Google's NodeLocal DNSCache page names DPv2 as exempt from extra NP rules; Q229 measured the opposite (node-local-dns peer required). Capture a minimal repro w/ cluster version and report it — a default-on footgun on Autopilot. |
