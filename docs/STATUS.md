...
| <a id="Q279"></a>Q279 | [Wire self-signed CA into dogfood v2 CRD conversion caBundle](plan/gke-dogfood.md) | `infra` | 🔲 | S | Dogfood applies CRs at v2alpha1 (storage v2beta1) via /convert; it's self-signed so the CRD chart's conversion.caBundle is empty → CR applies fail. Wire in the webhook-server-cert CA (setup.sh + gke-dogfood.md); validate w/ live GMC. |
