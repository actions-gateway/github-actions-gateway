...
| <a id="Q67"></a>Q67 | Regenerate stale CRDs (drift from bumped `k8s.io/api`) | `infra` | 🔲 | S | Committed RunnerGroup CRD's embedded PodSpec is stale (`workloadRef`→`schedulingGroup`, DRA fields) — codegen was broken so it never regenerated. Now unblocked; run `make manifests` and commit the regenerated CRDs. |
