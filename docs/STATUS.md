...
|---|---|---|---|---|---|
| <a id="Q35"></a>Q35 | [K8s audit — §F Observability & operational](plan/k8s-best-practices.md#f-observability--operational-) | `infra` `1.0-gate` | 🔲 | M | 🟡 F5/F6 done (AGC zap flag + health probes). Left: F1 dual loggers `slog`+`zap` (incompatible JSON); F2 no OTel tracing; F3/F4 GMC leader-election grace + tuning flags. See [§F](plan/k8s-best-practices.md#f-observability--operational-). |
| <a id="Q86"></a>Q86 | Cap + redact upstream HTTP response bodies in error paths | `security` | 🔲 | S | Logging-audit Theme B: error paths interpolate upstream HTTP bodies; 2 UNCAPPED carry creds: `runner_auth.go:248/259` (OAuth token), `github_registrar.go:97` (JIT cred+key). Cap+redact, reuse `capBody`. See [logging-audit](plan/logging-audit.md). |
