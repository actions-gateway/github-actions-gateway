...
|---|---|---|---|---|---|
| <a id="Q235"></a>Q235 | [Default worker image is non-functional (inject the wrapper)](plan/worker-wrapper-injection.md) | `milestone` `bug` `infra` | ▶ | M | Default + ARC custom images can't run jobs: bare-upstream DefaultWorkerImage has no wrapper entrypoint. Fix: inject the wrapper at runtime (image volume / init fallback) so any actions/runner image runs unmodified — ARC parity. From Q224. |
...
| <a id="Q237"></a>Q237 | [Docs quality audit: score docs/ against the six-goal rubric](development/documentation-standards.md#goals-what-good-looks-like) | `docs` | 🔲 | M | Score docs/ against the six-goal rubric in [doc standards](development/documentation-standards.md), incl. the AI-slop check — not just scannability. Whole-tree: decide M-vs-L (plan doc) or fan-out workflow on start; ranked findings. |
