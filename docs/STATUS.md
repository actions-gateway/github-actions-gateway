...
| <a id="Q224"></a>Q224 | [GKE dogfood: route production CI green](plan/gke-dogfood.md) | `milestone` `infra` | 🔲 | M | Re-route #5: accounting blocker (Q260) RESOLVED — Option A live-confirmed; unit-test/integration/coverage green, 9/9 completejob OK, no 15-min cancel. Full-matrix sweep now gated on worker capacity (maxWorkers → [Q248](#Q248)). |
...
| <a id="Q248"></a>Q248 | [Right-size GAG dogfood CI runner pods + node pool](plan/dogfood-runner-rightsizing.md) | `infra` | 🔲 | M | Green-unblock capacity DONE (re-route #4: non-preemptible workers-od pool, 3 nodes, no preemption; peak 34% CPU, ~3.8GiB/pod; cpu req 1 fits 4 pods/3 nodes). Broader right-sizing (bake final requests/limits, node sizing, tiers) still open. |
| <a id="Q265"></a>Q265 | [Benchmark Option A fan-out throughput vs scale-set](plan/q260-fanout-completion-reconciliation.md) | `infra` `speed` `tests` | 🔲 | M | Benchmark Option A fan-out throughput before the Q264 scale-set rewrite. Warm, right-sized pool (maxListeners ≫ maxWorkers×fan-out). Key Q: pool holds near maxListeners or collapses (re-route #5 saw 2/8)? Tuning vs wall decides Option A vs Q264. |
...
|---|---|---|---|---|
| <a id="Q264"></a>Q264 | [Migrate AGC acquisition to the runner-scale-set protocol](plan/q264-scale-set-protocol.md) | `infra` | L | **Decision:** Option E — spike + probes DONE, **viable** (#520; all protocol unknowns settled, plan §5a). Re-route #5 live-confirmed Option A reconciles the fan-out AGC-side, so Option E is NOT triggered for correctness. Revive if the [Q265](#Q265) benchmark shows Option A's fan-out tax is a hard wall (not tuning), or for per-group acquire SPOF / a future GitHub fan-out change. |
