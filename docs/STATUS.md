...
| <a id="Q276"></a>Q276 | [Decide the v2 CRD chart install/upgrade story](operations/install.md#optional-the-v2-api-crds) | `infra` `docs` | 🔲 | S | v2 CRD chart (~2.5MB rendered) > 1MiB Helm release-Secret limit → `helm install` fails; install/e2e/docs now use `helm template \| kubectl apply --server-side` (Q74). Decide long-term: split CRDs, crds/ dir, or accept apply-render (no helm-managed upgrade). |
---
