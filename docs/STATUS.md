...
|---|---|---|---|---|---|
| <a id="Q92"></a>Q92 | Auto-throttle bypassed by raw `go`/`go test -race` outside `make` | `infra` `bug` | 🔲 | S | Throttle (#159/#164) only wraps `make` recipes; raw `go build`/`go test -race` in parallel sessions bypass it; an unthrottled `-race` froze WindowServer. Add a PreToolUse hook wrapping heavy go cmds in taskpolicy -c utility/nice+ionice. |
| <a id="Q93"></a>Q93 | Flaky AGC integration tests using global broker-stub sessions | `tests` `bug` | 🔲 | S | Tests like pod_provisioning enqueue on global `RegisteredSessions()[len-1]`; a prior test's lingering-active session (shutdown DeleteSession failed) gets picked → provisioner gets no job. Migrate to owner-scoped `ActiveSessionsForOwner` (#166). |
...
| <a id="Q90"></a>Q90 | Add a LICENSE file + wire the `licenses` image label | `docs` `infra` `1.0-gate` | 🔲 | S | No LICENSE file → code is all-rights-reserved by default, blocking adoption and the `org.opencontainers.image.licenses` label (omitted in Q36). Owner picks a license; add LICENSE + wire the `licenses` arg/label in docker-bake.hcl. |
