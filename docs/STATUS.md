...
| <a id="Q196"></a>Q196 | [Credentials discriminated-union shape (v2beta1 blocker)](design/appendix-h-v2-api-decomposition.md#h15-other-breaking-changes-worth-batching) | `infra` `security` | 🔲 | S | v2beta1 blocker: alpha→beta is the last free breaking change, so the credential shape must be right before the cut. Nest githubAppRef under a discriminated `spec.credentials` parent. Lands at the Q74 graduation. See §H.15. |
| <a id="Q197"></a>Q197 | [Workload-identity credentials (external signer)](design/05-security.md) | `security` `infra` | 🔲 | L | Additive 2nd member of the Q196 union: sign the App JWT via an external signer (no in-cluster PEM). MVP = Vault transit + k8s auth (kind-validatable); cloud KMS follows. Design the schema pre-beta to validate the union; reconciliation fast-follows. |
| <a id="Q181"></a>Q181 | [Validate the ~1000-session ceiling at scale](design/appendix-a-capacity-slos.md) | `tests` `infra` | 🔲 | L | Headline ~1000-session / "4000× memory" claim is arithmetic, not measured (appendix-a marks it unvalidated). Load-test one AGC to the ceiling (memory, GC, latency) and publish. Highest-leverage moat item; not API work. |
...
| <a id="Q191"></a>Q191 | [Broker-compatibility probe suite + report](design/03-api-contracts.md) | `tests` | 🔲 | M | GAG re-implements the broker protocol, so GitHub can break it silently. Expand cmd/probe into a compatibility suite and publish a periodic compat report — turns the re-implementation risk into a managed, visible asset. |
