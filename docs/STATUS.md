...
| <a id="Q172"></a>Q172 | [v2 optional templateRef (default ClusterRunnerTemplate)](plan/v2-api.md#deferred-out-of-the-critical-path) | `infra` | 🔲 | M | Relax templateRef required→optional (non-breaking): resolve unset via ActionsGateway.defaultTemplateRef → a default-marked ClusterRunnerTemplate (StorageClass pattern; ≤1 default; fail-closed TemplateNotFound if none). Cuts onboarding friction; parallels shipped Q168. |
| <a id="Q170"></a>Q170 | [Kubernetes Events for job lifecycle](design/02-architecture.md) | `infra` | 🔲 | S | Broaden the AGC EventRecorder to emit Events on job-lifecycle transitions (acquisition fail, eviction-retry exhaustion, quota rejection, session fail) — only metrics/conditions surface them today; reaper already emits pod events. For kubectl describe. |
