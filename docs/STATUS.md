...
| <a id="Q250"></a>Q250 | [Fix operator copy-paste correctness drift (docs audit Batch A)](plan/q237-docs-quality-audit.md#remediation) | `docs` `bug` | 🔲 | M | 17 high-severity goal-1 findings from the Q237 audit: broken `kubectl apply`/`patch`, wrong CRD apiVersion/group/fields, SA names, metrics ports (8081→8443). Sample verified vs code; re-confirm each before editing. |
| <a id="Q251"></a>Q251 | [Fix stale defaults & counts docs drift (docs audit Batch B)](plan/q237-docs-quality-audit.md#remediation) | `docs` | 🔲 | M | 16 medium goal-1 findings from the Q237 audit: stale defaults (MaxListeners 10→1, proxy CPU 100m→500m), worker-image override mechanism, stale make-check/image/test-gate counts. Re-confirm each vs code. |
