...
| <a id="Q204"></a>Q204 | [Pre-v2: NetworkPolicy egress extensibility check](design/appendix-g-future-enhancements.md) | `security` `infra` | 🔲 | S | Pre-v2 gate with [Q74](#Q74): all foreseen NP egress holes (KMS metadata, telemetry, worker deps) are additive per appendix G.9 — no breaking change. Open call: fold the Vault peer descriptor into a shared EgressPeer type before v2 freezes shapes. |
| <a id="Q218"></a>Q218 | [Worker pod disruption-safety defaults](plan/ecosystem-integration-landscape.md#what-to-evaluate-further--backlog-candidates) | `infra` | 🔲 | S | v2beta1 gate (production-relyable, cf. Q15): node consolidation/descheduler can strand running jobs. Stamp `karpenter.sh/do-not-disrupt` + `safe-to-evict:false` + descheduler-exclusion on worker pods, like the securityContext gap-fill. |
