...
| <a id="Q289"></a>Q289 | ValidatingAdmissionPolicy backstop for the PriorityClass allowlist (G.7) | `security` | 🔲 | S | Webhook gate shipped: podTemplate can no longer name a preempting class. Residual per G.7 — direct `runnergroups` RBAC bypasses the webhook, and stored objects are never re-validated. Fix = a VAP with a paramKind allowlist. |
| <a id="Q284"></a>Q284 | Expand PodScheduling to the full desirable scheduling surface (EgressProxy + ActionsGateway) | `infra` | 🔲 | S | Add topologySpreadConstraints + priorityClassName (evicted proxy = tenant egress down). Gate the latter on a SEPARATE infra allowlist, not the worker one ([Q289](#Q289)) — else a tenant lifts its workers to proxy priority. Additive. |
