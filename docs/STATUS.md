...
| <a id="Q359"></a>Q359 | [Worker right-sizing profiles (recommendations first)](plan/runner-sizing-profiles.md) | `infra` | 🔲 | L | Killer-feature gap vs ARC. Phases 1–2 shipped (usage metrics + recipe; status recommendations + SizingDrift). Next: Phase 3 opt-in profiles at pod-build time. Live validation of both phases rides the next dogfood session; see the plan. |
