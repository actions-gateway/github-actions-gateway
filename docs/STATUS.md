...
| <a id="Q55"></a>Q55 | Verify provisioner-test goleak cascade fix held in CI | `tests` `bug` | 🔲 | S | Intermittent CI failure on GitHub-hosted runners: `Eventually-on-Secret then findPod-once` race in 2 provisioner tests leaked a goroutine, cascading into ~20 goleak failures. Fixed by `waitForPodCreated` helper in 59c0714. If `internal/provisioner` flakes recur, migrate the remaining ~18 Eventually-on-Pod call sites to the same helper as defense-in-depth. |
