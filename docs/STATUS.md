...
| <a id="Q224"></a>Q224 | [GKE dogfood: route production CI (green CI blocked)](plan/gke-dogfood.md) | `milestone` `infra` | 🔲 | M | Jobs green individually on gag-ci; concurrent matrix still wedges. Duplicate job-acquisition ([Q260](#Q260)) fix is code-complete — awaits a dogfood re-route to reconfirm green before this closes. Not capacity ([Q248](#Q248)). |
| <a id="Q260"></a>Q260 | [AGC duplicate job-acquisition wedges concurrent burst](plan/gke-dogfood.md) | `infra` | ▶ | M | Fix code-complete: per-group job-claim dedup before AcquireJob — siblings handed the same delivery skip the acquire, keeping the slot (metric `jobs_duplicate_delivery_total`). Awaits dogfood re-route to reconfirm Q224. Distinct from Q259 recycle. |
