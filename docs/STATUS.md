...
| <a id="Q224"></a>Q224 | [GKE dogfood: route production CI (green CI blocked)](plan/gke-dogfood.md) | `milestone` `infra` | 🔲 | M | Concurrent matrix STILL wedges: [Q260](#Q260) fix (c850764) live-validated ineffective 2026-07-03 — siblings acquire one job via distinct RunnerRequestIDs, collide on the planID Secret; pool collapsed 8→2. Not capacity ([Q248](#Q248)). |
| <a id="Q260"></a>Q260 | [AGC duplicate job-acquisition wedges concurrent burst](plan/gke-dogfood.md) | `infra` | 🔲 | M | First fix (c850764, #503) live-validated INEFFECTIVE 2026-07-03: dedup keys per-delivery RunnerRequestID, but siblings get distinct IDs for one job → all collide on the planID Secret. Re-key dedup on planID (post-acquire). Blocks [Q224](#Q224). |
