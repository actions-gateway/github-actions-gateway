...
| <a id="Q281"></a>Q281 | [Bump dogfood GAG_IMAGE_TAG to a post-Q74 release to activate CR conversion](plan/gke-dogfood.md) | `infra` | 🔲 | S | Dogfood GMC pins rc.6, predating Q74's conversion webhook (#557), so CR conversion 404s and Q279's caBundle fix stays dormant. Bump GAG_IMAGE_TAG (setup.sh) to the first post-Q74 release, re-run setup, confirm a v2 CR round-trips through /convert. |
...
| <a id="Q280"></a>Q280 | [Finish service-mesh validation: ambient/Cilium + Linkerd egress](operations/service-mesh-coexistence.md) | `tests` `docs` | 🔲 | S | Residual from Q220 (Istio+Linkerd sidecar validated; guide corrected). Exercise Istio ambient + Cilium end-to-end (untested); confirm Linkerd skip-outbound-ports restores GAG egress — its sidecar 504'd the AGC token fetch even with prereqs met. |
---
