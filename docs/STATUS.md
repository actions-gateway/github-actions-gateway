...
|---|---|---|---|---|---|
| <a id="Q148"></a>Q148 | Flake: e2e E2E_AGC_SingleUseSelfHeal — 240s timeout (job 2 worker pod never provisioned) | `tests` `bug` `infra` | 🔲 | M | Webhook-timeout NOT the cause (1×, other tenant, post-failure). Real fail: job 2 worker pod never provisioned (run 27693280595). Cause unconfirmed — Q87 demoted AGC session/job logs to debug. Fix: e2e logLevel=debug. OPEN pending e2e. |
