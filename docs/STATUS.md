...
| <a id="Q59"></a>Q59 | IPRangeReconciler stalls on first `api.github.com/meta` fetch → `ProxyConnectWorks` e2e flake | `tests` `bug` `infra` | 🔲 | S | Confirmed 2026-06-01 in PR #106 run 26796252748 (NP `ipBlock` peers empty >3 min), matching 9fe0718's hypothesis. Likely fix: IPRangeReconciler retries Fetch with backoff before declaring ready. |
