...
|---|---|---|---|---|---|
| <a id="Q95"></a>Q95 | Worker pod lifecycle: completed + stuck pods are never cleaned up | `bug` `1.0-gate` | 🔲 | M | No OwnerReference/deadline on worker pods; cleanup deletes only the Secret (provisioner.go:380,703). Completed pods accumulate (02/04 docs wrongly claim GC); stuck-Pending pods hold ceiling slots forever. Add ownerref+TTL + deadline + Tier-A test. |
| <a id="Q96"></a>Q96 | GMC's own image is fail-open on digest pinning (CI-blind) | `security` `infra` `1.0-gate` | 🔲 | S | validateImageDigest covers only AGC/PROXY images (gmc/cmd/main.go:252), not GMC's own; chart defaults gmc.image to :latest when digest empty, and the polaris gate always renders with a digest so CI can't catch it. Secure-by-default fail-open. |
| <a id="Q97"></a>Q97 | Published images are amd64-only (no multi-arch/arm64) | `infra` `1.0-gate` | 🔲 | M | publish.yml has no platforms: key; worker Dockerfile hardcodes GOARCH=amd64; no TARGETARCH plumbing. All images (incl. injected agc/proxy) are amd64-only, unschedulable on arm64 nodes. Publish multi-arch or document the amd64-only limitation. |
| <a id="Q98"></a>Q98 | No Helm chart distribution/publishing pipeline | `infra` `1.0-gate` | 🔲 | M | 1.0 bar is "operator can helm install the chart" but nothing publishes one: no OCI chart push, no chart repo/index.yaml, no chart signing; Chart.yaml still prerelease:true. publish.yml ships only the 4 images. Add helm package+push. |
