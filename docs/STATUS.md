...
|---|---|---|---|---|---|
| <a id="Q234"></a>Q234 | [rc.3 job execution fails: RenewJob 401](plan/gke-dogfood.md) | `milestone` `bug` `infra` | 🔲 | M | Worker pod runs to Completed but AGC RenewJob→401 "Not authorized for this job"; job hangs, recycle loops 422. App perms OK → job-token/run_service_url auth, not perms. Root-cause broker RenewJob path. From Q224; see runbook B8. |
| <a id="Q224"></a>Q224 | [GKE dogfood: deploy + live GAG validation](plan/gke-dogfood.md) | `milestone` `infra` | 🚫 | M | Validated live on rc.3: GMC+AGC roll, gateway Ready, App-Secret creds, Q229 token fetch, listeners register, worker pod runs to Completed. 🚫 Q234: job won't finish (RenewJob 401). C2/D after Q234; e2e is Q231. |
