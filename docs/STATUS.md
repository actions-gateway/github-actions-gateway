...
|---|---|---|---|---|---|
| <a id="Q235"></a>Q235 | [Default worker image is non-functional (bare upstream)](operations/tenant-onboarding.md) | `milestone` `bug` `infra` | 🔲 | M | Bare-upstream names.DefaultWorkerImage lacks GAG's wrapper entrypoint, so default installs silently no-op jobs (worker exits empty, RenewJob 401). Chart has no worker slot. Fix: wire WORKER_IMAGE to the signed first-party worker digest. From Q224. |
| <a id="Q224"></a>Q224 | [GKE dogfood: route production CI (validation done)](plan/gke-dogfood.md) | `milestone` `infra` | 🔲 | M | Validated end-to-end on rc.3: control plane, App creds, Q229 DNS, registration, real job→worker pod→GitHub success (worker image MUST be first-party wrapper). Remaining: implement Part C2 GAG_RUNNER routing + turn on production CI. See Q235. |
| <a id="Q236"></a>Q236 | [Dogfood scale-to-0 hangs on the GMC PodDisruptionBudget](plan/gke-dogfood.md) | `infra` `bug` | 🔲 | S | dogfood-stop "resize pool to 0" never drains: GMC PDB (minAvailable:1, replicaCount:1) blocks evicting the only replica → node lingers + keeps billing (~50min seen). Fix: podDisruptionBudget.enabled=false in values-dogfood.yaml + dogfood-setup.sh. |
...
| <a id="Q233"></a>Q233 | [AGC: inject DefaultWorkerImage when template names runner container](../cmd/agc/internal/provisioner/provisioner.go) | `bug` `infra` | 🔲 | S | AGC sets DefaultWorkerImage only when it *creates* the runner container, not when a RunnerTemplate names one image-less → pod invalid (image required). Fix: default when found container image empty. Workaround: set image explicitly. From Q224. |
...
| <a id="Q237"></a>Q237 | [Docs quality audit: score docs/ against the six-goal rubric](development/documentation-standards.md#goals-what-good-looks-like) | `docs` | 🔲 | M | Score docs/ against the six-goal rubric in [doc standards](development/documentation-standards.md) — correct, findable, complete, fit-for-purpose, usable, trustworthy (incl. AI-slop check), not just scannability. Plan doc + ranked findings. |
...
|---|---|---|---|---|
| <a id="Q238"></a>Q238 | [Versioned docs tree (per-release docs)](plan/docs-six-layer-audit.md) | `docs` | M | **Event:** 2+ supported releases ship with materially divergent docs. Today docs describe `main` with `(planned)` markers, single source of truth on GitHub. Then adopt a versioned-docs mechanism (site generator / versioned tree). Rationale in the six-layer audit. |
