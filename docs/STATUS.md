...
| <a id="Q240"></a>Q240 | [Worker wrapper exits 100 on success → pod phase Failed](../cmd/worker/main.go) | `bug` `infra` | 🔲 | S | Runner.Worker returns a 100-offset code (Succeeded=100); the wrapper propagates it verbatim, so successful jobs exit 100 -> pod Failed. Benign: AGC ignores exit code, no metric/retry. Fix: translate it, exit 0 on success so pod ends Succeeded. |
