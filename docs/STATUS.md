...
|---|---|---|---|---|---|
| <a id="Q261"></a>Q261 | [AGC crash-loops in a v1-only install (v2 reconciler ungated)](../cmd/agc/main.go) | `bug` | 🔲 | S | AGC crash-loops in v1-only installs: `cmd/agc/main.go` registers the v2 RunnerSet reconciler unconditionally → with v2alpha1 CRDs absent, cache never syncs, `mgr.Start` exits(1) ~2m. Gate on CRD presence like the GMC. Masked by e2e/dogfood v2 CRDs. |
| <a id="Q269"></a>Q269 | [WORKER_MODE=scaleset leaks into cmd/worker unit tests](../cmd/worker/worker_test.go) | `bug` `tests` | 🔲 | S | ScaleSet worker sets WORKER_MODE=scaleset on the runner container; dogfooding GAG's own CI there, the job's `go test` inherits it → cmd/worker tests fail on the jitconfig branch (Q264 P4: unit-test+coverage). Pin WORKER_MODE. Blocks [Q224](#Q224). |
| <a id="Q267"></a>Q267 | [Broker-credential recycle seam: post-recycle token-exchange 400 exits the listener](../cmd/agc/internal/listener/goroutine.go) | `bug` `infra` | ✅ | M | Live-confirmed #8: at maxListeners 48 (collapsed to online 0 in #7) the pool HOLDS — 0 token-400, 0 fatal deregister, 0 collapse. Token-400 didn't arise live (Q266 loser-park + clean ns); retry path covered by offline repro #528. |
| <a id="Q266"></a>Q266 | [Fan-out loser slot-stranding collapses the pool under sustained burst](../cmd/agc/internal/listener/goroutine.go) | `bug` `infra` | ✅ | M | Fix live-confirmed (#7): loser-slot-stranding seam GONE — 0 fatal deregister exits (Q265: 41/38), losers park, Option A+dedup fire. Residual collapse = SEPARATE online-session/broker-credential recycle seam → [Q267](#Q267). |
| <a id="Q224"></a>Q224 | [GKE dogfood: route production CI green](plan/gke-dogfood.md) | `milestone` `infra` | 🔲 | M | ScaleSet P4 (2026-07-05): fan-out starvation ELIMINATED by construction — all 7 assigned+ran+concluded (vs classic 2/7), 0 dedup/collision. Clean-green blocked by [Q269](#Q269) WORKER_MODE leak + capacity. Do not close. |
| <a id="Q264"></a>Q264 | [Migrate AGC acquisition to the runner-scale-set protocol](plan/q264-scale-set-protocol.md) | `infra` | ▶ | L | P4 RAN (2026-07-05): fan-out ELIMINATED by construction (7/7 vs classic 2/7); U3 pod-core settled (real run.sh --jitconfig worker). Clean-green blocked by [Q269](#Q269) + capacity; P5 (default flip/retire classic) gated on it. §6-P4. |
| <a id="Q271"></a>Q271 | [Dogfood CI: opt-in GAG routing](plan/gke-dogfood.md) | `infra` | ✅ | S | DONE: unit-test/integration-test jobs → GAG only on `workflow_dispatch`+`target_gag=true`; push/PR/Dependabot stay ubuntu-latest. Scopes contention vs a global GAG_RUNNER flip ([Q264](#Q264) P4, PR #541). Kept GAG_RUNNER for [Q224](#Q224). |
...
| <a id="Q248"></a>Q248 | [Right-size GAG dogfood CI runner pods + node pool](plan/dogfood-runner-rightsizing.md) | `infra` | 🔲 | M | Disk ceiling RESOLVED (#7): worker disk pd-balanced→pd-standard, off the 500GB SSD quota → maxWorkers CPU-bound not SSD-bound, NO quota bump (live DISKS=400, SSD=220/500). Persisted setup.sh maxWorkers 8. Pod-request refinement open. |
| <a id="Q265"></a>Q265 | [Benchmark Option A fan-out throughput vs scale-set](plan/q260-fanout-completion-reconciliation.md) | `infra` `speed` `tests` | 🔲 | M | Benchmark DONE (#6): NO completion-tax wall → Q264 deferred; collapse = loser slot-stranding ([Q266](#Q266)). #7: SSD cap gone (pd-standard, [Q248](#Q248)), Q266 seam fixed live; clean holds-at-maxWorkers gated on the online-session seam. |
| <a id="Q270"></a>Q270 | [ScaleSet listener: generatejitconfig 409 mislabel + no-backoff cursor wedge](../scaleset/client.go) | `bug` `infra` | 🔲 | S | Q264 P4 minor: `scaleset.statusError` maps every 409→SessionConflictError (mislabels a generatejitconfig runner-name conflict); listener retries generatejitconfig with no backoff on a persistent 409 → replay loop wedges the batch. |
| <a id="Q242"></a>Q242 | [Implement G.1 proxy destination allowlist](plan/q242-g1-proxy-destination-allowlist.md) | `security` `infra` | ▶ | L | Impl merged #460–#464. Concurrent-green via ScaleSet: fan-out class ELIMINATED ([Q264](#Q264) P4 2026-07-05, 7/7 assigned+ran), clean-green pending [Q269](#Q269) fix + re-run. FQDN [Q245](#Q245). v2beta1 blocker. |
...
| <a id="Q245"></a>Q245 | [FQDN egress: split intent from CNI backend + GKE backend](plan/q245-fqdn-intent-backend-split.md) | `security` `infra` | 🔲 | L | Design landed: intent CIDR\|FQDN + operator --fqdn-policy-backend (none\|cilium\|calico\|gke); gke=networking.gke.io FQDNNetworkPolicy (union, NOT default-deny→base NP must persist). v2alpha1-free reshape; Q74 inherits. Impl+live GKE deferred. |
...
| <a id="Q269"></a>Q269 | [Adoption signal to make the "v2 adoption sufficient" gate measurable](plan/v1-classic-sunset-review.md) | `docs` `infra` | S | **Decision:** We move to plan the v1alpha1-API removal (or the classic-protocol removal) and need the "sufficient v2 adoption" gate to be *measured*, not guessed. Record/instrument a concrete known-tenants / still-dogfood-only signal (ties to GTM Phase 1). Gates the removal decisions in the [sunset review](plan/v1-classic-sunset-review.md) §6.4. |
| <a id="Q270"></a>Q270 | [Scale-set upstream maturity watch (gates classic-machinery removal)](plan/v1-classic-sunset-review.md) | `infra` | S | **Event:** `actions/scaleset` reaches GA/v1.0 **or** the auto-assign contract (upstream issue #107) gets documented/answered. Until then classic stays the fallback and classic-machinery removal ([Q264](#Q264) P5 / v2beta1 graduation) holds; also the trigger to revisit the U6 vendor-vs-own client decision. Per the [sunset review](plan/v1-classic-sunset-review.md) §6.4. |
