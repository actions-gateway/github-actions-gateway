...
| <a id="Q247"></a>Q247 | [AGC runner session recovery after job recycling](plan/gke-dogfood.md) | `infra` | ▶ | M | Root cause: RenewJob used broker MessageID as jobId, not RunnerRequestID, so locks never renewed — long jobs recycled (dup pods, orphan at CompleteJob). One-line fix + regression test landed; live GAG e2e pending. Blocks [Q224](#Q224). |
