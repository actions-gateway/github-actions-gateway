...
|---|---|---|---|---|---|
| 5i | Fix `E2E_GMC_PDBPreventsEvictionBelowMinAvailable` NotFound flake | `tests` `bug` | ▶ | S | Recurring e2e flake (last seen on [PR #64 job 78408530495](https://github.com/actions-gateway/github-actions-gateway/actions/runs/26608341871/job/78408530495)). The preceding `HPADrivesScaleUp` test patches HPA `minReplicas: 2 → 1` in `DeferCleanup`, triggering an async HPA-driven scale-down. The PDB test then fetches a pod name and ~50ms later calls the eviction API; the pod has been terminated by then, returning `Error from server (NotFound)` instead of the expected `429 TooManyRequests`. A previous fix (commit `a86b262`, 2026-05-25) added `WaitForDeploymentReady` before the pod-name fetch, but that helper only checks `readyReplicas != 0` and does not block on terminating pods. Fix: wrap the `{fetch pod name, issue eviction}` pair in `Eventually`, retrying any non-`TooManyRequests` outcome (including transient `NotFound`) until either the PDB rejection or a successful eviction lands. |
