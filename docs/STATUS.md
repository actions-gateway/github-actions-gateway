...
| <a id="Q224"></a>Q224 | [GKE dogfood: route production CI (green CI blocked)](plan/gke-dogfood.md) | `milestone` `infra` | 🔲 | M | Re-route #4 (2026-07-04): capacity + collisions FIXED (3 non-preempted nodes, 0 collisions). Still NOT green — blocker shifted to GitHub fan-out completion/assignment accounting (beyond [Q260](#Q260)); #513 completejob flag doesn't help (keep off). |
| <a id="Q260"></a>Q260 | [AGC fan-out completion accounting wedges concurrent burst](plan/q260-fanout-completion-reconciliation.md) | `infra` | ▶ | M | planID dedup + #512 linger EFFECTIVE (0 collisions). Blocker=GitHub fan-out completion accounting; design + fails-today offline repro landed; fix (winner-fan-out-completion) gated on live re-route #5. Keep #513 flag OFF. Blocks [Q224](#Q224). |
...
| <a id="Q242"></a>Q242 | [Implement G.1 proxy destination allowlist](plan/q242-g1-proxy-destination-allowlist.md) | `security` `infra` | ▶ | L | Impl merged #460–#464. Individually green; concurrent green BLOCKED — re-route #4 fixed capacity+collisions but GitHub fan-out accounting ([Q260](#Q260)) still blocks. Deliverable 7 = [Q224](#Q224); FQDN = [Q245](#Q245). v2beta1 blocker. |
...
| <a id="Q264"></a>Q264 | [Migrate AGC acquisition to the runner-scale-set protocol](plan/q260-fanout-completion-reconciliation.md) | `infra` | 🔲 | L | Option E in the Q260 design doc: one scale-set listener + batch acquireJobs replaces N per-runner sessions → kills the fan-out race class by construction. Pursue if [Q260](#Q260) rules Option A infeasible; retires the virtual-runner model. |
---
