...
|---|---|---|---|---|---|
| <a id="Q388"></a>Q388 | [Verify endpoint removal under graceful node shutdown](operations/node-shutdown-budgets.md#upstream-state-of-the-art) | `bug` `infra` | 🔲 | S | kubernetes#116965 claims endpoints aren't removed until a pod fully stops on this path — if so the linger never quiesces and burns a 15s GKE Spot budget. Our `/readyz` failure is a different removal path, so may not apply. Reproduce on kind. |
