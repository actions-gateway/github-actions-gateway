...
|---|---|---|---|---|---|
| <a id="Q154"></a>Q154 | [Verify ceiling-held acquired job is cancelled vs redelivered (Q59 assumption)](plan/acquire-admission-control.md) | `infra` | 🔲 | M | Tier-A e2e: confirm a ceiling-held, already-acquired job is cancelled (not redelivered) by GitHub — the assumption Q59's pre-acquisition gate is built on (shipped unit-tested, source-read only). Also exercise redelivery-after-skip. |
| <a id="Q156"></a>Q156 | Surface provisioning + credential failures as conditions; promote condition types to API consts | `infra` | 🔲 | M | On a provisioning error Reconcile returns before updateStatus, so conditions go stale. Add Degraded(step) on ActionsGateway + token CredentialUnavailable on RunnerGroup (only an Event today). Make condition types/reasons api consts. |
...
| <a id="Q158"></a>Q158 | Aggregate child RunnerGroup health into ActionsGateway status (rollup condition) | `infra` | 🔲 | S | GMC doesn't watch RunnerGroups, so child CredentialUnavailable/Degraded (Q156) is invisible on the ActionsGateway single pane. Add Watches(RunnerGroup)→owner + an aggregate rollup condition; fold in Q157's conditions. Watch-driven, no polling. |
