...
| <a id="Q224"></a>Q224 | [GKE dogfood: route production CI green](plan/gke-dogfood.md) | `milestone` `infra` | 🔲 | M | Re-route #8: [Q267](#Q267) confirmed. NOT green (2/7): GitHub fan-out distinct-delivery starvation. AGC-lever spike ([plan](plan/q224-fanout-dispatch-lever-spike.md)): NO reliable lever → [Q264](#Q264)/Option E only structural fix. Do not close. |
...
| <a id="Q242"></a>Q242 | [Implement G.1 proxy destination allowlist](plan/q242-g1-proxy-destination-allowlist.md) | `security` `infra` | ▶ | L | Impl merged #460–#464. Concurrent-green NOT met (#8): [Q267](#Q267)/[Q266](#Q266)/[Q248](#Q248) all resolved, full matrix blocked by GitHub fan-out distinct-delivery starvation ([Q224](#Q224)/[Q264](#Q264)). FQDN [Q245](#Q245). v2beta1 blocker. |
...
|---|---|---|---|---|
| <a id="Q264"></a>Q264 | [Migrate AGC acquisition to the runner-scale-set protocol](plan/q264-scale-set-protocol.md) | `infra` | L | **Decision:** Option E — spike + probes DONE, **viable** (#520; all protocol unknowns settled, plan §5a). Re-route #5 live-confirmed Option A reconciles the fan-out AGC-side. **[Q265](#Q265) benchmark (re-route #6, 2026-07-05): NO completion-tax wall** — Option A's `completejob` tax is never the binding constraint (0 worker-capacity rejections at maxListeners 48 & 16), so Option E is **NOT** triggered. The sustained-throughput collapse is a fan-out loser slot-stranding recycle seam ([Q266](#Q266)), fixable AGC-side. Revive only if a clean re-benchmark (post-Q266, wider maxWorkers via SSD-quota bump) shows a real tax/topology wall, or for per-group acquire SPOF / a future GitHub fan-out change. **Re-route #8 (2026-07-05): the fan-out distinct-delivery starvation IS a demonstrated topology wall** (distinct from the ruled-out tax wall) — and the **AGC-lever spike** ([q224-fanout-dispatch-lever-spike.md](plan/q224-fanout-dispatch-lever-spike.md)) found **no reliable AGC-side escape hatch** (unique names = non-lever; warm-listener baseline = probabilistic stopgap only). This **strengthens** Option E as the only structural fix — still a deferred v-next go/no-go, not force-triggered. |
