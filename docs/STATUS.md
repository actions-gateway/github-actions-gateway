...
|---|---|---|---|---|---|
| <a id="Q155"></a>Q155 | Deflake cross-tenant NetworkPolicy isolation e2e | `tests` `bug` | 🔲 | S | isolation_test.go:140 asserts a single cross-tenant curl pod ends Failed without confirming NetworkPolicy is live in the kindnet dataplane; latency lets the lone connect succeed → flake. Default CNI only (calico ok). Gate enforcement before curl. |
