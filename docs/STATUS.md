...
| <a id="Q283"></a>Q283 | EgressProxy reconciler fights its own HPA (proxy pool cannot scale out) | `infra` `security` | 🔲 | S | Live-confirmed in envtest: the reconciler resets proxy Deployment .spec.replicas to minReplicas every pass (obj.Spec = desired.Spec + Owns(Deployment)), reverting an HPA scale-out 5→2 in <1s. The proxy pool cannot stay scaled out. Pre-existing. |
