...
|---|---|---|---|---|---|
| 5b | Add port 6443 to AGC NP apiserver egress rule (post-DNAT fix) | `bug` `infra` | 🔲 | S | Root-cause diagnosis in [plan/5b-root-cause.md](plan/5b-root-cause.md): the "NP additivity broken under kindnet" hypothesis was wrong. `kube-network-policies` evaluates packets *after* kube-proxy's DNAT, so in kind a connection to `kubernetes.default.svc` arrives at the policy eval as `<node-ip>:6443`, not `10.96.0.1:443`. The AGC NP's `port: 443` rule never matches → drop. Same DNAT trap as the proxy `ipBlock`→`PodSelector` fix in PR #59, just on the port axis. **Fix:** in `buildAGCNetworkPolicy` add a `port: 6443` egress rule alongside `443` (Option A in the writeup), and add a regression unit test asserting both ports appear. Independent of [PR #62](https://github.com/actions-gateway/github-actions-gateway/pull/62) — the AGC NP is still broken in kind with or without PR #62 until this port rule lands. Also extend the "NetworkPolicy enforces after kube-proxy DNAT" section in `docs/development/kind-iteration.md` with the port-mismatch variant. |
...
| 6 | [M3/M4 kind end-to-end validation](plan/milestone-3.md) | `milestone` | 🚫 | M | → 5b. With #59 fixes and JIT config plumbing (5a, 2026-05-27) shipped, the test reaches worker-pod creation, Runner.Worker parses the job, and the runner config files are in place; remaining blocker is NP additivity (5b). |
