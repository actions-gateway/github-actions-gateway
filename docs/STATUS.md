...
| <a id="Q181"></a>Q181 | [Validate the ~1000-session ceiling at scale](design/appendix-a-capacity-slos.md) | `tests` `infra` | 🔲 | L | Headline ~1000-session memory claim is a single-AGC-process property: kind-validatable vs a fake broker (RSS/heap/GC at 1000 long-poll goroutines), no big cluster/real GitHub. 1000 running worker pods is a separate real-cluster test. |
