...
Last refreshed: 2026-05-29 (5f ✅ — TLS-pool construction extracted from `cmd/agc/main.go` into `cmd/agc/internal/transport.BuildProxyTrustPool(certPEM []byte) (*x509.CertPool, error)`. New `proxytrust_test.go` covers all four cases from the Queue item: (a) nil/empty PEM returns `(nil, nil)` for the no-TLS-proxy fall-through, (b) a leaf signed by the supplied proxy CA verifies, (c) a leaf signed by an unrelated CA is rejected, and (d) unparseable PEM returns an error (rather than a system-roots-only pool that would over-trust). A fifth test asserts the combined pool's subject count is exactly `system + 1`, catching the exact PR #59 regression of replacing rather than appending; it skips on macOS where `SystemCertPool().Subjects()` is opaque and runs on Linux CI. Earlier landmarks: 5e IP fetcher merge regression test ✅, 5d ALPN HTTP/1.1-only test ✅, 5c Tier-A `ProxyConnectWorks` ✅, named-pipe ✅, GithubRegistrar ✅, eviction retry CRD fields ✅, M2 envtest goroutine-leak suite ✅, credential rotation ✅, M3 metric assertions ✅, M4 test gaps ✅, open docs items ✅, AGC rename ✅, go-workspace prefix-match ✅, Make UX Phase 2 ✅, e2e test speed ✅, envtest/unit test split ✅, M2 kind activeSessions check ✅, ARC alignment ✅, JIT config plumbing for worker ✅. PR #59 fixes shipped: workload NP `ipBlock` → `podSelector`, proxy HTTP/2 disable, IP-range fetcher `actions`→`api+actions+web`, AGC TLS pool replace → append, wrapper `--startuptype workerprocess` → `spawnclient`.
