...
| <a id="Q266"></a>Q266 | [Fan-out loser slot-stranding collapses the pool under sustained burst](../cmd/agc/internal/agentpool/pool.go) | `bug` `infra` | 🔲 | M | Fan-out slot-stranding collapses the pool under burst (Q265 re-route #6): a deduped loser's slot stays 422-blocked for the winner's runtime, past the bounded recycle backoff → listener exit. Fix: defer loser recycle until its winner completes. |
| <a id="Q224"></a>Q224 | [GKE dogfood: route production CI green](plan/gke-dogfood.md) | `milestone` `infra` | 🔲 | M | Re-route #5: Q260 accounting RESOLVED. Q265 re-route #6: NO completion-tax wall (tax never binding). Full-matrix throughput now gated on the fan-out loser-slot-stranding recycle seam (Q266) + worker capacity (Q248, SSD-bound). |
...
| <a id="Q248"></a>Q248 | [Right-size GAG dogfood CI runner pods + node pool](plan/dogfood-runner-rightsizing.md) | `infra` | 🔲 | M | Capacity unblock DONE (re-route #4: workers-od ×3, peak 34% CPU, ~3.8GiB/pod, 4 pods/3 nodes). Q265 re-route #6: maxWorkers≈4 is SSD-bound (500GB SSD quota), NOT CPU/mem — raising it needs SSD-quota bump. Broader right-sizing open. |
| <a id="Q265"></a>Q265 | [Benchmark Option A fan-out throughput vs scale-set](plan/q260-fanout-completion-reconciliation.md) | `infra` `speed` `tests` | 🔲 | M | Benchmark DONE (re-route #6): NO completion-tax wall — tax never the binding constraint. Collapse cause = fan-out loser slot-stranding recycle seam (Q266), not the tax → Q264 stays deferred. Bounded: SSD caps maxWorkers≈4 + record clutter. |
| <a id="Q242"></a>Q242 | [Implement G.1 proxy destination allowlist](plan/q242-g1-proxy-destination-allowlist.md) | `security` `infra` | ▶ | L | Impl merged #460–#464. Individually green; concurrent-green accounting UNBLOCKED (Q260 Option A live-confirmed re-route #5); residual = worker capacity ([Q248](#Q248)). Deliverable 7 = [Q224](#Q224); FQDN = [Q245](#Q245). v2beta1 blocker. |
