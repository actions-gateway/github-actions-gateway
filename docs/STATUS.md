...
|---|---|---|---|---|---|
| <a id="Q53"></a>Q53 | Drop redundant `go vet` step from lint CI job | `speed` `infra` | 🔲 | S | The standalone `go vet` step in `.github/workflows/unit-test.yml` loops over all 9 modules (~2m53s on PR #91) but `govet` is already enabled in `.golangci.yml` and runs as part of the golangci-lint step. Remove the step — no coverage loss. |
