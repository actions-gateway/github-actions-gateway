...
|---|---|---|---|---|---|
| <a id="Q235"></a>Q235 | [Default worker image is non-functional (bare upstream)](operations/tenant-onboarding.md) | `milestone` `bug` `infra` | 🔲 | M | Bare-upstream names.DefaultWorkerImage can't run jobs (no wrapper entrypoint). Fix: wire WORKER_IMAGE to the signed worker digest, or inject the wrapper at runtime so any ARC custom image runs unmodified (parity). From Q224. |
