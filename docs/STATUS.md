...
| <a id="Q63"></a>Q63 | Frame priority tiers offensively: oversubscription → utilization → cost | `docs` | 🔲 | S | Priority tiers are framed defensively (no starvation) but not offensively — the floor enables quota oversubscription → higher utilization → lower cost. Add to exec summary + [cost model](design/appendix-f-cost-model.md). |
