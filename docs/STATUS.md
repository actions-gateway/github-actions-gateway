...
| <a id="Q201"></a>Q201 | [Workload-identity AGC provisioning + Vault kind e2e](design/05-security.md#57-workload-identity-the-no-pem-delegation-model) | `security` `infra` | 🔲 | M | Q197 follow-up: wire GMC provisioning of the workload-identity AGC (signer env, projected SA token, Vault role binding) + AGC main's vaultsigner provider; in-cluster Vault kind e2e covers the no-PEM round-trip. Such gateways fail closed until done. |
