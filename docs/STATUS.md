...
| <a id="Q278"></a>Q278 | [Run the pre-GA dogfood validation gate before the v1.1.0 GA tag](operations/release.md#validate-the-release-candidate-on-dogfood) | `infra` `security` `tests` | 🔲 | M | Deploy the latest RC to dogfood (`GAG_IMAGE_TAG=<rc-tag>`), run the e2e matrix green, smoke the signed v2 CRD asset (verify-blob + apply). Promote to v1.1.0 only if green; a red run is stop-ship. |
