...
| <a id="Q224"></a>Q224 | [GKE dogfood: route production CI (green CI blocked)](plan/gke-dogfood.md) | `milestone` `infra` | 🔲 | M | Concurrent matrix STILL wedges via [Q260](#Q260). Q260 re-fix (planID-keyed dedup) is code-complete — awaits a dogfood re-route to reconfirm green before this closes. Not capacity ([Q248](#Q248)). |
| <a id="Q260"></a>Q260 | [AGC duplicate job-acquisition wedges concurrent burst](plan/gke-dogfood.md) | `infra` | ▶ | M | Re-fix code-complete (this PR): dedup re-keyed on planID, post-acquire — losing siblings skip provisioning + recycle; RunnerRequestID gate removed. Tests fail vs c850764. Awaits dogfood re-route. Blocks [Q224](#Q224). |
