...
|---|---|---|---|---|---|
| <a id="Q148"></a>Q148 | Flake: e2e E2E_AGC_SingleUseSelfHeal — 240s timeout; GMC validating webhook context-deadline-exceeded | `tests` `bug` `infra` | 🔲 | M | **[Flakes-first](development/maintaining-backlog.md#flake-fixes-go-first) top of queue.** E2E_AGC_SingleUseSelfHeal timed out 240s on main 2026-06-17 (run 27693280595); GMC validating webhook hit context-deadline-exceeded under load. |
| <a id="Q139"></a>Q139 | Flake: e2e E2E_GMC_TenantProvisioning_ProxyConnectWorks — curl through proxy gets HTTP 504 | `tests` `bug` `infra` | 🔲 | M | curl CONNECT through per-tenant proxy got HTTP 504; recurred on main 2026-06-17 (run 27661005022), earlier on PR 231. Proxy tunnel timeout under CI load. Top of queue (flakes-first). |
