...
| <a id="Q243"></a>Q243 | [Per-tenant egress-IP reference architecture (cloud)](plan/q243-egress-ip-reference-arch.md) | `security` `infra` `docs` | 🔲 | L | Reference arch done. Live validation 2026-07-07: per-range Cloud NAT gives 2 tenants distinct+stable IPs (proven). OPEN (v2beta1 gate): EgressProxy has no scheduling controls; anti-affinity spreads pods across NATs → no per-tenant egress IP. |
| <a id="Q282"></a>Q282 | [Pod scheduling pass-through on EgressProxy + ActionsGateway CRs](plan/q243-egress-ip-reference-arch.md) | `security` `infra` | 🔲 | M | EgressProxy + ActionsGateway CRs pass no scheduling controls to their pods (workers do). Add nodeSelector/tolerations/affinity → unblocks Q243 egress-IP pinning. Secure-by-default: govern placement so tenants cannot self-select egress IPs. |
