...
|---|---|---|---|---|---|
| <a id="Q258"></a>Q258 | [probe TestRunProbe_HappyPath -race flake](../cmd/probe/main_test.go) | `tests` `flake` | ▶ | S | Test cancels ctx when server AcquireJobCalls()==1, but the stub bumps that counter before writing the response → cancel races the in-flight AcquireJob POST under -race+CI load (#496). Fix (this PR): wait for the post-acquire acknowledge call. |
| <a id="Q257"></a>Q257 | [proxy_test bind-wait 2s ceiling flakes under CI starvation](../cmd/proxy/proxy_test.go) | `tests` `flake` | ▶ | S | `startServer` + `TestServer_ListenAndServeShutdown` waited only 2s for `srv.ready` (3 TCP binds); CI CPU-starvation blew past it → spurious "did not bind within 2s" (PR #494, run 28507868290). Raise ceiling 2s→10s; slowloris assertion unaffected. |
...
| <a id="Q224"></a>Q224 | [GKE dogfood: route production CI (green CI blocked)](plan/gke-dogfood.md) | `milestone` `infra` | 🔲 | M | planID dedup effective 2026-07-04. Full green still blocked: spot preemption ([Q248](#Q248)) → 600s/15-min timeouts. The late-redelivery pod-collision ([Q260](#Q260)) is now code-complete; awaits combined capacity + re-route #4. |
| <a id="Q260"></a>Q260 | [AGC duplicate job-acquisition wedges concurrent burst](plan/gke-dogfood.md) | `infra` | ▶ | M | planID dedup EFFECTIVE 2026-07-04. Pod-collision residual code-complete (#512, claim lingers till pod GC). Accounting residual: completejob-abandon mechanism landed GUARDED (off by default, needs live confirm). Blocks [Q224](#Q224). |
...
| <a id="Q242"></a>Q242 | [Implement G.1 proxy destination allowlist](plan/q242-g1-proxy-destination-allowlist.md) | `security` `infra` | ▶ | L | Impl merged #460–#464. Dogfood jobs green individually; concurrent green blocked — [Q260](#Q260) dedup validated 2026-07-04; capacity + redelivery edge still block green. Deliverable 7 = [Q224](#Q224); FQDN = [Q245](#Q245). v2beta1 blocker. |
...
| <a id="Q262"></a>Q262 | [getting-started onboarding example fails as written](getting-started.md) | `docs` `bug` | 🔲 | S | getting-started.md onboarding fails as written: `runnerGroups[].name` (step 4) rejected by v1alpha1 strict decode; step-2 ResourceQuota rejects the requests-less AGC pod without a LimitRange. Fix example + add a LimitRange. Found via Q193. |
