...
| <a id="Q361"></a>Q361 | [ScaleSet worker leaks a JIT Secret per job](plan/structural-debt-audit-2026-07.md) | `bug` `security` | 🔲 | S | **Bug.** `ProvisionScaleSetWorker` never deletes the `job-ss-` Secret it stages (3 failure exits + success); classic `provision()` cleans all 6. Credential Secrets accrue per job on the default protocol. F1. |
