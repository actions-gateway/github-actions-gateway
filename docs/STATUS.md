...
| <a id="Q247"></a>Q247 | [AGC runner session recovery after job recycling](plan/gke-dogfood.md) | `infra` | ▶ | M | 3 facets, all fixed + tested: (1) RenewJob sent MessageID not RunnerRequestID (#481); (2) unbounded renewal wedges loop (#485); (3) RenewJob used broker token not job-scoped SystemVssConnection token, 401. Live GAG e2e pending. Blocks [Q224](#Q224). |
...
| <a id="Q255"></a>Q255 | [Per-module unit-test coverage → ≥75%](plan/coverage-to-75-per-module.md) | `tests` | 🔲 | L | Bring every module ≥75% hand-written coverage (ratchet floors). Below bar: api 47.6, probe 46.6, gmc 66.3, worker 73.7. One agent Workflow, one PR: author-only agents + single validation run (desktop-safe); `probe.run()` refactor isolated commit. |
