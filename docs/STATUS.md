...
| <a id="Q255"></a>Q255 | [Per-module unit-test coverage → ≥75%](plan/coverage-to-75-per-module.md) | `tests` | 🔲 | L | Bring every module ≥75% hand-written coverage (ratchet floors). Below bar: api 47.6, probe 46.6, gmc 66.3, worker 73.7. Test-only except a `probe.run()` refactor (split into refactor-then-tests PRs); thin entrypoints skipped per audit. |
