...
| M4: GMC + proxy | `milestone` | ✅ | Multi-tenant, delete-isolation, e2e proxy job live-validated 2026-06-12 (helm install + real GitHub); 4 bugs found → [Q114](#Q114)–[Q117](#Q117) — [plan](plan/milestone-4.md) |
...
| <a id="Q115"></a>Q115 | Default worker SecurityContext breaks the actions-runner image | `bug` `security` `1.0-gate` | 🔲 | S | applySecurityDefaults stamps runAsNonRoot:true; actions-runner image user is non-numeric (USER runner) so kubelet rejects every default-path worker pod. Live-found 2026-06-12 (M4 §12). Gap-fill runAsUser 1001; make Tier-B use the real worker image. |
| <a id="Q114"></a>Q114 | AGC self-healing for single-use JIT agents | `bug` `1.0-gate` | 🔲 | L | GitHub deletes a JIT runner after one job; AGC keeps the stale agent/session → GetMessage 200-EOF/401 loops forever, re-register 409s on surviving names. Tenant dies after ~maxListeners jobs. Live-found 2026-06-12 (M4 §12). Re-register post-job. |
...
| <a id="Q116"></a>Q116 | First-class GitHub org URL config for tenants | `infra` `1.0-gate` | 🔲 | M | Production AGCs need GITHUB_ORG_URL but no CR/chart field exists; the M4 live run had to patch the testing-only --allow-agc-extra-env onto the GMC. Add a spec field (CR or RunnerGroup) + onboarding docs. Live-found 2026-06-12 (M4 §12). |
...
| <a id="Q100"></a>Q100 | Multiplexer permanent-baseline goroutines can multiply unbounded | `infra` `bug` | 🔲 | M | A recoverable baseline crash leaves ActiveCount==0 during RestartDelay; a reconcile then re-calls mux.Start → 2nd permanent baseline (multiplexer.go:57,149). Repeated outages ratchet up, leaking sessions past maxListeners. Make Start idempotent. |
...
| <a id="Q13"></a>Q13 | [M5 load test harness](plan/milestone-5.md) | `milestone` `tests` | 🔲 | L | Unblocked 2026-06-12 (Q12 done). **Highest "right thing" risk — pitch is thousands of virtual sessions per AGC and nothing pins that claim.** Throughput also depends on [Q114](#Q114) (single-use JIT agents); the harness must model re-registration. |
...
| <a id="Q118"></a>Q118 | Runner-version pin drift + empty agent.version at session creation | `infra` `bug` | 🔲 | S | Dependabot bumped the worker Dockerfile to 2.335.1 (#197) but DefaultWorkerImage (provisioner.go:98) still pins 2.334.0, violating the lockstep rule; GMC also never sets GITHUB_RUNNER_VERSION, so CreateSession sends an empty agent.version. |
