...
|---|---|---|---|---|---|
| <a id="Q134"></a>Q134 | Flake: e2e "no session registered" 180s timeout (WorkerPodAdmitted / SessionRegistered) | `tests` `bug` `1.0-gate` | 🔲 | S | Fix #231 bounds control-plane broker calls with per-call deadlines. But the 'no session registered' 180s timeout recurred on main 2026-06-15 (run 27529447832, worker_securitycontext_test) — not settled; keep open. |
| <a id="Q135"></a>Q135 | Flake: e2e E2E_AGC_MultipleJobsQueued — 4min timeout waiting for 2nd worker pod | `tests` `bug` | 🔲 | S | job_lifecycle_test.go:179 times out on the 2nd queued job's worker pod (240s). Recurred on main 2026-06-15 (run 27524770550, #234) — still flaking, not settled. Timing-sensitive worker-pod spawn under load; e2e session-recycle class. |
