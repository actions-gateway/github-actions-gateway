...
| <a id="Q278"></a>Q278 | [Validate signed release artifacts post-publish (v2 CRD asset)](operations/release.md) | `security` `infra` `tests` | S | **Decision:** a `v*` release is planned — re-enters the Queue high (release-gate). Post-publish smoke: apply the signed v2 CRD asset, `cosign verify-blob`, assert the five CRDs register. Distinct from dogfood's from-source path (Q277). |
