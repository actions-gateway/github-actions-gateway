...
| Release 1.0 | `milestone` | ⚠️ | Bar = installable + multi-tenant + trustworthy; load test ([Q13](#Q13)) & gVisor ([Q15](#Q15)) deferred post-1.0. Bucket A + Q12 proven 2026-06-12; publish pipeline proven live via v1.0.0-rc.1 2026-06-15 ([Q98](#Q98)); remaining: GA promote — [plan](plan/release-1.0.md) |
...
| <a id="Q98"></a>Q98 | Helm chart distribution/publishing pipeline | `infra` `1.0-gate` | 🔲 | M | RC v1.0.0-rc.1 published; pipeline proven: 4 multi-arch images + chart pushed to ghcr, cosign-signed (SBOM+provenance), prerelease; verified via verify-release + helm pull. Remaining: promote v1.0.0 (flip prerelease, Artifact Hub, docs, site Q129). |
...
| <a id="Q73"></a>Q73 | Sync GMC's bundled RunnerGroup CRD with the AGC source | `infra` `bug` | 🔲 | S | GMC's bundled RunnerGroup CRD (`…runnergroups.yaml`) drifted from AGC source: missing fields + different PodTemplateSpec (k8s.io/api skew), risking silent field pruning on deploy. Add a sync target + drift CI check. Slice A of [Q142](#Q142). |
| <a id="Q142"></a>Q142 | Helm the sole cluster-install path; kill chart/config CRD duplication | `infra` `bug` `tests` | ▶ | L | Umbrella: Helm as sole cluster-install path. A) single-source CRDs: gen chart CRDs from controller-gen sources + drift gate incl. GMC-bundled==AGC (absorbs Q73); B) e2e via helm not make deploy; C) delete kustomize overlays (plan/drop-kustomize.md). |
...
| <a id="Q129"></a>Q129 | [Public GitHub Pages website](plan/website.md) | `docs` `infra` | ▶ | L | [Public site](plan/website.md): MkDocs scaffold + landing + vs-ARC + Pages workflow shipped, builds in CI. Remaining: public launch (gated on enabling Pages; Q99 done — landing copy must keep the honesty caveats); link reconcile → [Q52](#Q52). |
