...
| <a id="Q320"></a>Q320 | GMC gauges are v1-only; v2 deploys lack control-plane metrics | `infra` `bug` | 🔲 | S | All GMC gauge collectors read the v1 `ActionsGatewayList`, so a v2-only deploy reports zero `managed_gateways` + no `proxy_quota_*`/`egress_rules_stale`. Extend to count v2 gateways + EgressProxy. |
