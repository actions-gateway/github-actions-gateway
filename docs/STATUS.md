...
Last refreshed: 2026-05-28 (5d ✅ — `TestProxy_TLS_RejectsHTTP2_ALPN` added to `cmd/proxy/proxy_test.go`: drives the production `ListenAndServe` over a self-signed TLS cert, dials with `NextProtos: ["h2", "http/1.1"]`, and asserts (a) the negotiated protocol is `http/1.1` and (b) a CONNECT over that handshake returns the canonical `HTTP/1.1 200 Connection established` status line. Verified to fail with `actual: "h2"` and a server-side `bogus greeting "CONNECT ..."` log when the HTTP/2 disable is reverted — i.e. it catches the exact PR #59 regression. Earlier landmarks: 5c Tier-A `ProxyConnectWorks` ✅, named-pipe ✅, GithubRegistrar ✅, eviction retry CRD fields ✅, M2 envtest goroutine-leak suite ✅, credential rotation ✅, M3 metric assertions ✅, M4 test gaps ✅, open docs items ✅, AGC rename ✅, go-workspace prefix-match ✅, Make UX Phase 2 ✅, e2e test speed ✅, envtest/unit test split ✅, M2 kind activeSessions check ✅, ARC alignment ✅, JIT config plumbing for worker ✅. PR #59 fixes shipped: workload NP `ipBlock` → `podSelector`, proxy HTTP/2 disable, IP-range fetcher `actions`→`api+actions+web`, AGC TLS pool replace → append, wrapper `--startuptype workerprocess` → `spawnclient`.
