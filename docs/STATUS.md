...
|---|---|---|---|---|---|
| <a id="Q224"></a>Q224 | [GKE dogfood: route production CI (validation done)](plan/gke-dogfood.md) | `milestone` `infra` | 🔲 | M | Validated end-to-end on rc.3: control plane, creds, Q229 DNS, registration, real job→worker pod→GitHub. Default image now works via injection (Q235). Remaining: Part C2 GAG_RUNNER routing + turn on production CI (re-validates injection live). |
