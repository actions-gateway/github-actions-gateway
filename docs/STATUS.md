...
| <a id="Q359"></a>Q359 | [Worker right-sizing profiles (recommendations first)](plan/runner-sizing-profiles.md) | `infra` | 🔲 | L | Killer-feature gap vs ARC. Phase 1 (usage metrics + operator recipe) shipped; next: Phase 2 recommendations in RunnerSet status, Phase 3 opt-in profiles. Live validation of the sampler rides the next dogfood session; see the plan. |
