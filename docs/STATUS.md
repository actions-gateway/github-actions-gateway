...
| <a id="Q378"></a>Q378 | [Flake: TestReconcile_ReaperDefaults loses requeue race to baseline recheck](../cmd/agc/internal/controller/runnergroup_reaper_test.go) | `tests` `flake` | 🔲 | S | On slow -race CI the stub listener dies before reconcile #2, so ActiveCount()<MaxListeners swaps the asserted 4 m reaper requeue for the 15 s baseline recheck (PR #724; 10× local -race pass). Pin BaselineRecheckInterval high in reaper tests. |
| <a id="Q379"></a>Q379 | [lint-backlog: reject conflict markers in STATUS.md](../scripts/lint-backlog.sh) | `infra` `bug` | 🔲 | S | A stray `<<<<<<< HEAD` from a rebase resolution passed lint-backlog, CI status-lint, and merged to main via PR #724 (fixed same day). The linter parses rows and skips non-row lines, so markers sail through; grep-reject the three marker forms. |
| <a id="Q381"></a>Q381 | [lint-backlog: reject Q-IDs already used on main](../scripts/lint-backlog.sh) | `infra` `retro` | 🔲 | S | Two collisions 07-20 (Q361→Q373, Q362→Q380): branches read Next ID optimistically, so parallel sessions claim the same ID and it surfaces only at rebase. Diff a PR's new IDs vs main; fail on reuse. |
| <a id="Q382"></a>Q382 | [Verify CI checks exist, not just that none failed](development/testing.md#path-gated-workflows-verify-the-heavy-gates-actually-ran) | `docs` `retro` | 🔲 | S | A push produced no workflow runs for ~10 min (fixed by close/reopen). Absence of checks reads like nothing-failing — verify check-runs exist on the head SHA. Also ad-hoc shell is zsh: no word-splitting. |
