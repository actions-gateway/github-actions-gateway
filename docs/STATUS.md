...
| <a id="Q95"></a>Q95 | Production path for AGC `OTEL_*` tracing config via GMC | `infra` | 🔲 | S | AGC honours standard `OTEL_*` env for tracing (Q35/F2), but the GMC-built AGC Deployment can only set them via testing-gated `AGC_EXTRA_*`. Add a production path (forward `OTEL_*` or a spec field); keep OTLP auth headers out of env. |
