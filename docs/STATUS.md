...
|---|---|---|---|---|---|
| 5h | Fix `TestAGC_Reconciler_JobAcquisitionCycle` pool-exhausted flake | `tests` `bug` | ▶ | S | CI flake observed on PR #63 ([job 78394454901](https://github.com/actions-gateway/github-actions-gateway/actions/runs/26603989391/job/78394454901)): replacement listener exits with `non-retriable: pool exhausted: no agent available` index=1 after a job acquisition. Test sets `maxListeners=2` and enqueues the job after waiting only for "≥1 session registered" — but the reconciler may still be racing to create the second agent Secret when the replacement listener tries to claim it. Either tighten the test pre-wait to "all maxListeners agent Secrets exist" before EnqueueJob, or fix the underlying agent-pool readiness signal so a replacement spawn cannot observe a partially-populated pool. Reproduce via `for i in $(seq 1 50); do go test -race -run TestAGC_Reconciler_JobAcquisitionCycle -count=1 ./cmd/agc/internal/controller/integration/...; done` until the failure mode appears; passes locally on first try, fails ~1% in CI. |
