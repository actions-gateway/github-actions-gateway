...
// and rejects a gitHubURL whose host falls in a bound EgressProxy's noProxyCIDRs
// (Q322 — the gateway is the object that binds the GitHub host, so a gateway write
// can assemble the GitHub-bypass pair from its side).
//
...
// ActionsGateway kind, wired to the infra-only PriorityClass allowlist (Q284) and the
// manager's uncached API reader for the noProxyCIDRs GitHub-bypass guard (Q322). The
...
func SetupActionsGatewayWebhookWithManager(mgr ctrl.Manager, infraPriorityClasses *allowlist.PriorityClassAllowlist) error {
	v := &ActionsGatewayCustomValidator{InfraPriorityClasses: infraPriorityClasses, reader: mgr.GetAPIReader()}
