...
var _ = Describe("E2E_GMC_Isolation", Ordered, func() {
	...
	It("E2E_GMC_NetworkPolicyScopedToNamespace: NetworkPolicies exist in each namespace", func() {
		...
		Expect(utils.ResourceExists("networkpolicy", nsA, "actions-gateway-agc")).To(BeTrue())
		...
		Expect(utils.ResourceExists("networkpolicy", nsB, "actions-gateway-agc")).To(BeTrue())
	})
