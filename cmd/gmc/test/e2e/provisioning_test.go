...
var _ = Describe("E2E_GMC_Provisioning", Ordered, func() {
	...
	AfterEach(func() {
		if CurrentSpecReport().Failed() {
			...
			// scores the same lines into a verdict. Label-gated for the reason
			// the v2 sibling is, and that gate is also what keeps the duplicate
			// log dump to the one spec the proxy's account is about.
