...
type ProxyConfig struct {
	...
	// operator's responsibility. The GMC appends the cluster-internal destinations
	// automatically — svc.cluster.local, kubernetes.default.svc, localhost,
	// 127.0.0.1, and this cluster's API server ClusterIP, read from
	// KUBERNETES_SERVICE_HOST — so neither the API server nor the service CIDR
	// needs to be named here. Set this field only to exempt an additional internal
	// destination of your own; every entry bypasses the tenant's egress proxy and
	// so escapes its per-tenant egress-IP attribution, so keep the list short.
	// +optional
