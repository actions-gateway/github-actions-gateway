...
// TestV1alpha1_RunnerGroup_ApiserverWarnsOnDeprecation pins Q633. The
// actions-gateway.github.com/v1alpha1 RunnerGroup CRD carries deprecated: true
// and a deprecationWarning, so the removal notice reaches an operator at apply
// time rather than only in the docs. v2alpha1 has warned since Q411; v1alpha1,
// the version whose users are furthest behind, did not.
//
// Deprecation removes nothing: the create and the read below both succeed, and
// the version stays served until v2.0.0.
func TestV1alpha1_RunnerGroup_ApiserverWarnsOnDeprecation(t *testing.T) {
	const ns = "v1alpha1-deprecation"
	createNSForAGC(t, ns)

	c, rec := warningCapturingClient(t)

	rg := newRunnerGroup(ns, "warned", 1)
	require.NoError(t, c.Create(ctx, rg))
	t.Cleanup(func() { _ = k8sClient.Delete(context.Background(), rg) })

	created := rec.take()
	assert.Contains(t, created, "actions-gateway.github.com/v1alpha1 RunnerGroup is deprecated")
	assert.Contains(t, created, "actions-gateway.com/v2beta1 RunnerSet", "the warning must name the replacement")
	...
	require.NoError(t, c.Get(ctx, client.ObjectKeyFromObject(rg), &got))
