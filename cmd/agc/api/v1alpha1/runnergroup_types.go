...
// the AGC declines to claim the next job, leaving it queued at GitHub for redelivery,
// so no job ever waits for a token while holding a GitHub job lock.
...
type RunnerGroupStatus struct {
	...
	// WorkersUnschedulable. The type and reason string constants are exported from
	// this package (see conditions.go).
	// +optional
	// +listType=map
	// +listMapKey=type
