...
3. **After making changes** — review the diff to confirm it matches the design, is well tested, and achieves the intent.
   ...
   - **Moved, archived, or added a doc? `make check` covers it** — `doc-links`, `plan-index-check`, and `no-plan-refs-check` all run inside it.
     Moving a doc is exactly what breaks them (a relative link re-based one level, a dangling `#QNNN` anchor, a code comment citing the path that just changed), so read their lines in the output rather than only the exit code; run the three directly when you want a fast answer without the full gate — plus the three that fire on *writing* prose rather than moving a doc, **`em-dash-check`, `md-reflow-check`, and `release-pins-check`, whenever you wrote or edited any**.
     Each runs in seconds and each is what a docs change trips at the very end of a full `make check`: `em-dash-check` twice in two days (Q699, then the session that closed it), then all three at once in Q715, costing three ten-minute cycles.
     Run them as a set the moment the prose is written, not after the code gate.
