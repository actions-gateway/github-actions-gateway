...
**Never `cd "$(git rev-parse --show-toplevel)"`** — the `$(…)` prompts under both guards, and each Bash session already starts at the worktree root. Isolate every directory change in a subshell (`(cd cmd/agc && go test ./...)`) so the parent cwd can't drift; on the rare occasion you genuinely must reset cwd, `cd` to the **literal** worktree path (no `$(…)`/`$VAR`/`~`).
