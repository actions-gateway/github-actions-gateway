...
**Avoid large heredocs in Bash commands** — a multi-line inline body can defeat the hooks' command-string parsing, turning a safe call into a spurious prompt or parse failure. Write the content to a file with the Write tool (throwaway content → gitignored `tmp/`) and have the Bash command read it.

**Never `cd "$(git rev-parse --show-toplevel)"`** — the `$(…)` prompts under branch-guard (workspace-guard 1.5+ resolves this one substitution itself), and each Bash session already starts at the worktree root. Isolate every directory change in a subshell (`(cd cmd/agc && go test ./...)`) so the parent cwd can't drift; on the rare occasion you genuinely must reset cwd, `cd` to the **literal** worktree path (no `$(…)`/`$VAR`/`~`).
...
- **Pin the target explicitly on every ad-hoc `kubectl`/`gcloud` — never rely on the active context.** Parallel sessions share `~/.kube/config` and `~/.config/gcloud`, so another session can silently repoint yours mid-task. Pass `--context` (kubectl) and `--project`/`--account`/`--zone|--region` (gcloud) per invocation; never `use-context`/`gcloud config set`. Detail: kind-iteration.md § Target the cluster explicitly.
- **Verify the resolved target before any mutating command — an explicit `--context` still points *somewhere*.** Before a destructive verb (kubectl `apply`/`delete`/`scale`/`patch`/`rollout`, helm `upgrade`/`uninstall`, gcloud/terraform `delete`/`destroy`), echo the effective target and stop if it looks like prod; never proceed against a prod-looking target without explicit user confirmation. The GKE dogfood cluster is hard-classified prod — the prod-guard hook denies ad-hoc destructive commands against it (`PROD_GUARD_OVERRIDE=<reason>` for an intentional one); add legitimate non-prod targets to `.claude/prod-guard.json` under `nonprod` rather than approving repeatedly. Detail: kind-iteration.md § Verify the resolved target.
...
- **Never foreground-poll CI, logs, or files.** No blocking `gh pr checks --watch` / `gh run watch` / `tail -f` / `sleep`-loop on the main thread — let Auto-fix/PR-monitor and background tasks track it, or take one non-blocking snapshot and move on (testing.md § [Never foreground-poll CI, logs, or files](docs/development/testing.md#never-foreground-poll-ci-logs-or-files)).
- **Slow tiers get an explicit timeout or run in the background.** envtest, kind e2e, and `go test -race`/`make test-race` overrun the Bash default timeout and get killed — pass an explicit timeout or launch as a background task, never a default-timeout foreground run (testing.md § [Slow tiers need an explicit timeout or a background run](docs/development/testing.md#slow-tiers-need-an-explicit-timeout-or-a-background-run)).
...
| Editing `docs/STATUS.md` (any change to the Queue, Deferred, Progress table, or header) | `docs/development/maintaining-backlog.md` (format = the `backlog` skill; lint: `scripts/lint-backlog.sh`) — take new IDs from `**Next ID:**` and bump it in the same edit; done rows are **deleted**; Notes and Deferred triggers are **present tense, no status history**, **hard 250-char cap**, **>200 chars must link a doc** (a link counts its full source length). If fitting the cap means dropping a decision or a finding, write the doc first — durable rationale → `docs/design/`, in-flight context → `docs/plan/` — whatever the item's `Sz`. |
