...
3. **After making changes** — review the diff to confirm it matches the design, is well tested, and achieves the intent.
   ...
   - **Run `make docs-gates` the moment prose is written or a doc moves, not after the code gate.** It is every gate a prose change can fail, in seconds; `make check` runs the same ones ten minutes later at its very end, which is where this lands otherwise (Q699, the session that closed it, Q715, Q844).
