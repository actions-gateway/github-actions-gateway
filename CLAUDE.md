...
3. **After making changes** — review the diff to confirm it matches the design, is well tested, and achieves the intent.
   ...
   If that was a plan's last STATUS reference, update its `docs/plan/README.md` row and archive the plan in the same change — don't defer to an audit (`make plan-index-check` enforces this; see `docs/development/maintaining-backlog.md#archiving-completed-plan-docs`).
   - **Moved, archived, or added a doc? `make check` covers it** — `doc-links`, `plan-index-check`, and `no-plan-refs-check` all run inside it.
     Moving a doc is exactly what breaks them (a relative link re-based one level, a dangling `#QNNN` anchor, a code comment citing the path that just changed), so read their lines in the output rather than only the exit code; run the three directly when you want a fast answer without the full gate — plus **`em-dash-check` whenever you wrote or edited prose**, which fires on *writing* a doc rather than moving one, runs in ~2s, and is what a docs change most often trips at the very end of a full `make check` (twice in two days: Q699, then the session that closed it).
...
- **go-throttle** prefixes `go build`/`go test` with the local throttle prefix: a bare form is rewritten and auto-allowed; a compound, redirected, or wrapped `-race` form is rewritten and *asked* (no longer blocked); a `-race` form holding *two* go invocations is *denied* with the reason.
...
- **Merges go through the merge queue** — `gh pr merge --squash` enqueues; the queue validates the candidate merge result and a failing entry is kicked back to its PR with the failure attached (the signal pr-sentinel reacts to).
  A push to a queued PR dequeues it and restarts its checks, so prefer everything in the first push and weigh the CI a push restarts (docs-only gates are seconds; the e2e matrix is a quarter-hour).
