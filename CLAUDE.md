...
- **A bulk rename/rewrite is verified by reconciliation, not by an empty leftover grep** — an empty result cannot tell "no sites remain" from "my query never matched them" (Q571 silently skipped 62 files that way). Assert a known-affected site *did* change, and reconcile before/after occurrence counts. Detail: testing.md § [A bulk mechanical change proves itself by reconciliation](docs/development/testing.md#a-bulk-mechanical-change-proves-itself-by-reconciliation-not-by-an-empty-leftover-query).
