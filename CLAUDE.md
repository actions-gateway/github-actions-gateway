...
4. **Commit when done** — once a task is complete and validated, commit with git **without asking for permission** — committing is automatic in this repo. Keep commits small and focused. Never commit broken code or failing tests. **Always commit `docs/STATUS.md` changes in their own isolated commit**, separate from code and plan-doc changes. `docs/STATUS.md` is high-contention across concurrent branches; isolating its changes makes rebase conflicts trivial to resolve.
5. **Open a PR when the task is finished** — after committing and pushing the branch, open a PR with `gh pr create`, **automatically and without asking for permission**. But first, stop and ask yourself: **"Is this ready for review?"** Open the PR only when you can honestly answer yes to all of:
...
`scripts/local-throttle.sh prefix` prints the platform throttle (`taskpolicy …` on macOS, `nice`[` + ionice`] on Linux) or **nothing** on CI/headless/SSH shells, so the prefix is a safe no-op there and needs no `if`. The failure mode is real — an unthrottled `go test -race` in a parallel worktree session crashed WindowServer during the session that filed Q92.

A `PreToolUse` hook (`scripts/claude-go-throttle-hook.sh`, wired in `.claude/settings.json`) now closes the Makefile-only coverage gap automatically: a **bare** `go build`/`go test` you run through the Bash tool is transparently rewritten to carry the same prefix, so a forgotten prefix no longer means an unthrottled run. It auto-allows *only* that bare form — never a **compound** command (`cd … && go test …`) or one with a **redirect** (`go test … > out`), so its `allow` can't ride a non-go segment or an outside-workspace redirect past the permission system or the branch-guard/workspace-guard hooks. Such a command carrying `-race` is therefore **blocked** with a reminder to add the prefix yourself — keep using the `TP=…; (cd … && $("$TP" prefix) go test -race ./...)` form above for those. Like the Makefile throttle, the hook is a no-op on CI/headless/SSH (it reuses `local-throttle.sh`).
...
- **A *real* non-git command in a chain defeats auto-approval — but labels, no-ops, and read-only pagers ride along.** Auto-approval fires only when every segment is recognized-safe: a git/gh command, a read-only pager piped after one (`git log | head`, `gh pr checks 123 | head -20`), or a side-effect-free label/no-op (`echo`/`printf`/`true`/`false`/`:`). So `git log … ; echo "---" ; git log …` stays auto-approved, but `git commit && <real-cmd>` prompts — the real command can't ride along. Two things also drop an otherwise-safe chain back to a prompt: an output redirect that writes a file (`git log > f`, `echo x > f` — but **not** `/dev/null`, the standard streams, or fd-dups like `2>&1`), and a command substitution (`$(…)` / `` `…` ``).
