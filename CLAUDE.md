...
A `PreToolUse` hook (`scripts/claude-go-throttle-hook.sh`, wired in `.claude/settings.json`) now closes the Makefile-only coverage gap automatically: a **simple** `go build`/`go test` you run through the Bash tool is transparently rewritten to carry the same prefix, so a forgotten prefix no longer means an unthrottled run. It cannot safely auto-rewrite a **compound** command (`cd … && go test …`), so a compound carrying `-race` is **blocked** with a reminder to add the prefix yourself — keep using the `TP=…; (cd … && $("$TP" prefix) go test -race ./...)` form above for those. Like the Makefile throttle, the hook is a no-op on CI/headless/SSH (it reuses `local-throttle.sh`).
