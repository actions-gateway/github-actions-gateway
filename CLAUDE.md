...
| Picking the next task, tracking progress, adding new items | `docs/queue/` (`scripts/docs/next-task.sh`; there is no `make next-task`) — it skips any item an open PR names and says which, so read the skip before overriding it with `--allow QNNN` (Q990) |
