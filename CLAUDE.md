...
3. **After making changes** — review the diff to confirm it matches the design, is well tested, and achieves the intent.
   ...
   **Closing an item reaches outside `docs/queue/`** — dead `#QNNN` anchors, a roadmap bullet whose orphaned continuation line breaks the *previous* bullet's word cap, a plan doc to archive (which re-bases its own outbound links) and its `docs/plan/README.md` row.
   ...
   - **Run `make docs-gates` the moment prose is written or a doc moves, `make shellcheck` the moment a `scripts/*.sh` is edited, and `make lint` while the Go is being written — not after the code gate.** Each is the cheap bulk of what that change can fail, in seconds (`shellcheck` 37 s over all 210 scripts; `lint` is change-scoped to the modules the branch can affect); `make check` runs the same ones ten minutes later at its very end, `lint` as one of three sequential heavy phases, which is where this lands otherwise (Q699, the session that closed it, Q715, Q844, Q870).
     ...
     It also does **not** run `roadmap-check`, which caps a `docs/features.md` entry at 45 words, so editing that page needs `scripts/docs/check-roadmap.sh` too — a badge spends about three words of the entry's budget, which is enough to push a passing entry over.
     **`shellcheck` has the same gap**: a new script also needs `make errexit-prologue-check`, which requires `shopt -s inherit_errexit` under `set -euo pipefail` and runs beside shellcheck in one CI job, so a green local `make shellcheck` still reddens that job.
     Both were found the same way, a PR going red on a gate its named target never ran, which is why the line above claims the cheap bulk rather than the whole set.
...
- **piped-gate** *denies* when a command whose exit status is the answer (`make`, `go build/test`, a `scripts/` gate, `git pull`/`push`/`rebase`/`merge`/`commit`) is piped into a filter, when a command reads `$PIPESTATUS` (bash-only; empty in zsh), or when a **backgrounded** call ends in something that drops the gate's status (an `echo`, a `||` fallback, a trailing `&`). Fix: `cmd > tmp/out.log 2>&1; echo "EXIT=$?"`, then grep the file — plus `rc=$?; echo "EXIT=$rc"; exit $rc` when backgrounded.
  It also denies at two repo-state moments: a `git push` whose moved base overlaps this branch's own files (Q665; rebase and re-run the gate, or a queue kickback finds it), and a `gh pr create` whose changed *lines* an open PR already changes (Q668, narrowed to line ranges by Q862; read that PR and override with the reading; re-scoping to dodge the overlap is the wrong move).
  Both discount the merge-driver-owned files, but the push check only while `git merge-tree` says the merge resolves, so a merge-driver-owned-file-only branch can still be told it is dirty (Q790).
