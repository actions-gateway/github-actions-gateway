...
- **A CLI tool isn't on `PATH`? Run `scripts/check-tools.sh` (or `make doctor`) and surface its guidance** — never hardcode an absolute tool path or mutate `PATH` inline for one command. Detail: `CONTRIBUTING.md`.
