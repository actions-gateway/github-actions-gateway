...
- New near-term or long-term work → add to the backlog in priority order: one file per item at `docs/queue/QNNN.md`, ranked rather than positioned.
...
A row's title caps at 72 chars and its body does not cap, so the *why* belongs in the row itself.
...
1. **Before making changes** — review `DESIGN.md` and any relevant docs in `docs/` to confirm the plan matches the design intent.
   ...
   - **The status you report is a claim too.** "Green", "stuck", "converging" need a measurement, not an inference: an exit code read through a pipe is the pipe's (`make check | tail` reports `tail`'s), a backgrounded `; echo "EXIT=$?"` chain reports the `echo`'s 0 to the task notification (end it `rc=$?; echo "EXIT=$rc"; exit $rc`), a tool that exits 0 silently may have checked nothing, a state seen once is not a steady state, a count grouped by symptom is not a measurement of cause (exercise the system that would produce the effect; don't correlate records), a count or superlative you cite is re-derived as you write it, never recalled from an earlier scan, a gate that fails on your branch is not yours until it fails on the base too (Q741 cost two wrong hypotheses), and a check sequenced before a state-changing command with `;` does not gate it.
     ...
     **When the thing being explained is a measurement, the changelog of whatever produced it is evidence too**, and "not answerable from this dataset" does not license "not answerable": a six-week anomaly written off as unanswerable was named, dated and quantified by one changelog entry.
     **A claim with no evidence behind it is the one nothing prompts you to check**, because every rule above fires on evidence being handled and an assertion resting on none reads as background instead.
     Three shapes bite: **provenance** ("this tooling is vendored from the skill" — read off resemblance, and backwards; a `git log` on each side settles it), **a reading that aged** (an audit of an upstream file, correct when taken and refuted 35 minutes later by a merge in a repo this one does not own), and **a decision recalled from a summary** (a user's answer survives compaction as a bare phrase, detached from the question it answered; re-read the question before acting on it, and never state it back as theirs unchecked).
     Say when a read was taken, and re-read before acting on it.
     **Invoke the `verify-claims` skill** before reporting a gate result, diagnosing a failure, writing a test or gate, or letting a claim about where code came from decide something; it states these rules in general, and testing.md § [The status you report is a claim too](docs/development/testing.md#the-status-you-report-is-a-claim-too) records what each one cost here.
...
3. **After making changes** — review the diff to confirm it matches the design, is well tested, and achieves the intent.
   ...
   **A design-doc-only update is not sufficient: if a change alters what an operator does, configures, or observes (defaults, fields, failure modes, annotations, metrics, admission rejections), the operator-facing `docs/operations/` docs must be updated too.** Then update the backlog: delete the completed item — **except a `flake` item: its fix parks it in flake watch, and only a groom retires it to the ledger** ([lifecycle](docs/development/maintaining-backlog.md#flake-fixes-go-first)); update the plan's row in `docs/plan/README.md` if a plan-level status changed.
   **Closing an item reaches outside `docs/queue/`** — dead `#QNNN` anchors, a roadmap bullet whose orphaned continuation line breaks the *previous* bullet's word cap, a plan doc to archive (which re-bases its own outbound links) and its `docs/plan/README.md` row.
   ...
   - **A change under `docs/` is a change to a live web page.** That tree publishes to [`actions-gateway.com`](https://actions-gateway.com/) via MkDocs, so review a page a reader scans (`index.md`, `why-gag.md`, `features.md`, the `operations/` docs, and the backlog, which the `dev` version publishes at `/dev/queue/`) with the `rendered-page-review` skill against `make docs-serve`: it measures the render, which `make check` never builds ([website.md](docs/development/website.md#measure-the-render-the-source-cannot-answer-these-questions)).
     **Adding or moving a page under `docs/` needs `make docs-build` too**, whatever a reader would make of it: `--strict` fails on a published page in no nav section, so 173 store pages reddened CI on a branch whose `make check` was green twice ([what belongs in nav](docs/development/website.md#what-belongs-in-nav)).
     Nothing outside `docs/` publishes, so `README.md`, `DESIGN.md`, and `CONTRIBUTING.md` render on github.com only.
   - **Run `make docs-gates` the moment prose is written or a doc moves, `make shellcheck` the moment a `scripts/*.sh` is edited, and `make lint` while the Go is being written — not after the code gate.** Each is the cheap bulk of what that change can fail, in seconds (`shellcheck` 37 s over all 210 scripts; `lint` is change-scoped to the modules the branch can affect); `make check` runs the same ones ten minutes later at its very end, `lint` as one of three sequential heavy phases, which is where this lands otherwise (Q699, the session that closed it, Q715, Q844, Q870).
     ...
     It **does** now run `roadmap-check`, which caps a `docs/features.md` entry at 45 words (a badge spends about three of them, enough to push a passing entry over), along with the six other page-scoped gates missing from it until Q930 and the three store gates missing until Q1040, so a prose edit no longer needs a hand-run checker of its own.
     **Commit a `docs/queue/` row before running it**: the store rules read `HEAD`, never the working tree, so an uncommitted row is invisible to the rule that would reject it ([why](docs/development/maintaining-backlog.md#commit-the-row-then-run-the-gate)).
     **`shellcheck` still has the gap**: a new script also needs `make errexit-prologue-check`, which requires `shopt -s inherit_errexit` under `set -euo pipefail`, and `make script-modes-check`, which requires an entry point be committed executable (`git update-index --chmod=+x`) and a sourced `lib/` file not; both run beside shellcheck in one CI job, so a green local `make shellcheck` still reddens that job.
     Both gaps were found the same way, a PR going red on a gate its named target never ran, which is why the line above claims the cheap bulk rather than the whole set.
...
5. **Open a PR when the task is finished** — after committing and pushing, open it with `gh pr create`, automatically and without asking.
   ...
   - Every doc per step 3 is updated (design **and** operator-facing), and the backlog reflects the completed work.
...
Anchor it to command position (the go-throttle hook is the worked example: it parses instead of scanning, so a name inside a quoted string or a heredoc body is a word and never a command, Q624), and assert both directions (`scripts/agent/foreground-guard-patterns-test.sh`, `scripts/agent/claude-go-throttle-hook-test.sh`): a pattern that stops matching fails as silently as one that matches everything.
...
- **Backlog changes get their own commit when the change also touches code** — the item is the *why* and the code is the *what*, so a reviewer should not have to separate them by hand.
  This is a courtesy, not a gate: the rule that enforced it existed to contain conflicts on one contended file, and one file per item removed that cost (Q889).
- **Commit explicit file paths, never a directory** (`git add cmd` and friends), and prefer `git commit -- <paths>` over `git add` plus a bare `git commit`: the pathspec form makes the commit's contents independent of whatever is already staged.
  Reading `git status` first is not enough on its own — `git mv` stages its rename immediately, and in `--short` output that is `R` in the *staged* column, which is easy to read as pending (Q844 shipped a rename into a backlog-only commit that way, caught by a gate that has since retired with the table).
  ...
  **A pathspec resolves against the tree and index, not against a commit**, so an untracked file needs `git add` first or the commit fails `did not match any file(s) known to git`, and `git commit --amend -- <path>` errors on a path the amended commit already deleted; a *tracked* file you have deleted needs no staging ([what does and does not follow](docs/development/maintaining-backlog.md#closing-a-row-what-else-moves)).
...
- **Merges go through the merge queue, and `gh pr merge` cannot enqueue into it** — it routes through `enablePullRequestAutoMerge`, which this repo disables (`allow_auto_merge: false`), so every form fails `Auto merge is not allowed for this repository` (re-measured 2026-09-04 on gh 2.100.0, with all 9 required checks green and `mergeStateStatus: CLEAN`, so it is not gh falling back on unfinished checks).
  ...
  **A heal that follows a queue eviction may also re-enqueue**, but only when `scripts/agent/pr-requeue-eligible.sh --assess <pr>` (before the rebase), `--rebased <pr>` (once the rebase is pushed, re-binding the record to the new head) and `--confirm <pr>` (after CI) all pass — a prior *human* enqueue, no current queue entry, and conflicts confined to the merge-driver-owned files.
  That restores the maintainer's own decision; a **first** enqueue is never yours to make (Q692).
  All three verdicts gate that re-enqueue, and the mutation above is how you run it once they pass; report the `ELIGIBLE` line either way, so the decision is auditable.
...
| Picking the next task, tracking progress, adding new items | `docs/queue/` (`scripts/docs/next-task.sh`; there is no `next-task` make target) — it skips any item an open PR names and says which, so read the skip before overriding it with `--allow QNNN` (Q990) |
...
| Editing the backlog (`docs/queue/QNNN.md`) | `docs/development/maintaining-backlog.md` (format = the `session-backlog` skill, but that doc wins where they overlap; lint: `make queue-lint`) — allocate new IDs with `make queue-id TITLE="…"`, which searches for near-duplicates before it claims — read the candidates it prints (there is no `Next ID` line; the globally-installed skill still says there is, and this repo overrides it); done rows are **deleted**; Notes and Deferred triggers are **present tense, no status history**. **The title caps at 72 chars and the body does not cap**: `queue.py` enforces `TITLE_MAX` alone, and the rendered table truncates notes to their first ~140 chars, so lead with the point rather than trimming to fit. Where a row would carry durable rationale, write the doc first — durable rationale → `docs/design/`, in-flight context → `docs/plan/` — whatever the item's `Sz`. |
