...
**Automatic disruption retry with fast lock cancel.** When a worker loses its job to infrastructure, the AGC stops lock renewal at once, so GitHub cancels the job when the outstanding lock lapses — within the remaining lock window, ~10 minutes at worst, instead of stranding it until someone clears the runner by hand. The AGC then calls GitHub's rerun API to reschedule.
...
**Per-tenant dedicated egress IP pool.** All GitHub traffic from the AGC and worker pods routes through a per-tenant proxy pool ([Tier 3](#architecture)), so each tenant gets egress IPs never shared with other tenants. That gives per-team allowlisting on the GitHub side, clean per-tenant audit attribution, and a contained blast radius when a rate limit or abuse flag lands.
